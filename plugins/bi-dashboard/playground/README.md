...
Run the plugin in the existing workspace playground using the fixture in `../example`. The demo workspace includes `.pi/extensions/bi-dashboard`, which loads the front plugin as a workspace-local extension. For live query data, use the eval runner or another host that loads `@hachej/data-bridge` as a trusted server plugin:

```bash
pnpm --filter @hachej/data-bridge build
...
The runner seeds a temp workspace from `../example` and loads `@hachej/data-bridge` plus `@hachej/boring-bi-dashboard` explicitly.

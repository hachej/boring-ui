...
import { LedgerAskUserStore, legacyAskUserPath, type AskUserAttentionCapability } from "./ledgerAskUserStore"
...
export type AskUserServerPluginOptions = {
  ...
  onClose?: () => void
  attention?: AskUserAttentionCapability
}
...
export function createAskUserServerPlugin(options: AskUserServerPluginOptions): WorkspaceServerPlugin {
  ...
  const store = options.store ?? options.runtime?.store ?? createDefaultStore(options.workspaceRoot, options.attention)
  ...
  const lifecycle: FastifyPluginAsync = async (app) => {
    ...
    if (store instanceof LedgerAskUserStore) await store.importLegacyOnce()
...
function createDefaultStore(workspaceRoot: string | undefined, attention: AskUserAttentionCapability | undefined): AskUserStore {
  if (!workspaceRoot) throw new Error("createAskUserServerPlugin requires workspaceRoot when store is not provided")
  if (!attention) throw new Error("createAskUserServerPlugin requires the host attention capability when store is not provided")
  return new LedgerAskUserStore(attention, legacyAskUserPath(workspaceRoot))
}

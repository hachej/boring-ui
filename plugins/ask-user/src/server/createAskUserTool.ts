...
export function createAskUserTool(options: AskUserToolOptions): AskUserToolDefinition {
  return {
    ...
    async execute(toolCallId, params, signal, sessionId) {
      ...
      try {
        const result = await options.runtime.ask({ ...input, toolCallId, sessionId: sessionId ?? resolveSessionId(options.sessionId) }, signal)

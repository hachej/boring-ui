...
function fixture(options: { authorizeError?: boolean } = {}) {
  ...
  const resolver: TrustedTaskToolBindingResolver = {
    run: async (_ctx, operation) => await operation(await resolve()),
  }
...
describe("manage_tasks execution", () => {
  ...
  it("redacts inaccessible linked session ids while preserving unlink handles", async () => {
    ...
    denied.links.push({
      ...
      taskId: "1",
      agentTypeId: "alpha",
      sessionId: "hidden-native-session",
      createdAt: "2026-07-18T00:00:00.000Z",
    })

    const result = await denied.tool.execute({ action: "get", adapterId: "source-a", taskId: "1" }, context)
    expect(result).toMatchObject({
      details: { links: [{ id: "hidden-link", adapterId: "source-a", taskId: "1" }] },
    })
    expect((result.details as { links: Array<Record<string, unknown>> }).links[0]).not.toHaveProperty("sessionId")
  })
  ...
  it("unlinks by link id without authorizing the transcript", async () => {
    ...
    const unlinked = await fixtureValue.tool.execute({ action: "unlink_session", linkId }, context)
  ...
  it("registers exactly one manage_tasks tool in the Tasks server plugin", () => {
    const plugin = createTasksServerPlugin({ sources: [source()], agentTypeId: "alpha" })

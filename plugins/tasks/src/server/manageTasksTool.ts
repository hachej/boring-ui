  ...
  return {
    ...
    async execute(params, ctx: ToolExecContext): Promise<ToolResult> {
      ...
      try {
        ...
        action = input.action
        return await bindingResolver.run(ctx, async (binding) => {
          ...
          case "get": {
            const [task, adapter, storedLinks] = await Promise.all([
            ...
            const links = await Promise.all(storedLinks.map(async (link) => {
              try {
                if (link.agentTypeId !== binding.agentTypeId) throw new Error("Agent owner mismatch")
                await binding.authorizeSession(link.sessionId)
                return link
              } catch {

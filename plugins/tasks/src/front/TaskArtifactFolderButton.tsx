  ...
  return (
    <>
      <button
        ...
        disabled={busy}
        className="grid size-7 place-items-center rounded-lg text-muted-foreground opacity-80 outline-none transition-colors hover:bg-muted hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/40 disabled:cursor-wait disabled:opacity-40 group-hover:opacity-100"
        aria-label={`Open artifact folder for ${task.number}`}
        title={error ?? "Open task folder"}
      >
        <FolderOpen className="size-3.5" strokeWidth={1.75} aria-hidden="true" />
      </button>

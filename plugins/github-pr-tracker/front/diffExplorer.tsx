...
function expansionFor(paths: string[]): Record<string, boolean> {
  const updates: Record<string, boolean> = {}
  for (const path of paths) updates[path] = true
  return updates
}
...
export function DiffExplorer({ pr }: { pr: PullRequest }) {
  ...
  React.useEffect(() => {
    ...
    setExpandedFolders((current) => {
      const needed = ancestorsOf(selectedFile.path).filter((path) => !current[path])
      return needed.length === 0 ? current : { ...current, ...expansionFor(needed) }
    })
    const row = treeScrollRef.current?.querySelector(`[data-file-row="${CSS.escape(selectedFile.path)}"]`)
    row?.scrollIntoView({ block: "nearest" })
  }, [selectedFile?.path])

  const selectFile = (path: string) => {
    setSelectedFilePath(path)
    setExpandedFolders((current) => ({ ...current, ...expansionFor(ancestorsOf(path)) }))
  }
  ...
  const selectFolder = (folder: string) => {
    ...
    if (first) setExpandedFolders((current) => ({ ...current, ...expansionFor([...(folder ? [folder] : []), ...ancestorsOf(first.path)]) }))
  }
    ...
    <section ref={sectionRef} tabIndex={-1} className="space-y-3 outline-none" onKeyDown={handleKeyDown} onClick={focusExplorer}>
      ...
      <div className="flex min-h-0 overflow-hidden rounded-lg border border-border" style={{ height: 600, maxHeight: "72vh" }}>
        <div className="flex min-h-0 flex-col" style={{ width: treeWidth, flex: `0 0 ${treeWidth}px` }}>
          <div ref={treeScrollRef} className="min-h-0 flex-1 overflow-auto">
            <FileTree
              files={scopedFiles}
              sort={sort}
              selectedFilePath={selectedFile?.path}
              expandedFolders={expandedFolders}
              onToggleFolder={(path) => setExpandedFolders((current) => ({ ...current, [path]: !current[path] }))}
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <div className="min-h-0 flex-1 overflow-auto" tabIndex={0}>
            {selectedFile ? (
              <DiffViewer file={selectedFile} prUrl={pr.url} />
            ) : (

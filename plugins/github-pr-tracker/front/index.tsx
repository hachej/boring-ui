...
function usePrData(pluginClient: WorkspacePluginClient) {
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      ...
      running = true
      try { await requestServerRefresh(pluginClient); if (!cancelled) await refresh() } catch {}
      finally { running = false }
    }
    const id = window.setInterval(() => { void tick() }, 60_000)
    return () => { cancelled = true; window.clearInterval(id) }
  }, [pluginClient, refresh])
...
function IssuesKanban({ data, onRefresh, pluginClient }: { data: PrData; onRefresh: () => void; pluginClient: WorkspacePluginClient }) {
  ...
  const labelIssue = (issue: IssueCard, add: string[], remove: string[], key: string) => {
    ...
    void run(key, () => requestAgentLabelIssue(pluginClient, issue.number, add, remove))
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button variant="secondary" onClick={() => void run("classify", () => requestAgentClassifyIssues(pluginClient))} disabled={!!busy}>{busy === "classify" ? <Spinner className="size-3" /> : "LLM classify: easy / needs-plan"}</Button>
      </div>
...
function MainPane({ params, containerApi }: PaneProps<{ number?: number; view?: "issues" | "prs" }>) {
  ...
  const { data, loading, error, refresh } = usePrData(pluginClient)
  const number = params?.number
  const [view, setView] = React.useState<"issues" | "prs">(params?.view ?? "issues")
  ...
  const classifyIssues = async () => {
    setClassifying(true)
    try { await requestAgentClassifyIssues(pluginClient) }
  ...
  return (
    <div className="flex h-full min-h-0 min-w-0 flex-col bg-background text-foreground">
      <div className="flex shrink-0 items-center gap-2 border-b border-border px-3 py-1.5 text-xs text-muted-foreground">
        ...
        {typeof number !== "number" && <div className="ml-auto flex items-center gap-1">
          <Button variant={view === "issues" ? "secondary" : "ghost"} size="xs" onClick={() => setView("issues")}>Issues</Button>
          <Button variant={view === "prs" ? "secondary" : "ghost"} size="xs" onClick={() => setView("prs")}>PRs</Button>
          <Button variant="ghost" size="xs" onClick={() => void classifyIssues()} disabled={classifying}>{classifying ? <Spinner className="size-3" /> : "LLM classify"}</Button>
        </div>}
        <Button variant="ghost" size="xs" className={typeof number === "number" ? "ml-auto text-muted-foreground" : "text-muted-foreground"} onClick={() => void refresh()} disabled={loading}>
      ...
      <div className="min-h-0 min-w-0 flex-1 overflow-auto p-4">
        <div className={typeof number === "number" ? "mx-auto max-w-5xl min-w-0" : "w-full min-w-0 max-w-none"}>
          ...
          {data && typeof number !== "number" && view === "prs" && (
            <PullRequestsDashboard prs={prs} onOpenPr={openPr} />
          )}
          {data && typeof number !== "number" && view !== "prs" && (
            <IssuesKanban data={data} onRefresh={() => void refresh()} pluginClient={pluginClient} />
          )}
...
export default definePlugin({
  ...
  panels: [
    { id: MAIN_PANEL_ID, label: "GitHub PR Tracker", placement: "workspace-page", component: MainPane },
  ],

...
export function createAppRunnerServerPlugin(options: AppRunnerServerPluginOptions): WorkspaceServerPlugin {
  ...
  return defineServerPlugin({
    ...
    label: "Apps",
    systemPrompt: [
      "An \"app\" is the app/ folder in the workspace: index.js (an ES module, Cloudflare-Worker-style, exporting a `fetch(request, env, ctx)` handler — env.APPDATA is an RPC-based data store) plus index.html and any static assets it needs.",
      "Call publish_app to deploy the app/ folder and get back a live URL. Use list_app_versions, rollback_app, and activate_app_version to manage published versions.",

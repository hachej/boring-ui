...
function matchesWorkspace(ctx: AutomationStoreCtx, workspaceId: string | undefined): boolean {
  if (!ctx.workspaceId) return true
...
function applyRunPatch(run: AutomationRun, patch: AutomationRunPatch): AutomationRun {
  ...
  for (const [key, value] of Object.entries(patch) as Array<[keyof AutomationRunPatch, AutomationRunPatch[keyof AutomationRunPatch]]>) {
    ...
    if (value === null) {
      deleteNullableRunField(next, key)
      continue
    }
    setRunField(next, key, value)
  }
  return next
}

function setRunField(run: AutomationRun, key: keyof AutomationRunPatch, value: NonNullable<AutomationRunPatch[keyof AutomationRunPatch]>): void {
  switch (key) {
    case "sessionId": run.sessionId = String(value); break
    case "status": run.status = value as AutomationRun["status"]; break
    case "scheduledFor": run.scheduledFor = String(value); break
    case "startedAt": run.startedAt = String(value); break
    case "completedAt": run.completedAt = String(value); break
    case "durationMs": run.durationMs = Number(value); break
    case "inputTokens": run.inputTokens = Number(value); break
    case "outputTokens": run.outputTokens = Number(value); break
    case "totalTokens": run.totalTokens = Number(value); break
    case "error": run.error = String(value); break
    default: break
  }
}

function deleteNullableRunField(run: AutomationRun, key: keyof AutomationRunPatch): void {
  switch (key) {
    case "sessionId": delete run.sessionId; break
    case "scheduledFor": delete run.scheduledFor; break
    case "startedAt": delete run.startedAt; break
    case "completedAt": delete run.completedAt; break
    case "durationMs": delete run.durationMs; break
    case "inputTokens": delete run.inputTokens; break
    case "outputTokens": delete run.outputTokens; break
    case "totalTokens": delete run.totalTokens; break
    case "error": delete run.error; break
    default: break
  }
}

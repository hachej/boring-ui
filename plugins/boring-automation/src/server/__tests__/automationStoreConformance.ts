...
export function runAutomationStoreConformance(createStore: () => AutomationStore) {
  ...
  it("treats undefined workspace context as an explicit unscoped store access", async () => {
    const store = createStore()
    const automation = await store.createAutomation(ctx, {
      title: "Daily summary",
      cron: "0 9 * * *",
    ...
    await expect(store.getAutomation({}, automation.id)).resolves.toMatchObject({ id: automation.id })

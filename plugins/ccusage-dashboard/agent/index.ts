...
async function fetchJson(url: string, headers: Record<string, string>): Promise<unknown> { const res = await fetch(url, { headers }); if (!res.ok) throw new Error(`${res.status} ${await res.text().catch(() => res.statusText)}`); return await res.json() }
type SecretCandidate = { path: string; fields: string[] }
type SecretValue = { value?: string; source: string; error?: string }
function normalizeSecretKey(key: string): string { return key.toLowerCase().replace(/[^a-z0-9]/g, "") }
function secretCandidate(path: string, fields: string[]): SecretCandidate { return { path, fields } }
function vaultEnvs(): NodeJS.ProcessEnv[] {
  const base = { ...process.env }
  const envs: NodeJS.ProcessEnv[] = []
  envs.push({ ...base, VAULT_ADDR: base.VAULT_ADDR || "http://127.0.0.1:8200" })
  if (base.VAULT_ADDR?.startsWith("https://127.0.0.1:8200") || base.VAULT_ADDR?.startsWith("https://localhost:8200")) envs.push({ ...base, VAULT_ADDR: base.VAULT_ADDR.replace(/^https:/, "http:") })
  if (base.VAULT_ADDR !== "http://127.0.0.1:8200") envs.push({ ...base, VAULT_ADDR: "http://127.0.0.1:8200" })
  return envs
}
function extractVaultData(parsed: unknown): Record<string, unknown> {
  const root = asRecord(parsed)
  const data = asRecord(root.data)
  return asRecord(data.data ?? data)
}
function pickSecretField(data: Record<string, unknown>, fields: string[]): { key: string; value: string } | undefined {
  const wanted = new Set(fields.map(normalizeSecretKey))
  for (const [key, raw] of Object.entries(data)) {
    const value = pickString(raw)
    if (value && wanted.has(normalizeSecretKey(key))) return { key, value }
  }
  const stringEntries = Object.entries(data).flatMap(([key, raw]) => {
    const value = pickString(raw)
    return value ? [{ key, value }] : []
  })
  return stringEntries.length === 1 ? stringEntries[0] : undefined
}
async function readVaultSecret(path: string, fields: string[]): Promise<SecretValue> {
  const errors: string[] = []
  for (const env of vaultEnvs()) {
    for (const field of fields) {
      try {
        const { stdout } = await execFileAsync("vault", ["kv", "get", `-field=${field}`, path], { timeout: 5_000, maxBuffer: 256 * 1024, env })
        const value = stdout.trim()
        if (value) return { value, source: `vault:${path}#${field}` }
        errors.push(`vault:${path}#${field}:empty`)
      } catch (err) {
        const text = err instanceof Error ? err.message : String(err)
        errors.push(`vault:${path}#${field}:${text.replace(/\s+/g, " ").slice(0, 180)}`)
      }
    }
    try {
      const { stdout } = await execFileAsync("vault", ["kv", "get", "-format=json", path], { timeout: 5_000, maxBuffer: 512 * 1024, env })
      const picked = pickSecretField(extractVaultData(JSON.parse(stdout)), fields)
      if (picked) return { value: picked.value, source: `vault:${path}#${picked.key}` }
    } catch (err) {
      const text = err instanceof Error ? err.message : String(err)
      errors.push(`vault:${path}#json:${text.replace(/\s+/g, " ").slice(0, 180)}`)
    }
  }
  return { source: `vault:${path}`, error: errors.slice(0, 3).join(" | ") || "not found" }
}
async function resolveSecret(envNames: string[], candidates: SecretCandidate[]): Promise<SecretValue> {
  for (const name of envNames) { const value = pickString(process.env[name]); if (value) return { value, source: `env:${name}` } }
  const errors: string[] = []
  for (const c of candidates) { const got = await readVaultSecret(c.path, c.fields); if (got.value) return got; if (got.error) errors.push(got.error) }
  return { source: "missing", error: errors.slice(0, 4).join(" | ") || "no env/vault value found" }
}
const CLAUDE_TOKEN_FIELDS = ["access_token", "accessToken", "oauth_token", "oauthToken", "token", "CLAUDE_CODE_OAUTH_TOKEN", "ANTHROPIC_OAUTH_TOKEN", "ANTHROPIC_ACCESS_TOKEN"]
const CODEX_TOKEN_FIELDS = ["access_token", "accessToken", "oauth_token", "oauthToken", "token", "CODEX_ACCESS_TOKEN", "OPENAI_OAUTH_TOKEN", "OPENAI_ACCESS_TOKEN"]
const CODEX_ACCOUNT_FIELDS = ["account_id", "accountId", "chatgpt_account_id", "chatgptAccountId", "CHATGPT_ACCOUNT_ID", "OPENAI_ACCOUNT_ID"]
const CLAUDE_TOKEN_CANDIDATES: SecretCandidate[] = [
  "secret/agent/claude-code/oauth", "secret/agent/claude-code", "secret/agent/claude", "secret/agent/anthropic", "secret/shared/claude-code/oauth", "secret/shared/claude-code", "secret/shared/claude", "secret/shared/anthropic",
].map((path) => secretCandidate(path, CLAUDE_TOKEN_FIELDS))
const CODEX_TOKEN_CANDIDATES: SecretCandidate[] = [
  "secret/agent/codex/oauth", "secret/agent/codex", "secret/agent/chatgpt", "secret/agent/openai", "secret/shared/codex/oauth", "secret/shared/codex", "secret/shared/chatgpt", "secret/shared/openai",
].map((path) => secretCandidate(path, CODEX_TOKEN_FIELDS))
const CODEX_ACCOUNT_CANDIDATES: SecretCandidate[] = [
  "secret/agent/codex/oauth", "secret/agent/codex", "secret/agent/chatgpt", "secret/agent/openai", "secret/shared/codex/oauth", "secret/shared/codex", "secret/shared/chatgpt", "secret/shared/openai",
].map((path) => secretCandidate(path, CODEX_ACCOUNT_FIELDS))
async function probeClaudeQuota(): Promise<ProviderQuotaSnapshot> {
  const fetchedAt = new Date().toISOString()
  const secret = await resolveSecret(["CLAUDE_CODE_OAUTH_TOKEN", "ANTHROPIC_OAUTH_TOKEN", "ANTHROPIC_ACCESS_TOKEN"], CLAUDE_TOKEN_CANDIDATES)
  const token = secret.value
  if (!token) return { provider: "claude", status: "unavailable", source: secret.source, fetchedAt, windows: QUOTA_WINDOWS.map((w) => unavailableWindow(w, secret.source, `Claude OAuth token not found in env or vault. ${secret.error || ""}`.trim())) }
  try {
    ...
    return { provider: "claude", status: "error", source: `anthropic-oauth-usage:${secret.source}`, fetchedAt, windows: QUOTA_WINDOWS.map((w) => ({ ...unavailableWindow(w, `anthropic-oauth-usage:${secret.source}`, error), status: "error" })), error }
  }
}
async function probeCodexQuota(): Promise<ProviderQuotaSnapshot> {
  const fetchedAt = new Date().toISOString()
  const secret = await resolveSecret(["CODEX_ACCESS_TOKEN", "OPENAI_OAUTH_TOKEN", "OPENAI_ACCESS_TOKEN"], CODEX_TOKEN_CANDIDATES)
  const accountSecret = await resolveSecret(["CHATGPT_ACCOUNT_ID", "OPENAI_ACCOUNT_ID"], CODEX_ACCOUNT_CANDIDATES)
  const token = secret.value
  const accountId = accountSecret.value
  if (!token) return { provider: "codex", status: "unavailable", source: secret.source, fetchedAt, windows: QUOTA_WINDOWS.map((w) => unavailableWindow(w, secret.source, `Codex/ChatGPT OAuth token not found in env or vault. ${secret.error || ""}`.trim())) }
  try {
    ...
    const error = err instanceof Error ? err.message : String(err)
    return { provider: "codex", status: "error", source: `chatgpt-wham-usage:${secret.source}`, fetchedAt, windows: QUOTA_WINDOWS.map((w) => ({ ...unavailableWindow(w, `chatgpt-wham-usage:${secret.source}`, error), status: "error" })), error }
  }
}

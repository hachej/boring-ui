...
function resolveEpicBead(beads: readonly EpicClosureIssue[], epicKey: string, branch: string): EpicClosureIssue | undefined {
  return beads.find((issue) => issue.id === epicKey || issue.id === `factory-${epicKey}` || issue.id === branch)
}
...
export async function executeCloseEpic(
  ...
  try {
    ...
    const epicBead = resolveEpicBead(beads, deps.epicKey, branch)
    if (!epicBead) {
      return textResult({ code: 'EPIC_BEAD_NOT_FOUND', message: `could not resolve epic Bead for ${deps.epicKey}`, callingSessionId }, true)
    }
    ...
    try {
      ...
      return textResult({
        ...
        message: 'close_epic stopped before child/epic close because snapshot invalidation failed',
        snapshotInvalidation: { status: 'failed', error: safeMessage(error) },
        epicBead: { id: epicBead.id, status: epicBead.status === 'closed' ? 'already-closed' : 'failed', error: 'blocked by failed snapshot invalidation' },
        supervision: { status: 'failed', error: 'blocked by failed snapshot invalidation' },
      }, false)
    }
    ...
    const nonEpicBeads = beads.filter((issue) => issue.id !== epicBead.id)

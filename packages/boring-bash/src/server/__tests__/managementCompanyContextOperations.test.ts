...
describe("management company_context operations", () => {
  ...
  test("management projection operations reject symlink escapes", async () => {
    const sourceRoot = await mkdtemp(join(tmpdir(), "boring-company-source-"));
    const outsideRoot = await mkdtemp(join(tmpdir(), "boring-company-outside-"));
    await seedCompanyContextFixture(sourceRoot);
    await mkdir(join(sourceRoot, "company", "hr"), { recursive: true });
    await writeFile(join(outsideRoot, "secret.md"), "outside secret", "utf8");

    const provider = new FixtureCompanyContextBindingProvider({
      sourceRoot,
      resolvePolicy: () => ({ allowedPathPrefixes: ["/company"], grantManagementBinding: true }),
    });
    const prepared = await provider.prepareBinding(managementCtx, managementBinding);
    await symlink(join(outsideRoot, "secret.md"), join(sourceRoot, "company", "hr", "escape.md"));
    const managementOps = createManagementProjectionOperations(prepared.handle);

    await expect(managementOps.read({ filesystem: COMPANY_CONTEXT_FILESYSTEM_ID, path: "/company/hr/escape.md" }))
      .rejects.toThrow("path escapes management projection root");
    await expect(managementOps.write({ filesystem: COMPANY_CONTEXT_FILESYSTEM_ID, path: "/company/hr/escape.md" }, "mutated"))
      .rejects.toThrow("path escapes management projection root");
    await expect(readFile(join(outsideRoot, "secret.md"), "utf8")).resolves.toBe("outside secret");
  });

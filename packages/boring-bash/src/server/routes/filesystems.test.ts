...
describe('filesystemsRoutes', () => {
  it('always returns the primary user workspace without host paths', async () => {
    ...
    expect(response.json()).toEqual({
      filesystems: [{
        ...
        rootDir: '.',
        access: 'readwrite',
        capabilities: { read: true, list: true, search: true, write: true, delete: true, move: true, mkdir: true, execute: true },
      }],
    })
    await app.close()
  })
  ...
  it('derives fine-grained capabilities from effective access and operation presence', async () => {
    ...
    expect(readonly).toEqual({
      ...
      rootDir: '/',
      access: 'readonly',
      capabilities: { read: true, list: true, search: true, write: false, delete: false, move: false, mkdir: false, execute: false },
    })
    expect(partial).toEqual({
      ...
      rootDir: '/',
      access: 'readwrite',
      capabilities: { read: true, list: true, search: true, write: true, delete: false, move: true, mkdir: false, execute: false },
    })
    await app.close()
  })

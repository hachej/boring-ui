...
describe('LocalWorkspaceStore', () => {
  describe('workspace CRUD', () => {
    it('creates a workspace and adds creator as owner', async () => {
      const ws = await createWorkspace('u1', 'My WS', 'app1')
    ...
    it('creates a workspace with an explicit id for system/bootstrap resources', async () => {
      const ws = await createWorkspace('u1', 'Company Context', 'app1', { id: '00000000-0000-4000-8000-000000000475', isDefault: false })
    ...
    it('treats explicit-id conflicts as insert-do-nothing and re-reads the existing workspace', async () => {
      ...
      const first = await createWorkspace('u1', 'Original', 'app1', { id, isDefault: false })
      const second = await createWorkspace('u2', 'Replacement', 'app1', { id, isDefault: true, managedBy: 'company-context' })
    ...
    it('list filters by userId and appId', async () => {
      await createWorkspace('u1', 'WS1', 'app1')
      await createWorkspace('u1', 'WS2', 'app2')
      await createWorkspace('u2', 'WS3', 'app1')
    ...
    it('list returns isDefault first, then createdAt DESC', async () => {
      const ws1 = await createWorkspace('u1', 'Oldest', 'app1')
      const ws2 = await createWorkspace('u1', 'Middle', 'app1')
      const ws3 = await createWorkspace('u1', 'Newest', 'app1')

...
| `EVENT_STORE_OPEN_FAILED` | `BORING_CHAT_DURABLE_STREAM=1` but the SQLite event-stream store could not be opened at its configured path (bad path/permissions); host falls back to in-memory streaming for that composition instead of crashing boot | n/a (boot-time diagnostic, not an HTTP response) | report-bug | error | stable (public API) |

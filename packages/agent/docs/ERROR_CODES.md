...
| `AGENT_FLEET_SEAT_INSTRUCTIONS_PATH_UNPUBLISHABLE` | `loadConfiguredAgentFleet` composed a fleet.yaml seat but could not publish its persona `instructions.md` location, because the seat name is not a safe workspace-relative path segment | 500 | rename the seat to `[A-Za-z0-9][A-Za-z0-9._-]*` (the seat still runs; only its instruction link is withheld) | warn | stable (trusted API) |

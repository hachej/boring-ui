...
export interface UsePiSessionsOptions {
  ...
  agentTypeId?: string
  workspaceId?: string
  /** Stable caller identity echoed only once this source's rows are authoritative. */
  ...
  /** Cross-tab active-session preference storage. Defaults to localStorage. */
...
export interface UsePiSessionsResult {
  ...
  loadingMore: boolean
  hasMore: boolean
  error: PiSessionsError | undefined
...
function sessionsErrorState(
  ...
  return { kind, requestScopeKey, attestationSourceKey, error: Object.assign(error, { kind }) }
}

export function usePiSessions(options: UsePiSessionsOptions = {}): UsePiSessionsResult {
  const enabled = options.enabled ?? true
  const apiBaseUrl = options.apiBaseUrl?.replace(/\/$/, '') ?? ''
  const agentTypeId = normalizeAgentTypeId(options.agentTypeId)
  const addressed = agentTypeId.length > 0
  const sessionsApiPath = options.sessionsApiPath ?? (addressed
    ? `/api/v1/agents/${encodeURIComponent(agentTypeId)}/sessions`
  ...
  const requestScopeKey = useMemo(() => requestScopeIdentity(apiBaseUrl, sessionsApiPath, agentTypeId, storageScope, headersKey, options.workspaceId), [agentTypeId, apiBaseUrl, headersKey, options.workspaceId, sessionsApiPath, storageScope])
  const dataSourceKey = useMemo(() => dataSourceIdentity(apiBaseUrl, sessionsApiPath, agentTypeId, storageScope, options.workspaceId), [agentTypeId, apiBaseUrl, options.workspaceId, sessionsApiPath, storageScope])
  const attestationSourceKey = useMemo(
  ...
  const bootResumeSource = useMemo<BootResumeSessionSource>(() => ({
    ...
    storageScope,
  }), [agentTypeId, apiBaseUrl, options.workspaceId, sessionsApiPath, storageScope])
  ...
  const loadedAttestationSourceRef = useRef(attestationSourceKey)
  ...
  const activeSessionKnown = Boolean(activeSession)
  ...
  const applySessions = useCallback((data: SessionSummary[], applyOptions: { background?: boolean; nextCursor?: string; requestGeneration: number }) => {
    ...
    const current = applyOptions.background && pageMayHaveMore
      ...
      : []
    const merged = applyPendingRenameTitles(serverData, mergeSessionLists(addressed, Array.from(pendingCreated.values()), serverData, current), applyOptions.requestGeneration)
    ...
    loadedRequestScopeRef.current = requestScopeKey
    loadedAttestationSourceRef.current = attestationSourceKey
    ...
    const bootResumeValidated = Boolean(
      ...
      && serverData.some((session) => session.id === storedBootResumeSessionId),
    )
    if (bootResumeValidated) {
    ...
    const hiddenResumeSessionId = storedBootResumeSessionId && !bootResumeValidated && !bootResumeConfirmed
      ? storedBootResumeSessionId
      : undefined
    updateResumeSessionId(hiddenResumeSessionId)
    setActiveSessionId((previous) => {
      if (hiddenResumeSessionId) return undefined
      const preferred = bootResumeValidated
        ? storedBootResumeSessionId
        : replacingScope ? replacingScopePreferred : previous ?? preferredSessionId()
  ...
  const refresh = useCallback(async (refreshOptions: PiSessionRefreshOptions = {}) => {
    ...
    try {
      ...
      if (!background) setErrorState(sessionsErrorState(requestError, 'fatal', requestScopeKey, attestationSourceKey))
      setLoading(false)
      if (refreshOptions.throwOnError) throw requestError
    }
  }, [addressed, applySessions, attestationSourceKey, captureMutationGuard, dataSourceKey, enabled, fetchImpl, mutationGuardIsCurrent, persistActive, preferredSessionId, requestHeaders, requestScopeKey, retryBaseMs, retryMaxMs, retryMaxRetries, sessionsListUrl, updateResumeSessionId])
  ...
  const loadMore = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (requestSeq !== loadMoreRequestSeqRef.current || version !== refreshVersionRef.current || !mutationGuardIsCurrent(sourceGuard)) return
      ...
      if (requestSeq === loadMoreRequestSeqRef.current && version === refreshVersionRef.current && mutationGuardIsCurrent(sourceGuard)) {
        const requestError = err instanceof Error ? err : new Error(String(err))
        setErrorState(sessionsErrorState(requestError, 'recoverable', requestScopeKey, attestationSourceKey))
      }
    } finally {
  ...
  }, [addressed, applyPendingRenameTitles, attestationSourceKey, captureMutationGuard, dataSourceKey, enabled, fetchImpl, hasMore, loading, loadingMore, mutationGuardIsCurrent, persistActive, requestHeaders, sessionsListUrl])
  ...
  }, [addressed, attestationSourceKey, captureMutationGuard, dataSourceKey, ensurePendingScope, fetchImpl, mutationGuardIsCurrent, refresh, requestHeaders, requestScopeKey, sessionsUrl])
  ...
  }, [attestationSourceKey, captureMutationGuard, dataSourceKey, enabled, ensurePendingScope, fetchImpl, mutationGuardIsCurrent, refresh, removeSessionLocally, requestHeaders, requestScopeKey, sessionsUrl])
  ...
  const currentErrorState = errorState?.requestScopeKey === requestScopeKey
    && errorState.attestationSourceKey === attestationSourceKey
    ? errorState
    : undefined
  ...
  return {
    ...
    sourceIdentity: (
      ...
      && loadedRequestScopeRef.current === requestScopeKey
      && loadedAttestationSourceRef.current === attestationSourceKey
    ) || (currentError?.kind === 'fatal' && !loading)
...
function normalizeAgentTypeId(agentTypeId: string | undefined): string {
  return typeof agentTypeId === 'string' && agentTypeId.length > 0 ? agentTypeId : ''
}

function requestScopeIdentity(apiBaseUrl: string, sessionsApiPath: string, agentTypeId: string, storageScope: string, headersKey: string, workspaceId?: string): string {
  return `${apiBaseUrl}\n${sessionsApiPath}\n${agentTypeId}\n${storageScope}\n${headersKey}\n${workspaceId ?? ''}`
}

function dataSourceIdentity(apiBaseUrl: string, sessionsApiPath: string, agentTypeId: string, storageScope: string, workspaceId?: string): string {
  return `${apiBaseUrl}\n${sessionsApiPath}\n${agentTypeId}\n${storageScope}\n${workspaceId ?? ''}`
}

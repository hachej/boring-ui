...
import type { InputAssetIntakeDecision } from './inputAssetIntake'
import type {
...
export interface AgentCoreRuntimeFactoryInput {
  inputAssetIntake: InputAssetIntakeDecision
}

export type AgentCoreRuntimeFactory = (input: AgentCoreRuntimeFactoryInput) => AgentCoreRuntime | Promise<AgentCoreRuntime>
...
export function createAgentRuntimeBridge(
  ...
  const runtimeLoader = createRuntimeLoader(config.runtimeFactory, inputAssetIntake)
  ...
  async function ensureBridge(sessionKey: string, sessionId: string, ctx: SessionCtx | undefined, service: PiChatSessionService): Promise<AgentSessionLiveState> {
...
function createRuntimeLoader(runtimeFactory: AgentCoreRuntimeFactory, inputAssetIntake: InputAssetIntakeDecision): {
  ...
  return {
    get() {
      runtimePromise ??= Promise.resolve().then(() => runtimeFactory({ inputAssetIntake }))
...
function createFacadeSessionStore(
  ...
  const store = async () => baseStore ?? (await getRuntime()).sessionStore
  return {
    ...
    async delete(ctx: SessionCtx, sessionId: string) {
      const runtime = await getRuntime()
      const accessCtx = await authorizeSessionAccess(runtime, sessionId, ctx, sessionContexts)
      if (!runtime.service.deleteSession) throw new Error('agent session deletion is unavailable')

...
test('createAgentApp omits native session start by default', async () => {
  const workspaceRoot = await makeTempDir('boring-agent-native-default-off-')
...
test.each(['direct', 'local'] as const)(
  'createAgentApp explicit native session capability is independent of %s runtime mode',
  async (mode) => {
    const workspaceRoot = await makeTempDir(`boring-agent-native-${mode}-`)
    const app = await createAgentApp({
      workspaceRoot,
      mode,
      logger: false,
...
test('createAgentApp explicit native session capability supports a representative custom remote adapter', async () => {
  const workspaceRoot = await makeTempDir('boring-agent-native-custom-remote-')

...
describe('system-prompt-size regression', () => {
  ...
  test.skipIf(!hasKey)('prompt size stays within budget and contains no per-tool guideline patterns', async () => {
    ...
    const created = await app.inject({
      ...
      payload: { title: 'Prompt size regression' },
    })
    expect(created.statusCode).toBe(201)
    const { id: sessionId } = JSON.parse(created.body) as { id: string }

    // Send a minimal message to trigger pi session bootstrap. This costs one
    // LLM turn but is required for the system prompt to materialize.

...
async function mountAgentHostLegacyRoutes(
  app: FastifyInstance,
  ...
  const agentHost = mountInput.created
...
/**
 * Normalize the legacy/application route profile into the nested Host seam.
 * The returned policy owns no Host or runtime adapter; the canonical Host
 * supplies its one CreatedAgentHost when registerRoutes mounts the profile.
 */
export function createAgentHostLegacyRoutePolicy(
  ...
  return Object.freeze({
    registerRoutes(input: AgentHostLegacyRoutePolicyMountInput) {
      return async (app: FastifyInstance) => {
        await mountAgentHostLegacyRoutes(app, normalizedOptions, input, scopePolicy)
      }
    },
  })
}

...
describe('fileless session access policy', () => {
  const filelessId = '11111111-2222-4333-8444-555555555555'

  function createFilelessService() {
    const adapter = createAdapter()
    let live = false
    const load = vi.fn(async () => {
      throw Object.assign(new Error('session not found'), { code: ErrorCode.enum.SESSION_NOT_FOUND })
    })
    const store: SessionStore = { ...sessionStore, load: load as unknown as SessionStore['load'] }
    const harness = {
      ...createHarness(adapter),
      sessions: store,
      hasPiSession: vi.fn(() => live),
    }
    const service = new HarnessPiChatService({ harness, sessionStore: store, workdir: '/workspace' })
    return { service, harness, adapter, setLive: (next: boolean) => { live = next } }
  }

  async function expectSessionNotFound(run: Promise<unknown>) {
    await expect(run).rejects.toMatchObject({ code: ErrorCode.enum.SESSION_NOT_FOUND })
  }

  it('admits a valid unseen id on prompt and creates the session', async () => {
    const { service, harness } = createFilelessService()

    await expect(service.prompt(ctx, filelessId, { message: 'hello', clientNonce: 'n1' }))
      .resolves.toMatchObject({ accepted: true })
    expect(harness.getPiSessionAdapter).toHaveBeenCalledWith(
      expect.objectContaining({ sessionId: filelessId }),
      expect.anything(),
    )
  })

  it('rejects a malformed unseen id on prompt', async () => {
    const { service, harness } = createFilelessService()

    for (const badId of ['../escape', 'a/b', 'has space', '', 'a..b']) {
      await expectSessionNotFound(service.prompt(ctx, badId, { message: 'hello', clientNonce: 'n1' }))
    }
    expect(harness.getPiSessionAdapter).not.toHaveBeenCalled()
  })

  it('404s read routes for a fileless id with no live runtime', async () => {
    const { service } = createFilelessService()

    await expectSessionNotFound(service.readState(ctx, filelessId))
    await expectSessionNotFound(service.subscribe(ctx, filelessId, 0, () => {}))
    await expectSessionNotFound(service.followUp(ctx, filelessId, { message: 'more', clientNonce: 'n2', clientSeq: 1 }))
    await expectSessionNotFound(service.interrupt(ctx, filelessId, {}))
    await expectSessionNotFound(service.clearQueue(ctx, filelessId, {}))
  })

  it('admits read routes once a prompt created the live runtime, before pi flushes any file', async () => {
    const { service, setLive } = createFilelessService()

    await expect(service.prompt(ctx, filelessId, { message: 'hello', clientNonce: 'n1' }))
      .resolves.toMatchObject({ accepted: true })
    setLive(true)

    await expect(service.readState(ctx, filelessId)).resolves.toMatchObject({ sessionId: filelessId })
    await expect(service.subscribe(ctx, filelessId, 0, () => {})).resolves.toMatchObject({ type: 'ok' })
    await expect(service.followUp(ctx, filelessId, { message: 'more', clientNonce: 'n2', clientSeq: 1 }))
      .resolves.toMatchObject({ accepted: true })
    await expect(service.interrupt(ctx, filelessId, {})).resolves.toMatchObject({ accepted: true })
    await expect(service.clearQueue(ctx, filelessId, {})).resolves.toMatchObject({ accepted: true })
  })

  it('admits read routes from a live channel even when the harness reports no pi session', async () => {
    const { service } = createFilelessService()

    await service.prompt(ctx, filelessId, { message: 'hello', clientNonce: 'n1' })
    // hasPiSession still false: the live channel alone must carry the admission.
    await expect(service.readState(ctx, filelessId)).resolves.toMatchObject({ sessionId: filelessId })
  })

  it('keeps 404ing a different fileless id after another session went live', async () => {
    const { service, setLive } = createFilelessService()

    await service.prompt(ctx, filelessId, { message: 'hello', clientNonce: 'n1' })
    setLive(false)
    await expectSessionNotFound(service.readState(ctx, '99999999-2222-4333-8444-555555555555'))
  })
})

      ...
      if (projectionOptions.legacyRoutePolicy) {
        return async (app: FastifyInstance) => {
          ...
          app.addHook('onClose', async () => {
            let firstError: unknown
            const capture = (error: unknown, message: string) => {
              if (firstError === undefined) firstError = error
            ...
            if (shutdownParticipants.length > 0) {
              for (const participant of shutdownParticipants) {
                try {
                  ...
                  capture(error, '[agent] plugin shutdown drain failed after an earlier cleanup error')
                }
              }
              lifecycle?.startDraining()
              try {
                ...
                capture(error, '[agent] failed to drain Agent Host after an earlier cleanup error')
              }
            }
            try {
              await lifecycle?.closeBindings()
            } catch (error) {
              capture(error, '[agent] failed to close legacy bindings after an earlier cleanup error')
            }
            try {
              await host.close()
            } catch (error) {
              capture(error, '[agent] failed to close Agent Host after an earlier cleanup error')
            }
            if (firstError !== undefined) throw firstError
          })

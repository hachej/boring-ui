...
export class SqliteAgentRequestLedger implements AgentRequestLedger {
  ...
  private readonly writer: AsyncSqliteLedgerWriter
  ...
  constructor(path: string) {
    ...
    this.migrate()
    this.writer = new AsyncSqliteLedgerWriter(this.database)
  }
  ...
  async acceptAdmission(key: AgentRequestKey, admissionReceipt: string): Promise<void> {
    await this.writer.execute(() => this.transition(key, ['pending-admission'], (record) => ({
      ...record,
  ...
  async beginEffect(key: AgentRequestKey): Promise<void> {
    await this.writer.execute(() => this.transition(key, ['admission-accepted'], (record) => ({
      key: record.key,
  ...
  async reject(key: AgentRequestKey, failure: AgentRequestFailure): Promise<void> {
    await this.writer.execute(() => this.transition(key, [
  ...
  async complete(key: AgentRequestKey, receipt: import('../../shared/index').JsonValue): Promise<void> {
    await this.writer.execute(() => this.transition(key, ['in-flight'], (record) => ({
      key: record.key,
      digest: record.digest,
      state: 'completed',
      receipt,
  ...
  async beginChildEffect(runRequestKey: AgentRequestKey, effectId: string): Promise<void> {
    await this.writer.execute(() => this.transitionEffect(runRequestKey, effectId, ['admitted'], (record) => ({ ...record, state: 'in-flight', updatedAt: Date.now() })))
  }

  async pauseChildEffect(runRequestKey: AgentRequestKey, effectId: string): Promise<void> {
    await this.writer.execute(() => this.transitionEffect(runRequestKey, effectId, ['admitted'], (record) => ({ ...record, state: 'paused', updatedAt: Date.now() })))
  }
  ...
  async markChildEffectOutcomeUnknown(runRequestKey: AgentRequestKey, effectId: string): Promise<void> {
    await this.writer.execute(() => this.transitionEffect(runRequestKey, effectId, ['in-flight', 'paused'], (record) => ({ ...record, state: 'outcome-unknown', updatedAt: Date.now() })))
  }
  ...
  private async createAttention(record: AgentAttentionLedgerRecord): Promise<void> {
    await this.writer.execute(() => {
  ...
  private async transitionAttention(
    attentionId: string,
    expected: readonly AgentAttentionStatus[],
    update: (record: AgentAttentionLedgerRecord) => AgentAttentionLedgerRecord,
  ): Promise<boolean> {
    return await this.writer.execute(() => this.transaction(() => {
  ...
  private async appendAttentionTranscriptEventIfMissing(
    attentionId: string,
    event: import('../../shared/index').JsonValue,
    matches: (event: import('../../shared/index').JsonValue) => boolean,
  ): Promise<boolean> {
  ...
  private async importAttentionOnce(records: readonly AgentAttentionLedgerRecord[]): Promise<number> {
    return await this.writer.execute(() => this.transaction(() => {

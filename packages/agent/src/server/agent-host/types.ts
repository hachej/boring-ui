...
export interface ResolvedEnvironmentScope {
  ...
  /**
   * App-declared expectations that must survive the runtime mode adapter's
   * `create()` round trip unchanged. Currently carries the readonly-workspace
   * path policy: the Environment lease fails closed if the provider drops or
   * mutates it, so a custom/misbehaving adapter cannot silently downgrade a
   * readonly guarantee into read-write access.
   */
  readonly compatibilityModeContext?: {
    readonly readonlyWorkspacePolicy?: {

...
test('vercel provisioning writes runtime artifacts through the remote workspace and sandbox cwd', async () => {
  ...
  await writeFile(join(packageRoot, 'package.json'), '{"name":"@example/remote","version":"0.0.0","bin":{"remote-tool":"dist/index.js"}}\n', 'utf8')
  ...
  const npmInstall = sandbox.history.find((entry) => entry.cmd.includes('npm install') && entry.cmd.includes('/workspace/.boring-agent/tmp/remote-'))
  expect(npmInstall?.cmd).toContain('/workspace/.boring-agent/cache/node')
  expect(npmInstall?.cmd).not.toContain(packageRoot)
  expect(Array.from(workspace.files.keys()).some((path) => path.startsWith('.boring-agent/tmp/remote-') && path.endsWith('.tgz'))).toBe(true)
  expect(sandbox.history.some((entry) => entry.cmd.includes('chmod +x') && entry.cmd.includes('/workspace/.boring-agent/bin/remote-tool'))).toBe(true)

...
9. **Mandatory verification loop — never declare the plugin done without it.** After every `/reload` (and after ANY change to the plugin's front code, dependencies, or manifest): call the `plugin_diagnostics` tool AND re-run `boring-ui-plugin test <kebab-name>`. A clean `/reload` banner is NOT proof the plugin works — front import failures (unresolved dependency, module evaluation error) surface only in `plugin_diagnostics` (`source: "plugin-front"`) and in the self-test (`PLUGIN_FRONT_ERROR`). If either reports an error: read the message, fix the code, ask the user to `/reload`, and repeat until both come back clean. Only then report success.

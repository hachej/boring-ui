...
export interface SessionIdentityRecordV1 {
  ...
  readonly invocations: { clear(): void };
...
function normalizeCreateInput(
  ...
  return {
    ...
    digest,
    createKey: multiRoot
      ? compositeSessionKey(workspaceId, normalized.clientLeaseId)
      : normalized.clientLeaseId,
    sessionKey: multiRoot
      ? compositeSessionKey(workspaceId, normalized.sandboxId)
      : normalized.sandboxId,
  };
}
type SessionStateErrorV1 =
  | "unavailable"
  | "composite-authority"
  | "not-found"
  | "workspace-mismatch"
...
export function sessionStateError(kind: SessionStateErrorV1): never {
  const values = {
    ...
    "composite-authority": [
      REMOTE_WORKER_ERROR_CODES_V1.requestInvalid,
      "remote-worker workspace authority is required",
    ],
    "not-found": [
...
export class RunscSessionStateV1<RecordV1 extends SessionIdentityRecordV1> {
  readonly sessions = new Map<string, RecordV1>();
  readonly leaseBindings = new Map<string, RecordV1>();
  private readonly legacyWorkspaceBindings = new Map<string, RecordV1>();
  private readonly pendingLegacyWorkspaces = new Set<string>();
  private readonly pendingSessionKeys = new Set<string>();
  private readonly pendingCreates = new Map<
    string,
    {
      digest: `sha256:${string}`;
      sandboxId: string;
      promise: Promise<SessionLeaseStateV1>;
    }
  >();
  ...
  create(
    ...
    const workspaceId = multiRoot
      ? validateCanonicalQuotaWorkspaceId(input.workspaceId)
      : validateQuotaWorkspaceId(input.workspaceId);
    const createKey = multiRoot
      ? compositeSessionKey(workspaceId, input.clientLeaseId)
      : input.clientLeaseId;
    ...
    if (existing) {
      if (existing.createDigest !== normalized.digest)
    ...
    if (pending) {
      ...
      return pending.promise.then((lease) =>
        multiRoot ? { ...lease, newlyAllocated: false } : lease,
      );
    }
    if (this.sweeping) sessionStateError("unavailable");
    if (
      this.sessions.has(normalized.sessionKey) ||
      this.pendingSessionKeys.has(normalized.sessionKey)
    )
      sessionStateError("conflict");
    if (
      !multiRoot &&
      (this.legacyWorkspaceBindings.has(normalized.workspaceId) ||
        this.pendingLegacyWorkspaces.has(normalized.workspaceId))
    )
      sessionStateError("conflict");
    ...
    this.pendingCreates.set(normalized.createKey, {
    ...
    const finish = (): void => {
      this.activeCreates -= 1;
      this.pendingCreates.delete(normalized.createKey);
      this.pendingSessionKeys.delete(normalized.sessionKey);
      this.pendingLegacyWorkspaces.delete(normalized.workspaceId);
    };
  ...
  sessionKey(record: RecordV1): string {
    return record.ownsWorkspaceMountSource
      ? compositeSessionKey(record.workspaceId, record.sandboxId)
      : record.sandboxId;
  }
  private leaseKey(record: RecordV1): string {
    return record.ownsWorkspaceMountSource
      ? compositeSessionKey(record.workspaceId, record.clientLeaseId)
      : record.clientLeaseId;
  }
  bind(record: RecordV1): void {
    this.pendingSessionKeys.delete(this.sessionKey(record));
  ...
  beginSweep(): void {
    if (
      ...
      this.pendingSessionKeys.size > 0
    )
      sessionStateError("unavailable");
    this.sweeping = true;
  }
  endSweep(): void {
    this.sweeping = false;
  ...
  lease(record: RecordV1, newlyAllocated: boolean): SessionLeaseStateV1 {
    ...
    return Object.freeze(
      record.ownsWorkspaceMountSource ? { ...legacy, newlyAllocated } : legacy,
    );
  }
}

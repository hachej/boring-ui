...
import { trustedWorkspaceMountSource, type TrustedWorkspaceMountSource } from "../dockerArgv";
import type { DockerCommandInput, DockerCommandResult, DockerCommandRunner } from "../dockerRunner";
...
function runner(implementation?: (input: DockerCommandInput) => Promise<DockerCommandResult>):
DockerCommandRunner & { run: ReturnType<typeof vi.fn> } {
  return {
    run: vi.fn(implementation ?? (async (input: DockerCommandInput) => {
...
function roots(prepare?: () => Promise<void>): RunscSandboxRootLifecycleV1 & {
  prepare: ReturnType<typeof vi.fn>; startupSweep: ReturnType<typeof vi.fn>;
} {
  return {
...
function runtime(docker: DockerCommandRunner, sandboxRoots: RunscSandboxRootLifecycleV1) {
...
describe("RunscSessionRuntimeV1 startup admission", () => {
  test("caps accepted sessions below the worst-case root recovery ceiling", async () => {
    ...
    const limit = Math.floor(RUNSC_RUNTIME_LIMITS_V1.maxStartupSweepContainers / 2);
    for (let index = 0; index < limit; index += 1) {
      await sessions.createComposite({ ...createInput, sandboxId: `sandbox-${index}`, clientLeaseId: `lease-${index}` });
    }
    expect(() => sessions.createComposite({ ...createInput, sandboxId: "sandbox-over", clientLeaseId: "lease-over" })).toThrowError(
      expect.objectContaining({ code: REMOTE_WORKER_ERROR_CODES_V1.createConcurrencyExhausted }),
    );
  ...
  test("joins one destructive sweep and makes success idempotent", async () => {
    ...
    const docker = runner(async (input) => {
      if (input.argv[0] === "ps") { await gate; return success(""); }
      return success();
    });
    const sandboxRoots = roots();
  ...
  test("rejects create before effects while a sweep owns admission", async () => {
    ...
    expect(() => sessions.createComposite(createInput)).toThrowError(
      expect.objectContaining({ code: REMOTE_WORKER_ERROR_CODES_V1.unavailable }),
    );
  ...
  test("rejects sweep while create is pending or a retirement record remains", async () => {
    ...
    await vi.waitFor(() => expect(sandboxRoots.prepare).toHaveBeenCalledTimes(1));
    ...
    await expect(sessions.dispose(lease.sandboxId, workspaceId)).rejects.toMatchObject({
  ...
  test("releases sweep admission after failure so retry and create can proceed", async () => {
    let attempts = 0;
    const docker = runner(async (input) => {
      if (input.argv[0] === "ps" && ++attempts === 1) throw new Error("list failed");
      if (input.argv[0] === "run") return success("container-id\n");
      if (input.argv[0] === "exec" && input.argv.at(-1) === "workspace") {
        return success(JSON.stringify({ openat2: true }));
      }
    ...
    await expect(sessions.createComposite(createInput)).resolves.toMatchObject({ sandboxId: "sandbox-a" });
  ...
  test("shutdown joins an admitted sweep before closing roots", async () => {
    ...
    await vi.waitFor(() => expect(docker.run).toHaveBeenCalledTimes(1));
    let closed = false;
    const shutdown = sessions.shutdown().then(() => { closed = true; });

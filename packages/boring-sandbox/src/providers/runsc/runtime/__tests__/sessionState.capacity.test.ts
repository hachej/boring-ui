...
const mount = trustedWorkspaceMountSource("/srv/workspaces", "00000000-0000-4000-8000-000000000001");
...
function allocate(
  ...
  return state.create(createInput, false, limit, limit, () => "unused", async (normalized, digest) => {
    effects();
    await gate;
...
describe("RunscSessionStateV1 owned-session capacity", () => {
  test("admits exactly the recovery ceiling and lets replay reuse its slot", async () => {
    ...
    await expect(allocate(state, input(1), 2, Promise.resolve(), effects)).resolves.toMatchObject({
      sandboxId: "sandbox-1",
    });
    expect(() => allocate(state, input(3), 2, Promise.resolve(), effects)).toThrowError(
  ...
  test("counts pending reservations and joins their idempotent replay", async () => {
    ...
    expect(() => allocate(state, input(2), 1, Promise.resolve(), effects)).toThrowError(
      expect.objectContaining({ code: REMOTE_WORKER_ERROR_CODES_V1.createConcurrencyExhausted }),
    );
  ...
  test("keeps a retirement-owned record charged until detach", async () => {
    ...
    state.detach(record);
    await expect(allocate(state, input(2), 1)).resolves.toMatchObject({ sandboxId: "sandbox-2" });
  });
});

...
describe("composePlugins", () => {
  ...
  it("flattens child and parent bindings into binding outputs", () => {
    const ChildBinding = () => null
    const ParentBinding = () => null
    const plugin = composePlugins({
      id: "macro",
      plugins: [{ id: "child", bindings: [ChildBinding] }],
      bindings: [ParentBinding],
    })

    expect(plugin.outputs).toEqual([
      { type: "binding", id: "child:binding:0", component: ChildBinding },
      { type: "binding", id: "macro:binding:0", component: ParentBinding },
    ])
  })

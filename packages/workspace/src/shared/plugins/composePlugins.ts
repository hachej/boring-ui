...
export interface ComposePluginsOptions {
  ...
  bindings?: WorkspaceFrontPlugin["bindings"]
  systemPrompt?: string
  /**
...
function pluginToOutputs(
  ...
  for (let i = 0; i < (plugin.bindings?.length ?? 0); i++) {
    const component = plugin.bindings![i]!
    outputs.push(
      withOwner(
        { type: "binding", id: `${plugin.id}:binding:${i}`, component },
        ownerPluginId,
      ),
    )
  }

...
describeIf("system-prompt diagnostic", () => {
  ...
  test("dumps the assembled system prompt for a dummy session", async () => {
    const created = await app.inject({
      ...
      payload: { requestId: "create-diag-system-prompt", title: "System prompt diagnostic" },
    })
    ...
    const prompted = await app.inject({
      ...
      payload: {
        requestId: "diag-system-prompt-1",
        content: "hello",
    ...
    for (;;) {
      const state = await app.inject({ method: "GET", url: `/api/v1/agents/default/sessions/${sessionId}/state` })
      if (state.statusCode === 200 && state.json<{ state?: { status?: string } }>().state?.status !== "streaming") break

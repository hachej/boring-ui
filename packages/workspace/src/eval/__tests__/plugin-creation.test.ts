 ...
 *
 * Plugin layer structure (4-layer model):
 *   front/   — React UI components (panels, commands, left tabs)
 *   agent/   — Agent tools, SDK, transforms, workspace-template, skills
 *   server/  — Trusted Node.js host logic
 *   shared/  — Platform-neutral types shared across layers
 *
 * The eval plugin is written into src/plugins/evalTaskListPlugin/front/ inside
 * the actual workspace package so the TypeScript compiler sees the real
 * @boring/workspace types. It is deleted in afterAll regardless of pass/fail.
 *
...
Plugins follow a 4-layer structure:
  front/   — React UI (panels, commands, left tabs) — write here
  agent/   — Agent tools, SDK, workspace-template — skip for now

...
export function InboxOverlay({ onClose, headerInsetStart = false, headerInsetEnd = false, pinStorageKey }: InboxOverlayProps) {
  ...
  const handleShellResult = useCallback((result: ReturnType<typeof shell.openInboxPreview>) => {
    setShellError(result.success ? null : result.message)
  }, [])
  const openPreview = useCallback((item: WorkspaceInboxItemViewModel) => { handleShellResult(shell.openInboxPreview(item)) }, [handleShellResult, shell])
  const openDedicated = useCallback((item: WorkspaceInboxItemViewModel) => { handleShellResult(shell.openInboxItemPanel(item)) }, [handleShellResult, shell])
    ...
    <div data-boring-workspace-part="inbox-overlay" className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="boring-scrollbar-discreet min-h-0 flex-1 overflow-y-auto bg-[color:oklch(from_var(--background)_calc(l-0.012)_c_h)] py-2" aria-live="polite">
        {items.length === 0 ? (
          ...
          <>
            <InboxSection title="Pinned" items={pinnedItems} onTogglePinned={togglePinned} onOpenPreview={openPreview} onOpenDedicated={openDedicated} onOpenChat={openChat} />
            <InboxSection title="Inbox" items={unpinnedItems} onTogglePinned={togglePinned} onOpenPreview={openPreview} onOpenDedicated={openDedicated} onOpenChat={openChat} />
          </>
        )}
      </div>
    </div>

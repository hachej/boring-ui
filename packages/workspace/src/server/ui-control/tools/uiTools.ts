...
export function createGetUiStateTool(uiBridge: UiBridge): AgentTool {
  return {
    ...
    description: [
      ...
      "- availableSurfaces (array): domain UI targets exposed by plugins, each with",
      "  { id, kind, title?, description?, targetHint?, examples?, metaSchema? }.",
      "  Use these with exec_ui openSurface when present.",
      "",
...
export function createExecUiTool(
  ...
  return {
    ...
    description: [
      ...
      "                 defines the mapping from domain target to panel/page",
      "                 component, for example a catalog row. Prefer kinds listed",
      "                 in get_ui_state.availableSurfaces.",

...
export function createExecUiTool(
  ...
  return {
    ...
    description: [
      ...
      "the persisted state may be stale, the file may not be focused). Calling",
      "openFile for the same path is idempotent: it focuses the workbench file",
      "slot. Saying \"already opened\" without calling the tool is a bug — the",
      "user explicitly requested an action; honor it.",
      "",
      "IMPORTANT: openFile is a single-slot focus action, not plural file",
      "presentation. Repeated calls with different paths replace the focused",
      "workbench file, so only the last remains visible. Use openFile for one",
      "file or the final focus. For multiple ad-hoc evidence files, present",
      "each workspace-relative path as a plain-text `@path` mention (not a code span)",
      "so chat renders an inline artifact card the user can open on demand.",
      "For evidence attached to a human decision, register every deliverable",
      "in the plural `ask_user`",
      "`artifacts[]` array. Do not use repeated openFile calls as a plural",
      ...
      "                 RUNNING url (your dev server, a preview host) in a",
      "                 sandboxed iframe. params: { url: string, title?: string }.",
      "                 Origins are allowlisted by the host (loopback by",
      "                 default); a disallowed url renders a blocked pane.",
      "                 Example: {kind:'openPanel', params:{id:'demo:br-1187',",

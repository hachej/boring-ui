...
describe("buildBoringSystemPrompt", () => {
  test("points plugin authors at workspace-installed package docs", async () => {
    ...
    expect(prompt).toContain(join(workspaceRoot, "node_modules", "@boring", "workspace", "dist", "docs", "plugins.md"))
    expect(prompt).toContain(join(workspaceRoot, "node_modules", "@boring", "workspace", "dist", "docs", "panels.md"))

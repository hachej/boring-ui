...
export function buildBoringSystemPrompt(opts: BuildBoringSystemPromptOptions): string {
  ...
  return [
    ...
    [
      ...
      "- Imperative method names: `registerComponent`, `addPanel`, `registerCommand` (no `Panel`), `registerTab` — the actual names are `registerPanel`, `registerPanelCommand`, `registerSurfaceResolver` (and you usually express these declaratively, not as method calls).",

...
describe("useSessionCreationCoordinator", () => {
  it("rechecks ownership at the deferred invocation boundary", async () => {
    ...
    const { result, rerender } = renderHook(() => useSessionCreationCoordinator<Session>({
      sourceKey: "source",
      validateResult,
  ...
  it("serializes canonical creates for one source", async () => {
    ...
    let first!: Promise<Session | undefined>
    let second!: Promise<Session | undefined>
    act(() => {
      first = result.current.coordinate({ dedupeKey: "first", create: firstCreate })
      second = result.current.coordinate({ dedupeKey: "second", create: secondCreate })
    })
  ...
  it("fails an undefined custom result cleanly and permits retry", async () => {
    ...
    act(() => {
      invalid = result.current.coordinate({ dedupeKey: "invalid", create: invalidCreate })
    })
    await expect(invalid).rejects.toMatchObject({ code: "SESSION_CREATE_PROTOCOL_ERROR" })

    let retry!: Promise<Session | undefined>
    act(() => {
      retry = result.current.coordinate({ dedupeKey: "retry", create: retryCreate })
    })
  ...
  it("cancels the old source and ignores its late canonical settlement", async () => {
    ...
    const newCreate = vi.fn(() => ({ id: "beta-created", agentTypeId: "beta" }))
    const { result, rerender } = renderHook(() => useSessionCreationCoordinator<Session>({
      sourceKey,
      validateResult,
      ownerIsCurrent: () => true,
    ...
    act(() => {
      current = result.current.coordinate({ dedupeKey: "new", create: newCreate })
    })
    await expect(current).resolves.toEqual({ id: "beta-created", agentTypeId: "beta" })

    act(() => oldGate.resolve({ id: "alpha-late", agentTypeId: "alpha" }))
  ...
  it("cancels active and queued tasks on withdrawal and unmount", async () => {
    ...
    const queuedCreate = vi.fn(() => ({ id: "queued" }))
    const { result, unmount } = renderHook(() => useSessionCreationCoordinator<Session>({
      sourceKey: "source",
      validateResult,
      ownerIsCurrent: () => true,
      ownershipReady: true,
    ...
    let active!: Promise<Session | undefined>
    let queued!: Promise<Session | undefined>
    act(() => {
      active = result.current.coordinate({
        dedupeKey: "auto-submit:1",
        create: () => gate.promise,
        onResolved: activeResolved,
      })
      queued = result.current.coordinate({ dedupeKey: "manual", create: queuedCreate })
    })
    await act(async () => { await Promise.resolve() })

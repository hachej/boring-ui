...
interface SessionCreationTask {
  dedupeKey: string
  create: () => unknown
  knownIds: Set<string>
  awaitingRow: boolean
  pendingPane?: PendingCreatePane
  pendingPaneInit?: Omit<PendingCreatePane, "knownIds">
  onCreated?: (session: unknown) => void
  onObserved?: (session: WorkspaceAgentSession) => void
...
interface SessionCreationCoordinator {
  sourceKey: string
  active: SessionCreationTask | null
  queue: SessionCreationTask[]
}
...
type SessionActionOwnership = {
  owner: object
  create: WorkspaceAgentSessionsApi["create"]
  switch: WorkspaceAgentSessionsApi["switch"]
  delete: WorkspaceAgentSessionsApi["delete"]
  rename?: NonNullable<WorkspaceAgentSessionsApi["rename"]>
  loadMore?: NonNullable<WorkspaceAgentSessionsApi["loadMore"]>
  refresh?: NonNullable<WorkspaceAgentSessionsApi["refresh"]>
}
...
>({
  ...
  const autoSubmitAttemptRef = useRef({ sourceKey: sessionOperationKey, requested: requestedAutoSubmitInitialDraft, token: 0 })
  const renderedAutoSubmitAttempt = autoSubmitAttemptRef.current
  if (renderedAutoSubmitAttempt.sourceKey !== sessionOperationKey || renderedAutoSubmitAttempt.requested !== requestedAutoSubmitInitialDraft) {
    autoSubmitAttemptRef.current = {
      sourceKey: sessionOperationKey,
      requested: requestedAutoSubmitInitialDraft,
      token: renderedAutoSubmitAttempt.token + 1,
    }
  }
  ...
  const creationCoordinatorRef = useRef<SessionCreationCoordinator>({
    sourceKey: sessionOperationKey,
    active: null,
    queue: [],
  })
  const drainCreationCoordinatorRef = useRef<() => void>(() => {})
  const finishCreationTaskRef = useRef<(task: SessionCreationTask, outcome: { value?: unknown; error?: unknown }) => void>(() => {})
  ...
  const sessionActionOwner = useMemo(() => ({}), [sessionOperationKey])
  const committedSessionOperationKeyRef = useRef(sessionOperationKey)
  useIsomorphicLayoutEffect(() => {
    if (committedSessionOperationKeyRef.current === sessionOperationKey) return
    committedSessionOperationKeyRef.current = sessionOperationKey
    ...
    const previousCoordinator = creationCoordinatorRef.current
    previousCoordinator.active?.resolve(undefined)
    for (const task of previousCoordinator.queue) task.resolve(undefined)
    creationCoordinatorRef.current = { sourceKey: sessionOperationKey, active: null, queue: [] }
    pendingCreatePaneRef.current = null
    optimisticCreatedPaneKeysRef.current.clear()
  }, [sessionOperationKey])
  useIsomorphicLayoutEffect(() => {
    committedSessionActionOwnerRef.current = sessionActionOwner
    return () => {
      if (committedSessionActionOwnerRef.current === sessionActionOwner) {
        committedSessionActionOwnerRef.current = null
      }
    }
  }, [sessionActionOwner])
  ...
  const updatePendingChatPanePlacementConsumed = useCallback((paneId: string) => {
  ...
  const updateSessionPinned = useCallback((sessionId: string, sessionAgentTypeId?: string) => {
  ...
  const remoteSessionsProtocolError = useSessionsProp
    ...
    : undefined
  const remoteSessionsRecoverableError = remoteSessionsSourceAttested
    && remoteSessionApi.error
    && (remoteSessionApi.error as Error & { kind?: string }).kind === "recoverable"
    ? remoteSessionApi.error
    : undefined
  const remoteSessionsTerminalError = remoteSessionsSourceAttested
    ? remoteSessionsRecoverableError ? undefined : remoteSessionApi.error ?? undefined
  ...
  const remoteSessionsPending = remoteSessionHookEnabled
    ...
    && !remoteSessionsTerminalError
  useIsomorphicLayoutEffect(() => {
    const ownership: SessionActionOwnership | null = remoteSessionsAvailable
      ? {
          owner: sessionActionOwner,
          create: remoteSessionApi.create,
          switch: remoteSessionApi.switch,
          delete: remoteSessionApi.delete,
          ...(remoteSessionApi.rename ? { rename: remoteSessionApi.rename } : {}),
          ...(remoteSessionApi.loadMore ? { loadMore: remoteSessionApi.loadMore } : {}),
          ...(remoteSessionApi.refresh ? { refresh: remoteSessionApi.refresh } : {}),
        }
      : null
    sessionActionOwnershipRef.current = ownership
    return () => {
      if (sessionActionOwnershipRef.current === ownership) sessionActionOwnershipRef.current = null
    }
  }, [remoteSessionApi.create, remoteSessionApi.delete, remoteSessionApi.loadMore, remoteSessionApi.refresh, remoteSessionApi.rename, remoteSessionApi.switch, remoteSessionsAvailable, sessionActionOwner])
  const sessionActionOwnerIsCurrent = useCallback(() => (
    committedSessionActionOwnerRef.current === sessionActionOwner
    && (!shouldUseRemoteSessions || sessionActionOwnershipRef.current?.owner === sessionActionOwner)
  ), [sessionActionOwner, shouldUseRemoteSessions])
  const consumePendingChatPanePlacement = useCallback((paneId: string) => {
    if (!sessionActionOwnerIsCurrent()) return
    updatePendingChatPanePlacementConsumed(paneId)
  }, [sessionActionOwnerIsCurrent, updatePendingChatPanePlacementConsumed])
  const toggleSessionPinned = useCallback((sessionId: string, sessionAgentTypeId?: string) => {
    if (!sessionActionOwnerIsCurrent()) return
    updateSessionPinned(sessionId, sessionAgentTypeId)
  }, [sessionActionOwnerIsCurrent, updateSessionPinned])
  const availableSessionActions = useMemo(() => {
    if (!remoteSessionsAvailable) return undefined
    const current = () => {
      const ownership = sessionActionOwnershipRef.current
      return ownership?.owner === sessionActionOwner ? ownership : undefined
    }
    return {
      ...
      rename: remoteSessionApi.rename
        ? (id: string, title: string) => current()?.rename?.(id, title)
        : undefined,
      loadMore: remoteSessionApi.loadMore ? () => current()?.loadMore?.() : undefined,
      refresh: remoteSessionApi.refresh
        ? (options?: { background?: boolean; throwOnError?: boolean }) => current()?.refresh?.(options)
        : undefined,
    }
  }, [remoteSessionApi.loadMore, remoteSessionApi.refresh, remoteSessionApi.rename, remoteSessionsAvailable, sessionActionOwner])
  const sessionControlsAvailable = !shouldUseRemoteSessions || Boolean(availableSessionActions)
  useEffect(() => {
    if (!remoteSessionsAvailable) return
    setRemoteSessionSnapshot((previous) => {
      const sameSource = previous.sourceKey === sessionOperationKey
  ...
  const autoSubmitSessionCreateFailedRef = useRef(autoSubmitSessionCreateFailed)
  autoSubmitSessionCreateFailedRef.current = autoSubmitSessionCreateFailed
  const autoSubmitSessionWorkspaceRef = useRef(sessionOperationKey)
  useEffect(() => {
    if (autoSubmitSessionWorkspaceRef.current !== sessionOperationKey) {
  ...
  const adoptUserCreatedAutoSubmitSession = useCallback((sessionKey: string) => {
    if (!autoSubmitSessionCreateFailedRef.current) return
    setAutoSubmitSessionCreateFailure({ sourceKey: sessionOperationKey, failed: false })
  ...
  }, [effectiveActiveSessionId, rawSwitch, sessionActionOwnerIsCurrent])
  ...
  const finishCreationTask = useCallback((task: SessionCreationTask, outcome: { value?: unknown; error?: unknown }) => {
    const coordinator = creationCoordinatorRef.current
    if (coordinator.sourceKey !== sessionOperationKey || coordinator.active !== task) return
    if (pendingCreatePaneRef.current === task.pendingPane) pendingCreatePaneRef.current = null
  ...
  const drainCreationCoordinator = useCallback(() => {
    ...
    const task = coordinator.queue.shift()
    if (!task) return
    ...
    task.knownIds = new Set(resolvedSessionsRef.current.map(workspaceSessionKeyFor))
    if (task.pendingPaneInit) {
      task.pendingPane = { ...task.pendingPaneInit, knownIds: task.knownIds }
      pendingCreatePaneRef.current = task.pendingPane
    }
  ...
  }, [sessionActionOwnerIsCurrent, sessionOperationKey])
  drainCreationCoordinatorRef.current = drainCreationCoordinator

  const coordinateSessionCreate = useCallback((options: {
    dedupeKey: string
    create: () => unknown
    pendingPane?: Omit<PendingCreatePane, "knownIds">
    onCreated?: (session: unknown) => void
    onObserved?: (session: WorkspaceAgentSession) => void
    onError?: (error: unknown) => void
    onSettled?: () => void
    ...
    const duplicate = coordinator.active?.dedupeKey === options.dedupeKey
      ? coordinator.active
      : coordinator.queue.find((task) => task.dedupeKey === options.dedupeKey)
    ...
    const task: SessionCreationTask = {
      dedupeKey: options.dedupeKey,
      create: options.create,
      knownIds: new Set(),
      awaitingRow: false,
      pendingPaneInit: options.pendingPane,
  ...
  useEffect(() => {
    ...
    const attemptToken = autoSubmitAttemptRef.current.token
    const attemptIsCurrent = () => (
      autoSubmitAttemptRef.current.sourceKey === sessionOperationKey
      && autoSubmitAttemptRef.current.requested
      && autoSubmitAttemptRef.current.token === attemptToken
      && sessionActionOwnerIsCurrent()
    )
    void coordinateSessionCreate({
      dedupeKey: "auto-submit",
      ...
      onObserved: (session) => {
        if (!attemptIsCurrent()) return
        setAutoSubmitSessionCreateFailure({ sourceKey: sessionOperationKey, failed: false })
        setAutoSubmitTarget({ sessionKey: workspaceSessionKeyFor(session), view: "pane" })
      },
      onError: () => {
        if (attemptIsCurrent()) setAutoSubmitSessionCreateFailure({ sourceKey: sessionOperationKey, failed: true })
      },
    }).catch(() => {
      // Failure state is set only for the still-current auto-submit attempt.
    })
  }, [agentTypeId, autoSubmitSessionCreateFailed, autoSubmitTarget, availableSessionActions, coordinateSessionCreate, defaultSessionTitle, sessionActionOwnerIsCurrent, sessionOperationKey])
  ...
  const resolvedDelete = useCallback((id: string, sessionAgentTypeId?: string) => {
    ...
    if (hasRemoteSessionData && availableSessionActions && !remoteSessionsPending && activeRemoteSessions.length <= 1) {
      ...
      return coordinateSessionCreate({
        dedupeKey: `replacement:${sessionKey}`,
        create: () => availableSessionActions.create({ title: defaultSessionTitle }),
        onError: () => setInitialRemoteSessionCreateFailed({ workspaceId, failed: true }),
        onSettled: () => pendingLastSessionDeleteRef.current.delete(sessionKey),
      }).then(() => (
        sessionActionOwnerIsCurrent()
    ...
    return sessionAgentTypeId ? rawDelete(id, sessionAgentTypeId) : rawDelete(id)
  }, [activeRemoteHasMore, activeRemoteSessions.length, availableSessionActions, coordinateSessionCreate, defaultSessionTitle, hasRemoteSessionData, rawDelete, remoteSessionsPending, sessionActionOwnerIsCurrent, workspaceId])
  ...
  useEffect(() => {
    ...
    setInitialRemoteSessionCreateFailed({ workspaceId, failed: false })
  }, [sessionOperationKey, workspaceId])
  ...
  useEffect(() => {
    ...
    setInitialRemoteSessionCreateFailed({ workspaceId, failed: false })
    void coordinateSessionCreate({
      dedupeKey: "initial-auto",
      create: () => availableSessionActions.create({
        title: defaultSessionTitle,
        ...(activeRemoteResumeSessionId ? { resumeSessionId: activeRemoteResumeSessionId } : {}),
      }),
      onError: () => setInitialRemoteSessionCreateFailed({ workspaceId, failed: true }),
      onSettled: () => setInitialRemoteSessionCreating({ workspaceId, creating: false }),
    }).catch(() => {
      // The visible initial-session failure state owns retry affordance.
    })
  }, [activeRemoteResumeSessionId, activeRemoteSessions.length, autoSubmitTarget, availableSessionActions, coordinateSessionCreate, defaultSessionTitle, remoteEmptySessionsSettling, remoteSessionsAvailable, workspaceId])
  ...
  useEffect(() => {
    ...
    const activeCreation = creationCoordinatorRef.current.sourceKey === sessionOperationKey
      ? creationCoordinatorRef.current.active
      : null
    const pendingCreatePane = activeCreation?.pendingPane ?? null
    ...
    const newlyObservedSession = activeCreation?.awaitingRow
      ? resolvedSessions.find((session) => !activeCreation.knownIds.has(workspaceSessionKeyFor(session)))
      : undefined
    const observedCreationKey = newlyObservedSession ? workspaceSessionKeyFor(newlyObservedSession) : null
    const pendingCreatedId = pendingCreatePane
      ? sessionKeys.has(chatSessionKey) && !pendingCreatePane.knownIds.has(chatSessionKey)
        ? chatSessionKey
        : observedCreationKey
      : null
    if (newlyObservedSession && activeCreation?.awaitingRow) {
      try {
        activeCreation.onObserved?.(newlyObservedSession)
        finishCreationTaskRef.current(activeCreation, { value: undefined })
      } catch (error) {
        activeCreation.onError?.(error)
        finishCreationTaskRef.current(activeCreation, { error })
      }
    }
  ...
  }, [adoptUserCreatedAutoSubmitSession, agentTypeId, autoSubmitTarget, chatSessionId, chatSessionKey, effectiveActiveSessionAgentTypeId, remoteSessionsPending, remoteSessionsTransitioning, resolvedSessions, sessionListAuthoritative, sessionOperationKey, workspaceId])
  ...
  const activeChatPaneState = chatPaneState.workspaceId === workspaceId
    ? chatPaneState
    : { workspaceId, ids: [], activeId: null }
  const restoredChatPaneIds = activeChatPaneState.ids.length > 0 ? activeChatPaneState.ids : [chatSessionKey]
  const addressedRemoteMode = shouldUseRemoteSessions && Boolean(agentTypeId)
  const validatedSessionKeys = useMemo(
    () => new Set(resolvedSessions.map(workspaceSessionKeyFor)),
    [resolvedSessions],
  )
  const chatPaneIds = addressedRemoteMode
    ? restoredChatPaneIds.filter((id) => validatedSessionKeys.has(id) || optimisticCreatedPaneKeysRef.current.has(id))
    : restoredChatPaneIds
  if (addressedRemoteMode && chatPaneIds.length === 0 && validatedSessionKeys.has(chatSessionKey)) {
    chatPaneIds.push(chatSessionKey)
  }
  useEffect(() => {
  ...
  }, [chatPaneState, chatSessionKey, rawSwitch, resolvedSwitch, sessionActionOwnerIsCurrent, workspaceId])
  useEffect(() => {
  ...
  }, [chatSessionKey, rawSwitch, sessionActionOwnerIsCurrent, workspaceId])
  ...
  }, [chatPaneState, chatPaneStateForWorkspace, chatSessionKey, rawSwitch, sessionActionOwnerIsCurrent, workspaceId])
  ...
  }, [chatPaneState, chatSessionKey, rawSwitch, sessionActionOwnerIsCurrent, workspaceId])
  ...
  const createChatSession = useCallback(() => {
    ...
    return coordinateSessionCreate({
      ...
      onObserved: (session) => adoptUserCreatedAutoSubmitSession(workspaceSessionKeyFor(session)),
    }).catch(() => {
      // Creation errors are surfaced by the session API/chat layer.
    })
  }, [activeChatPaneId, adoptUserCreatedAutoSubmitSession, chatPaneStateForWorkspace, chatSessionKey, coordinateSessionCreate, materializeCreatedSession, resolvedCreate, sessionActionOwnerIsCurrent, switchToCreatedSession, workspaceId])
  ...
  }, [adoptUserCreatedAutoSubmitSession, chatPaneStateForWorkspace, coordinateSessionCreate, materializeCreatedSession, resolvedCreate, sessionActionOwnerIsCurrent, switchToCreatedSession, workspaceId])
  ...
  const deleteSessionAndPane = useCallback((sessionId: string, sessionAgentTypeId?: string): void | Promise<unknown> => {
    ...
    const finishDelete = () => {
      ...
      const nextIds = deletingIndex >= 0 ? current.ids.filter((id) => id !== sessionKey) : current.ids
      const nextActiveId = current.activeId === sessionKey
    ...
    const deleted = resolvedDelete(sessionId, sessionAgentTypeId)
    if (deleted && typeof (deleted as PromiseLike<unknown>).then === "function") {
      return Promise.resolve(deleted).then((result) => {
        finishDelete()
        return result
      })
    }
  ...
  useIsomorphicLayoutEffect(() => {
    if (autoSubmitHydrationWorkspaceRef.current !== sessionOperationKey) {
      autoSubmitHydrationWorkspaceRef.current = sessionOperationKey
      setAutoSubmitSessionCreateFailure({ sourceKey: sessionOperationKey, failed: false })
      setAutoSubmitTarget(needsFreshRemoteSessionForAutoSubmit ? null : undefined)
    ...
    // Withdrawal wins over every unsettled phase: pending create, designated
    // target, or prior failure. The render-time attempt token fences any late
    // continuation before these state releases commit. A void create has no
    // network work left, so request cancellation also releases its row wait.
    const coordinator = creationCoordinatorRef.current
    const activeCreate = coordinator.active
    if (activeCreate?.dedupeKey === "auto-submit" && activeCreate.awaitingRow) {
      finishCreationTaskRef.current(activeCreate, { value: undefined })
    }
    coordinator.queue = coordinator.queue.filter((task) => {
      if (task.dedupeKey !== "auto-submit") return true
      task.resolve(undefined)
      return false
    })
    autoSubmitRequestConsumedRef.current = false
  ...
  useEffect(() => {
    if (!sessionActionOwnerIsCurrent()) return
    if (shouldUseRemoteSessions && !remoteSessionsAvailable) return
    onActiveSessionIdChange?.(effectiveActiveSessionId ?? null)
  }, [effectiveActiveSessionId, onActiveSessionIdChange, remoteSessionsAvailable, sessionActionOwnerIsCurrent, shouldUseRemoteSessions])
  ...
  const reloadAgentPluginsForSession = useCallback(async (sessionId: string) => {
    if (!sessionActionOwnerIsCurrent()) throw new Error("Session source is unavailable")
  ...
  }, [apiBaseUrl, resolvedRequestHeaders, sessionActionOwnerIsCurrent])
  ...
  const refreshSessionsBestEffort = useCallback(() => {
    ...
    try {
      void Promise.resolve(availableSessionActions?.refresh?.({ background: true })).catch(() => {
        // Session summary reconciliation is best-effort.
      })
    } catch {
      // Custom providers may throw before returning a promise.
    }
  ...
  const makeCenterParams = useCallback(
    (sessionKey: string, options: { bridgeEnabled?: boolean; view?: AutoSubmitDraftTarget["view"] } = {}) => {
      ...
      const admittedAutoSubmitAttemptToken = autoSubmitAttemptRef.current.token
      ...
      showSessions: false,
      onReloadAgentPlugins: sessionControlsAvailable ? async () => {
        if (!sessionActionOwnerIsCurrent()) throw new Error("Session source is unavailable")
        return typeof chatParams?.onReloadAgentPlugins === "function"
          ? chatParams.onReloadAgentPlugins()
          : reloadAgentPluginsForSession(sessionId)
      } : undefined,
      ...
      onAutoSubmitInitialDraftSettled: () => {
        if (
          !sessionActionOwnerIsCurrent()
          || autoSubmitAttemptRef.current.token !== admittedAutoSubmitAttemptToken
          || (requestedAutoSubmitInitialDraft && !admitsAutoSubmitInitialDraft)
        ) return
  ...
  const providerActiveSessionRef = chatPaneIds.includes(activeChatPaneId)
    ? workspaceSessionRefFromKey(activeChatPaneId)
    : null
  const providerActiveSessionId = providerActiveSessionRef?.sessionId ?? null
  const attentionSessions = useMemo(() => {
    ...
    if (effectiveActiveSessionId) {
      ...
      const key = workspaceSessionKey(ref.sessionId, ref.agentTypeId)
      if (!addressedRemoteMode || validatedSessionKeys.has(key)) refs.set(key, ref)
    }
    return [...refs.values()]
  ...
  const activeChatPaneRef = chatPaneIds.includes(activeChatPaneId) ? workspaceSessionRefFromKey(activeChatPaneId) : null
  ...
  const navParams = {
    ...
    pinnedRefs,
    onTogglePin: sessionControlsAvailable ? toggleSessionPinned : undefined,
    onSwitch: sessionControlsAvailable ? switchToChatPane : undefined,
    onOpenAsTab: sessionControlsAvailable ? openChatPane : undefined,
    onCreate: sessionControlsAvailable ? createChatSession : undefined,
    onDelete: sessionControlsAvailable ? deleteSessionAndPane : undefined,
    onLoadMore: sessionControlsAvailable ? availableSessionActions?.loadMore : undefined,
    hasMore: activeRemoteHasMore,
    loadingMore: activeRemoteLoadingMore,
    onClose: () => setNavOpen(false),
  }
  const canDeleteSessions = sessionControlsAvailable && Boolean(hasRemoteSessionData || onDeleteSession || !hasExplicitSessionProps)
  const commandPaletteSessionSearch = useMemo(() => (
    isPluginTabsLayout && sessionControlsAvailable
      ? {
          sessions: resolvedSessions,
      ...
      : undefined
  ), [activeChatPaneId, chatPaneIds, isPluginTabsLayout, openChatPane, resolvedSessions, sessionControlsAvailable, switchToChatPane])
  ...
  const shellCapabilitiesHost = useWorkspaceShellCapabilitiesHost({
    ...
    defaultSessionTitle,
    makeCenterParams,
    suppressDetachedInitialDraft: autoSubmitHydrationDisabled,
    resolveSessionKey,
    openChatPane,
  ...
  const createChatSessionInPopover = useCallback(() => {
    ...
    const openCreatedSession = (session: unknown) => {
      const materialized = materializeCreatedSession(session)
      if (!materialized) return
      shellCapabilitiesHost.openDetachedChatRef(materialized.key, {
        title: defaultSessionTitle,
        composingEnabled: true,
      })
      // Quick chat is auxiliary: restore the click-time canonical owner rather
      // than whichever colliding row happens to be visible after creation.
  ...
  const mainContent = remoteSessionsTerminalError ? (
    ...
    <ChatLayout
      ...
      onActiveChatPaneChange={sessionControlsAvailable ? activateChatPane : undefined}
      onCloseChatPane={sessionControlsAvailable ? closeChatPane : undefined}
      onCreateChatPaneAfter={isPluginTabsLayout || !sessionControlsAvailable ? undefined : createChatPaneAfter}
      onSplitChatPane={sessionControlsAvailable ? createChatPaneAfter : undefined}
      chatPaneSplitPending={chatPaneSplitPending}
      pendingChatPanePlacement={pendingChatPanePlacement}
      onPendingChatPanePlacementConsumed={sessionControlsAvailable ? consumePendingChatPanePlacement : undefined}
      onDropChatSession={sessionControlsAvailable ? openChatPane : undefined}
    ...
    <PluginTabsWorkspaceShell
      ...
      leftPane={(
        <AppLeftPane
          ...
          onCreateProjectSession={sessionControlsAvailable ? (projectId) => {
          ...
          onCreateSession={sessionControlsAvailable ? () => {
            if (!sessionActionOwnerIsCurrent()) return
            setLeftOverlay(null)
            void createChatSession()
          } : undefined}
          onCreateSplitSession={sessionControlsAvailable ? () => {
            if (!sessionActionOwnerIsCurrent()) return
            setLeftOverlay(null)
            void createChatPaneAfter(activeChatPaneId, "right")
          } : undefined}
          onCreatePopoverSession={sessionControlsAvailable ? createChatSessionInPopover : undefined}
          onOpenCommandPalette={openCommandPalette}
          onSwitchSession={sessionControlsAvailable ? switchToChatPane : undefined}
          onOpenSessionAsPane={sessionControlsAvailable ? openChatPane : undefined}
          onToggleSessionPinned={sessionControlsAvailable ? toggleSessionPinned : undefined}

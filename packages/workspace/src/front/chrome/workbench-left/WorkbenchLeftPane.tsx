...
export function WorkbenchLeftPane({
  ...
  // The rail itself now lives in `WorkbenchActivityRail`, so an embedded
  // workbench can present the same one. This pane still owns WHAT is on it.
  const rail = (
    <WorkbenchActivityRail
      entries={tabs}
      side={railSide}
      leading={railSide === "left" && onCollapse ? (
        <PaneCollapseButton
          className="workbench-rail-action"
          label="Hide workspace menu"
          side="right"
          onClick={onCollapse}
        >

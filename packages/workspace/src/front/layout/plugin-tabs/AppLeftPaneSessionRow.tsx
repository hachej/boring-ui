...
export function AppSessionRow({
  ...
  const renameCapability = session.canRename ?? (session as { renameable?: unknown }).renameable
  const canRenameSession = Boolean(onRename) && renameCapability === true && !isBrowserDraftSession(session)
  useEffect(() => {

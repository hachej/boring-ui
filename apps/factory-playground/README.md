...
`demo_sandbox` (plugin `factory-demo`, tool granted only to the Orchestrator) backs Gate 2's live demo. It resolves the caller's epic first and defaults `sha` to that epic worktree's `HEAD`. With `local-simulation`, or when Vercel lease creation fails and the named remote lease is conclusively absent/stopped, it starts an exact-SHA local lease and reports the fallback explicitly. Starts, status/list results, persisted records, and stops are isolated by epic key. State persists to `<stateRoot>/demos.json`; boot stops expired entries, drops dead local process groups, restores owner-host proxies, and leaves live demos recorded.

Local demos run as host processes with a minimal scrubbed environment and an exact-SHA lease root; they are not container-confined and remain trusted-code-only. The server must echo the `BORING_FACTORY_DEMO_READY_NONCE` value from the incoming `x-boring-factory-demo-token` request header in the response header of `readyPath`. Starts are serialized through this authenticated readiness check and retry another port after bind loss. The child is required to listen only on `127.0.0.1`; set `BORING_FACTORY_DEMO_HOST` to a non-wildcard IP address owned by the Factory host (for example, its Tailscale IP) to advertise an owner-reachable URL through the host-owned loopback proxy. See `docs/factory/RELIABILITY.md` for the remaining host-RCE boundary and the SBX1 confinement path.

...
export async function startCoreServer(port = 5420) {
  ...
  app.log.info({ port, defaultWorkspaceId: ws.id, devUser: DEV_USER_EMAIL }, 'custom-layout core server ready')

  return { app, defaultWorkspaceId: ws.id }
}

if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replace(/\\/g, '/'))) {
  const port = Number(process.env.CORE_PORT) || 5420

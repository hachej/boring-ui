...
async function proof(image: string): Promise<void> {
  const token = randomUUID(); const root = await mkdtemp('/dev/shm/boring-d1-boundary-')
  const hostA = path.join(root, 'host-a'); const hostB = path.join(root, 'host-b'); const missing = path.join(root, 'missing')
  const target = `d1-boundary-${token}`; const missingName = `${target}-missing`; const helper = `${target}-helper`; const cleanup = `${target}-cleanup`

...
import { createAgentHostActiveCollectionReader } from '../src/server/deployment/activeCollectionReader.js'
import { captureAgentHostCoreProofRevision, verifyAgentHostCoreProof } from '../src/server/deployment/agentHostCoreProof.js'
import { captureAgentHostDrFingerprint, createAgentHostDrRowsReader } from '../src/server/deployment/agentHostDrProof.js'
import { AgentHostErrorCode, strictAgentHostId } from '../src/server/deployment/agentHostPlan.js'
...
function failure(): never {
  process.stdout.write(`${JSON.stringify({ ok: false, error: { code: AgentHostErrorCode.PROOF_INVALID, details: { field: 'proof' } } })}\n`)
  process.exit(4)
}
...
function hostConfig() {
  const hostId = strictAgentHostId(process.env.BORING_AGENT_HOST_ID, 'hostId')
  const ownerUid = Number(process.env.BORING_AGENT_HOST_OWNER_UID)
  if (!Number.isSafeInteger(ownerUid) || ownerUid < 0) failure()
  const stateRoot = absolute(process.env.BORING_AGENT_HOST_PROOF_STATE_ROOT ?? '/var/lib/boring/agent-host')
  const hostRoot = resolve(stateRoot, hostId)
  return { hostId, ownerUid, hostRoot, reader: createAgentHostActiveCollectionReader({ hostRoot, hostId, ownerUid, appGid: 10001 }) }
}
...
async function cleanupIsolationResources(pid: number, workRoot: string): Promise<boolean> {
  ...
  const image = `boring-agent-host-006rq:${pid.toString(36)}`
...
async function liveIsolation(): Promise<unknown> {
  ...
  const workRoot = join(tmpdir(), `boring-agent-host-006rq-supervised-proof-${process.pid}`)
  const child = spawn(process.execPath, [harness, '--agent-host-isolation-worker'], {
    cwd: repoRoot,
    env: { ...process.env, BORING_AGENT_HOST_006_WORK_ROOT: workRoot },
    detached: true,
    shell: false,
...
try {
  ...
  if (process.argv[2] === 'capture') {
    const snapshot = await captureAgentHostCoreProofRevision(hostConfig().reader)

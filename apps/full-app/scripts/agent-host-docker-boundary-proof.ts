...
type DockerResult = Readonly<{ ok: boolean; stdout: string }>
...
function requireDocker(args: readonly string[]): string {
  ...
  if (!result.ok) throw new Error('docker')
  return result.stdout
}
function mount(source: string): string { return `type=bind,src=${source},dst=${DESTINATION},readonly` }

export function parsePinnedImage(argv: readonly string[], env: NodeJS.ProcessEnv): string {
  const args = argv[0] === '--' ? argv.slice(1) : argv
  const image = args.length === 0 ? env.AGENT_HOST_DOCKER_PROOF_IMAGE : args.length === 2 && args[0] === '--image' ? args[1] : undefined
  if (!image || !/^(?:sha256:[a-f0-9]{64}|[^\s]+@sha256:[a-f0-9]{64})$/.test(image)) throw new Error('image')
  return image
}

export function targetRunArgs(image: string, name: string, source: string): readonly string[] {
...
export function assertContainersAbsent(names: readonly string[], run: (args: readonly string[]) => DockerResult = docker): void {
  for (const name of names) {
    const result = run(['container', 'ls', '-a', '--filter', `name=^/${name}$`, '--format', '{{.Names}}'])
    if (!result.ok || result.stdout.trim() !== '') throw new Error('containerCleanup')
  }
}

export function validateInspect(raw: string, source: string, secrets: readonly string[], repositoryRoot: string): void {
  const record = (JSON.parse(raw) as unknown[])[0] as Record<string, any>
  const mounts = record?.Mounts as Array<Record<string, unknown>>
  if (!record || !Array.isArray(mounts) || mounts.length !== 1) throw new Error('inspect')
  const actual = mounts[0]
  if (actual?.Type !== 'bind' || actual.Source !== source || actual.Destination !== DESTINATION || actual.RW !== false) throw new Error('mount')
  const relative = path.relative(repositoryRoot, source)
  if (relative === '' || (!relative.startsWith('..') && !path.isAbsolute(relative))) throw new Error('repositoryMount')
  const serialized = JSON.stringify({ Config: record.Config, Mounts: record.Mounts })
  if (secrets.some((secret) => serialized.includes(secret))) throw new Error('secret')
}

function assertTargetResult(raw: string): void {
  const result = JSON.parse(raw.trim()) as Record<string, any>
  if (result.status !== 'pass' || JSON.stringify(result.uid) !== `[${APP_ID},${APP_ID}]` || JSON.stringify(result.gid) !== `[${APP_ID},${APP_ID}]`
    ...
    || result.denied?.write !== true || result.denied?.chmod !== true || result.denied?.unlink !== true) throw new Error('target')
}

async function waitForResult(name: string): Promise<string> {
...
function cleanupResources(image: string, root: string, names: readonly string[], cleanupName: string): void {
  ...
  if (!cleaned.ok) { try { rmdirSync(root) } catch {}; throw new Error('cleanupHelper') }
  assertContainersAbsent([...names, cleanupName])
  try { rmdirSync(root) } catch { throw new Error('cleanup') }
}

async function proof(image: string): Promise<void> {
  const token = randomUUID(); const root = await mkdtemp('/dev/shm/boring-agent-host-boundary-')
  const hostA = path.join(root, 'host-a'); const hostB = path.join(root, 'host-b'); const missing = path.join(root, 'missing')
  const target = `agent-host-boundary-${token}`; const missingName = `${target}-missing`; const helper = `${target}-helper`; const cleanup = `${target}-cleanup`
  const names = [target, missingName, helper]; let cleanupStarted = false; let interrupted: NodeJS.Signals | undefined
  const clean = () => { if (!cleanupStarted) { cleanupStarted = true; cleanupResources(image, root, names, cleanup) } }
  const onSignal = (signal: NodeJS.Signals) => {
    interrupted = signal
    if (cleanupStarted) return
    try { clean() } finally { process.exit(signal === 'SIGINT' ? 130 : 143) }
  }
  const onSigint = () => onSignal('SIGINT'); const onSigterm = () => onSignal('SIGTERM')
  ...
  try {
    ...
    if (JSON.stringify(imageInspect?.Config?.Entrypoint) !== JSON.stringify(['/usr/local/bin/web-entrypoint'])) throw new Error('entrypoint')
    if (![undefined, '', '0', '0:0', 'root', 'root:root'].includes(imageInspect?.Config?.User)) throw new Error('imageUser')
    ...
    const control = JSON.parse(await readFile(path.join(root, 'proof-control.json'), 'utf8')) as { raw: string; ref: string }
    const missingResult = docker(['run', '--rm', '--name', missingName, '--label', LABEL, '--mount', mount(missing), image, 'node', '-e', 'process.exit(0)'])
    try { await lstat(missing); throw new Error('missingCreated') } catch (error) { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error }
    if (missingResult.ok) throw new Error('missingSource')
    requireDocker(targetRunArgs(image, target, hostA))
    const targetOutput = await waitForResult(target); assertTargetResult(targetOutput)
    const inspected = requireDocker(['inspect', target]); validateInspect(inspected, hostA, [control.raw, control.ref], REPOSITORY_ROOT)
    const record = JSON.parse(inspected)[0] as Record<string, any>
    if (JSON.stringify(record).includes(hostB) || (record.Mounts as Array<Record<string, unknown>>).some((entry) => entry.Source === root)) throw new Error('siblingMount')
  } finally {
    try { clean() } finally { process.off('SIGINT', onSigint); process.off('SIGTERM', onSigterm) }
  }
  if (interrupted) { process.exitCode = interrupted === 'SIGINT' ? 130 : 143; return }

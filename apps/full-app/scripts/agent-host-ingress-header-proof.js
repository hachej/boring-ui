...
import { AGENT_HOST_CADDY_AMD64_ID, AGENT_HOST_CADDY_COMMAND, AGENT_HOST_CADDY_IMAGE, AGENT_HOST_CADDY_IMAGE_DEFAULTS, AGENT_HOST_CADDYFILE_DIGEST, } from '../src/server/deployment/agentHostIngressArtifacts.js';
...
export function validateAgentHostCaddyImageInspect(raw) {
...
export function validateAgentHostIngressContainerInspect(raw, source) {
...
export function assertAgentHostIngressEcho(raw, authority = AUTHORITY) {
...
async function proof() {
    ...
    const label = `ai.senecapp.agent-host-ingress-proof=${token}`;
    const network = `agent-host-ingress-${token}`;
    ...
    try {
        ...
        if (`sha256:${createHash('sha256').update(config).digest('hex')}` !== AGENT_HOST_CADDYFILE_DIGEST)
            throw new Error('configDigest');
        requireDocker(['pull', AGENT_HOST_CADDY_IMAGE]);
        requireDocker(['pull', AGENT_HOST_INGRESS_PROOF_HELPER_IMAGE]);
        ...
        if (!helper?.RepoDigests?.includes(AGENT_HOST_INGRESS_PROOF_HELPER_IMAGE))
        ...
        for (const testCase of AGENT_HOST_INGRESS_PROOF_CASES)
            assertAgentHostIngressEcho(await waitForEcho(Number(published[1]), testCase.forwarded, testCase.xfh));
    }
    finally {
        try {
            clean();

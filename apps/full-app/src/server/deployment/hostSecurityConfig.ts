...
const VARIABLE_ENV = ['PATH', 'NODE_VERSION', 'YARN_VERSION', 'BORING_AGENT_HOST_ID', 'BORING_AGENT_HOST_OWNER_UID', 'BETTER_AUTH_URL', 'CORS_ORIGINS',
  'BORING_MCP_PROD_ENABLED', 'BORING_AGENT_HOST_MAX_BINDINGS', 'BORING_AGENT_HOST_MAX_BUNDLE_BYTES',
  'BORING_AGENT_HOST_MAX_TOTAL_BUNDLE_BYTES', 'BORING_AGENT_HOST_MAX_CONCURRENT_PRELOADS'] as const
export const AGENT_HOST_SECURITY_ENV_POLICY = Object.freeze(Object.fromEntries(
...
export class AgentHostSecurityConfigError extends Error {
  readonly code = AGENT_HOST_SECURITY_CONFIG_ERROR
  constructor() { super(AGENT_HOST_SECURITY_CONFIG_ERROR); this.name = 'AgentHostSecurityConfigError' }
}

export interface AgentHostSecurityConfigV1 {
...
const FACT_KEYS = ['imageDefaults', 'agentHostId', 'publicationOwnerUid', 'agentMode', 'workspaceRoot', 'sessionRoot', 'trustedProxy',
...
async function parse(rawEnv: unknown, effective: unknown): Promise<AgentHostSecurityConfigV1> {
  ...
  const hostId = string(facts.agentHostId)
  if (!HOST_ID.test(hostId) || hostId !== env.BORING_AGENT_HOST_ID) throw new Error()
  ...
  if (typeof owner !== 'number' || !Number.isSafeInteger(owner) || owner < 0 || owner > 0xffff_fffe || owner === 10001
    || String(owner) !== env.BORING_AGENT_HOST_OWNER_UID) throw new Error()
  ...
  const expectedVariables = {
    ...
    BORING_AGENT_HOST_ID: hostId, BORING_AGENT_HOST_OWNER_UID: String(owner), BETTER_AUTH_URL: url,
    CORS_ORIGINS: cors.join(','), BORING_MCP_PROD_ENABLED: facts.boringMcpEnabled === true ? '1' : '0',
    BORING_AGENT_HOST_MAX_BINDINGS: String(policy.maxBindings), BORING_AGENT_HOST_MAX_BUNDLE_BYTES: String(policy.maxBundleBytes),
    BORING_AGENT_HOST_MAX_TOTAL_BUNDLE_BYTES: String(policy.maxTotalBundleBytes),
    BORING_AGENT_HOST_MAX_CONCURRENT_PRELOADS: String(policy.maxConcurrentPreloads),
  }
  ...
  const projection = Object.freeze({ schemaVersion: 1 as const, domain: DOMAIN, nodeEnv: 'production' as const,
    ...
    agentHostId: hostId, publicationOwnerUid: owner, agentMode: 'vercel-sandbox' as const,
...
export async function createAgentHostSecurityConfig(rawEnv: unknown, effective: unknown): Promise<AgentHostSecurityConfigV1> {
  try { return await parse(rawEnv, effective) } catch { throw new AgentHostSecurityConfigError() }
}

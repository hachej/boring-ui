...
import { captureD1CoreProofRevision, createD1CoreProofBindingIdDigest, createD1CoreProofOperationIdDigest } from './d1CoreProof.js'
...
export interface D1DrCaptureV1 {
  readonly identity: D1DrFingerprintV1
...
async function completeRevisionHistory(hostRoot: string, reader: Pick<D1ImmutableRevisionReader, 'readComplete'>) {
...
async function destructivePublications(rows: readonly unknown[]): Promise<D1DrFingerprintV1['destructivePublications']> {
  ...
  for (const raw of rows) {
    ...
    const operationId = strictD1Ref(row.operation_id, 'operationId')
    ...
    const removalBindingIds = row.removal_binding_ids.map((value) => strictD1Ref(value, 'removalBindingIds'))
    ...
    const identity = {
      ...
      expectedDesiredStateDigest: d1Digest(row.expected_digest, 'expectedDigest'),
      requestedTargetRevisionId: row.source_revision,
      requestedTargetDesiredStateDigest: row.source_digest === null ? null : d1Digest(row.source_digest, 'sourceDigest'),
      publicationRevisionId: row.target_revision,
      publicationDesiredStateDigest: d1Digest(row.target_digest, 'targetDigest'),
      removalBindingIds,
    }
    if (state === 'prepared') {
    ...
    if (state === 'committed') committed.push(Object.freeze({
      operationIdDigest: await createD1CoreProofOperationIdDigest(operationId), state,
      ...
      removalBindingDigests: Object.freeze((await Promise.all(removalBindingIds.map(createD1CoreProofBindingIdDigest))).sort()),
    }))
  }
...
export function createD1DrRowsReader(sql: postgres.Sql): (hostId: string, workspaceIds: readonly string[]) => Promise<D1DrRowsV1> {
    ...
    try {
      return await sql.begin('ISOLATION LEVEL REPEATABLE READ READ ONLY', async (transaction) => {
        ...
        await bound(transaction`SELECT count(*)::text AS rows, coalesce(sum(pg_column_size(a)), 0)::text AS bytes
          FROM d1_binding_admissions a WHERE host_id = ${hostId}`)
        await bound(transaction`SELECT count(*)::text AS rows, coalesce(sum(pg_column_size(j)), 0)::text AS bytes
          FROM d1_destructive_publication_events j WHERE host_id = ${hostId}`)
        ...
        const admissions = await collect(transaction`SELECT sequence::text, binding_id, execution_identity_digest, active_revision,
          first_desired_state_digest, admitted_at FROM d1_binding_admissions WHERE host_id = ${hostId} ORDER BY sequence`)
        const journal = await collect(transaction`SELECT sequence::text, operation_id, state, expected_revision, expected_digest,
          source_revision, source_digest, target_revision, target_digest, removal_binding_ids, recorded_at FROM d1_destructive_publication_events
...
function assertBoundedRows(rows: D1DrRowsV1): void {
  let chars = 0
...
export async function captureD1DrFingerprint(options: {
  readonly reader: D1ActiveCollectionReader
  readonly revisionReader: Pick<D1ImmutableRevisionReader, 'readComplete'>
  readonly hostId: string
  readonly hostRoot: string
  readonly workspaceRoot: string
  readonly sessionRoot: string
  readonly readRows: (hostId: string, workspaceIds: readonly string[]) => Promise<D1DrRowsV1>
}): Promise<D1DrCaptureV1> {
  try {
    ...
    const snapshot = await captureD1CoreProofRevision(options.reader)
    ...
    const [rows, revisions, workspaces, sessions, completeRevisions] = await Promise.all([
      options.readRows(hostId, workspaceIds), treeFingerprint(options.hostRoot, 'boring-d1-dr-revisions:v1'),
      treeFingerprint(options.workspaceRoot, 'boring-d1-dr-workspaces:v1'), treeFingerprint(options.sessionRoot, 'boring-d1-dr-sessions:v1'),
    ...
    const admittedBindingDigests = await Promise.all(rows.admissions.map((row) => {
      ...
      return createD1CoreProofBindingIdDigest((row as Record<string, string>).binding_id)
    }))
    admittedBindingDigests.sort()
    const identity = Object.freeze({
      hostIdentityDigest: await fingerprint('boring-d1-dr-host:v1', hostId),
      admissionHistoryDigest: await fingerprint('boring-d1-dr-admissions:v1', rows.admissions), admissionRows: rows.admissions.length,
      admittedBindingDigests: Object.freeze(admittedBindingDigests),
      journalHistoryDigest: await fingerprint('boring-d1-dr-journal:v1', rows.journal), journalRows: rows.journal.length,
      membershipDigest: await fingerprint('boring-d1-dr-membership:v1', rows.membership), membershipRows: rows.membership.length,
      ...
      stateRootDigest: await fingerprint('boring-d1-dr-state-root:v1', options.hostRoot),
      workspaceRootDigest: await fingerprint('boring-d1-dr-workspace-root:v1', options.workspaceRoot), workspaceDataDigest: workspaces.digest,
      sessionRootDigest: await fingerprint('boring-d1-dr-session-root:v1', options.sessionRoot), sessionHistoryDigest: sessions.digest,
    })

...
export interface AgentHostProvidedSecretV1 {
...
export interface AgentHostProvidedSecretMetadataV1 {
  readonly secretRef: string
  readonly providerVersionFingerprint: Sha256Digest
}
export interface AgentHostProvidedBindingInspectionV1 {
  ...
  readonly secrets: readonly AgentHostProvidedSecretMetadataV1[]
}
export interface AgentHostResolvedBindingSecretsV1 {
  readonly bindingId: string
  readonly secrets: readonly AgentHostProvidedSecretV1[]
}
export interface AgentHostBindingSecretProvider {
  /** Metadata-only: implementations must not fetch or expose secret values. */
  inspect(binding: AgentHostSiteBindingV1): Promise<AgentHostProvidedBindingInspectionV1>
  /** Value-bearing: ownership of every returned buffer transfers to the caller. */
  resolveSecrets(binding: AgentHostSiteBindingV1): Promise<AgentHostResolvedBindingSecretsV1>
}
export interface AgentHostSecretMaterializerOptions {
  readonly root: string
  readonly ownerUid: number
  readonly appUid: number
  readonly appGid: number
  readonly provider: AgentHostBindingSecretProvider
...
interface InspectedBinding { readonly inspection: AgentHostRuntimeInputsInspectionV1; readonly identity: AgentHostRuntimeInputsIdentityV1 }
...
interface ResolvedBinding {
  ...
  readonly identity: AgentHostRuntimeInputsIdentityV1
  readonly inspection: AgentHostRuntimeInputsInspectionV1
  readonly secrets: readonly OwnedSecret[]
}

function fail(code: AgentHostErrorCode, field: string): never { throw new AgentHostError(code, { field }) }
...
function providerFailure(error: unknown): never {
  if (error instanceof AgentHostError && (
    error.code === AgentHostErrorCode.SECRET_UNAVAILABLE ||
    error.code === AgentHostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED ||
    error.code === AgentHostErrorCode.COLLECTION_NOT_READY
  )) throw error
  fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
}
async function inspectBinding(binding: AgentHostSiteBindingV1, provider: AgentHostBindingSecretProvider): Promise<InspectedBinding> {
  let raw: unknown
  try { raw = await provider.inspect(binding) } catch { fail(AgentHostErrorCode.SECRET_UNAVAILABLE, 'secret') }
  try {
    ...
    const bindingId = strictAgentHostRef(raw.bindingId, 'provider.bindingId')
    const secrets = raw.secrets.map((value) => {
      if (!exact(value, ['secretRef', 'providerVersionFingerprint'])) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      return Object.freeze({
        secretRef: strictAgentHostRef(value.secretRef, 'provider.secretRef'),
        providerVersionFingerprint: agentHostDigest(value.providerVersionFingerprint, 'provider.providerVersionFingerprint'),
      })
    }).sort((left, right) => left.secretRef < right.secretRef ? -1 : left.secretRef > right.secretRef ? 1 : 0)
    if (new Set(secrets.map((secret) => secret.secretRef)).size !== secrets.length) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
    const actualRefs = secrets.map((secret) => secret.secretRef)
    if (binding.secretRefs.some((ref) => !actualRefs.includes(ref))) fail(AgentHostErrorCode.SECRET_UNAVAILABLE, 'secret')
    if (!same(actualRefs, binding.secretRefs) || bindingId !== binding.bindingId) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
    const attestation: AgentHostRuntimeInputsAttestationV1 = Object.freeze({
      environment: Object.freeze({ versionFingerprint: agentHostDigest(raw.environmentVersionFingerprint, 'provider.environmentVersionFingerprint') }),
      workspaceAllocation: Object.freeze({ versionFingerprint: agentHostDigest(raw.workspaceAllocationVersionFingerprint, 'provider.workspaceAllocationVersionFingerprint') }),
      sessionAllocation: Object.freeze({ versionFingerprint: agentHostDigest(raw.sessionAllocationVersionFingerprint, 'provider.sessionAllocationVersionFingerprint') }),
      secrets: Object.freeze(secrets),
    })
    return Object.freeze({
      inspection: Object.freeze({ bindingId, attestation }),
      identity: await createAgentHostRuntimeInputsIdentity(binding, attestation),
    })
  } catch (error) { return providerFailure(error) }
}

export function createAgentHostRuntimeInputsInspector(provider: AgentHostBindingSecretProvider) {
  return async (desired: AgentHostDesiredSnapshotV1): Promise<readonly AgentHostRuntimeInputsInspectionV1[]> => Object.freeze(
    await Promise.all(desired.plan.bindings.map(async (binding) => (await inspectBinding(binding, provider)).inspection)),
  )
}

async function resolveBinding(
  binding: AgentHostSiteBindingV1,
  inspected: InspectedBinding,
  provider: AgentHostBindingSecretProvider,
  owned: Uint8Array[],
  totals: { files: number; bytes: number },
  seenBuffers: Set<ArrayBuffer>,
): Promise<ResolvedBinding> {
  let raw: unknown
  try { raw = await provider.resolveSecrets(binding) } catch { fail(AgentHostErrorCode.SECRET_UNAVAILABLE, 'secret') }
  const transferred: Uint8Array[] = []
  try {
    if (!isRecord(raw)) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
    ...
    const captured = raw.secrets.map((value) => {
      if (!isRecord(value)) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      const descriptor = Object.getOwnPropertyDescriptor(value, 'value')
      const bytes = descriptor && Object.hasOwn(descriptor, 'value') ? descriptor.value : undefined
      if (!exact(value, ['secretRef', 'providerVersionFingerprint', 'value'])) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      if (!(bytes instanceof Uint8Array) || bytes.buffer instanceof SharedArrayBuffer ||
        ...
        fail(AgentHostErrorCode.SECRET_UNAVAILABLE, 'secret')
      }
      seenBuffers.add(bytes.buffer)
      return { value, bytes }
    })
    const secrets: OwnedSecret[] = captured.map(({ value, bytes: source }) => {
      const secretRef = strictAgentHostRef(value.secretRef, 'provider.secretRef')
      const fingerprint = agentHostDigest(value.providerVersionFingerprint, 'provider.providerVersionFingerprint')
      const bytes = Uint8Array.from(source); zero(source); owned.push(bytes)
      totals.files += 1; totals.bytes += bytes.byteLength
      if (totals.files > MAX_SECRET_FILES || totals.bytes > MAX_TOTAL_BYTES) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      return { ref: secretRef, fingerprint, bytes }
    }).sort((left, right) => left.ref < right.ref ? -1 : left.ref > right.ref ? 1 : 0)
    if (new Set(secrets.map((secret) => secret.ref)).size !== secrets.length) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
    ...
    if (!same(metadata, inspected.inspection.attestation.secrets)) fail(AgentHostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED, 'runtimeInputs')
...
function manifest(candidate: AgentHostStoredCandidateV1, resolved: readonly ResolvedBinding[]): string {
  return JSON.stringify({
    schemaVersion: 1,
    domain: 'boring-agent-host-binding-secrets:v1',
...
export function createAgentHostBindingSecretMaterializer(options: AgentHostSecretMaterializerOptions): AgentHostApplyEffects['materialize'] {
  ...
  return async (candidate, rawExpected) => {
    ...
    try {
      if (!REVISION_RE.test(candidate.revisionId)) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      const desired = await canonicalizeAgentHostDesiredSnapshot(candidate.desired)
      if (await digestAgentHostDesired(desired) !== candidate.desiredStateDigest || !same(canonicalizeAgentHostSecretRefsEnvelope(candidate.secretRefs, desired), candidate.secretRefs)) {
        fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      }
      const expected = await Promise.all(rawExpected.map((value) => {
        ...
        if (!binding) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      ...
      if (!same(expected.map((value) => value.bindingId), desired.plan.bindings.map((binding) => binding.bindingId))) fail(AgentHostErrorCode.COLLECTION_NOT_READY, 'materialize')
      ...
      if (!same(inspected.map((entry) => entry.identity), expected)) fail(AgentHostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED, 'runtimeInputs')

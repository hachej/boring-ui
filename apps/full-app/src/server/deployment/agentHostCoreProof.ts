...
export interface AgentHostCoreProofBindingV1 {
...
export interface AgentHostCoreProofRevisionV1 {
  readonly revisionId: string
  readonly desiredStateDigest: Sha256Digest
  readonly coreImageDigest: Sha256Digest
  readonly bindings: readonly AgentHostCoreProofBindingV1[]
}
export interface AgentHostCoreProofReportV1 {
...
function record(value: unknown, keys: readonly string[]): Record<string, unknown> {
  try {
    assertAgentHostExactKeys(value, keys, 'proof')
    return value
  } catch {
    failed()
  }
...
function digest(value: unknown): Sha256Digest {
  try {
    return agentHostDigest(value, 'proof')
  } catch {
    failed()
  }
}
function ref(value: unknown): string {
  try {
    return strictAgentHostRef(value, 'proof')
  } catch {
    failed()
  }
}
...
export async function createAgentHostCoreProofBindingIdDigest(value: string): Promise<Sha256Digest> {
  return identityDigest('boring-agent-host-proof-binding:v1', ref(value))
}
export async function createAgentHostCoreProofOperationIdDigest(value: string): Promise<Sha256Digest> {
  return identityDigest('boring-agent-host-proof-operation:v1', ref(value))
}

/** Captures only redacted identities from the same served authority used by live AgentHost requests. */
export async function captureAgentHostCoreProofRevision(reader: AgentHostActiveCollectionReader): Promise<AgentHostCoreProofRevisionV1> {
  try {
    ...
    const bindings = await Promise.all(
      collection.desired.plan.bindings.map(async (binding) => {
        ...
        return Object.freeze({
          ...
          bindingIdDigest: await createAgentHostCoreProofBindingIdDigest(binding.bindingId),
          hostnameDigest: await identityDigest('boring-agent-host-proof-hostname:v1', binding.hostname),
          workspaceIdDigest: await identityDigest('boring-agent-host-proof-workspace:v1', binding.workspaceId),
...
async function binding(raw: unknown): Promise<AgentHostCoreProofBindingV1> {
  ...
  if (bindingIdDigest !== await createAgentHostCoreProofBindingIdDigest(bindingId)) failed()
  return Object.freeze({
    bindingId,
    bindingIdDigest,
...
async function snapshot(raw: unknown): Promise<AgentHostCoreProofRevisionV1> {
...
function authorization(raw: unknown, initial: AgentHostCoreProofRevisionV1): void {
  ...
  const actual = raw.map((item) => {
    ...
    const cross = check(value.crossBinding, 'denied', AgentHostErrorCode.HOST_SCOPE_VIOLATION)
    if (
...
export async function verifyAgentHostCoreProof(raw: unknown, liveIsolationEvidence: unknown): Promise<AgentHostCoreProofReportV1> {
  try {
    const value = record(raw, ['schemaVersion', 'domain', 'initial', 'authorization', 'nPlusOne', 'rollback', 'timing', 'dr', 'redaction'])
    if (value.schemaVersion !== 1 || value.domain !== 'boring-agent-host-core-proof:v1') failed()

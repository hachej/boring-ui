...
const attestedBrand: unique symbol = Symbol('AttestedAgentHostDatabaseConnection')
const liveConnections = new WeakMap<AttestedAgentHostDatabaseConnection, LiveConnection>()
...
export interface AttestedAgentHostDatabaseConnection {
  readonly [attestedBrand]: true
}
export interface AgentHostAdmissionTarget {
  readonly hostId: string
  readonly bindingId: string
  readonly workspaceId: string
  readonly defaultDeploymentId: string
}
export interface AgentHostBindingAdmission {
...
export interface AgentHostAdmissionFenceKey {
  readonly hostId: string
  readonly bindingId: string
}
export interface AgentHostAdmissionLedger {
  readonly databaseRef: string
  listBindingIds(hostId: string, databaseRef: string): Promise<readonly string[]>
  admit(activeReader: AgentHostActiveCollectionReader, target: AgentHostAdmissionTarget): Promise<AgentHostBindingAdmission>
  withBindingFences<T>(keys: readonly AgentHostAdmissionFenceKey[], operation: (sql: postgres.ReservedSql) => Promise<T>): Promise<T>
  close(): Promise<void>
}
...
function failed(): AgentHostError {
  return new AgentHostError(AgentHostErrorCode.ADMISSION_RECORD_FAILED, { field: 'admission' })
}
function mismatch(): AgentHostError {
  return new AgentHostError(AgentHostErrorCode.ADMISSION_IDENTITY_MISMATCH, { field: 'executionIdentityDigest' })
}
function preserveMismatch(error: unknown): never {
  if (error instanceof AgentHostError && error.code === AgentHostErrorCode.ADMISSION_IDENTITY_MISMATCH) throw error
  throw failed()
}
...
function row(value: AdmissionRow | undefined): AgentHostBindingAdmission {
  ...
  return Object.freeze({
    sequence,
    hostId: strictAgentHostId(value.hostId, 'admission.hostId'),
    bindingId: strictAgentHostRef(value.bindingId, 'admission.bindingId'),
    executionIdentityDigest: agentHostDigest(value.executionIdentityDigest, 'admission.executionIdentityDigest'),
    firstRevisionId: revision(value.firstRevisionId),
    firstDesiredStateDigest: agentHostDigest(value.firstDesiredStateDigest, 'admission.firstDesiredStateDigest'),
    admittedAt: new Date(value.admittedAt),
  })
}

function take(connection: AttestedAgentHostDatabaseConnection): LiveConnection {
...
export function isAgentHostReservedConnectionLost(sql: postgres.ReservedSql): boolean {
  return reservedConnectionStates.get(sql)?.lost ?? false
}

/** AgentHost-005c is the sole production caller after it proves this client. */
export function mintAttestedAgentHostDatabaseConnection(
  databaseRef: string,
  sql: postgres.Sql,
  options: { readonly ownsClient?: boolean } = {},
): AttestedAgentHostDatabaseConnection {
  const capability = Object.freeze({ [attestedBrand]: true as const })
  liveConnections.set(capability, { databaseRef: strictAgentHostRef(databaseRef, 'databaseRef'), sql, ownsClient: options.ownsClient !== false })
  return capability
}

export async function closeAttestedAgentHostDatabaseConnection(connection: AttestedAgentHostDatabaseConnection): Promise<void> {
  const live = liveConnections.get(connection)
  if (!live) return
  liveConnections.delete(connection)
...
async function insertOrRead(sql: postgres.ReservedSql, target: AgentHostAdmissionTarget, identity: AdmissionIdentity): Promise<AgentHostBindingAdmission> {
  let transaction = false
  try {
    if (isAgentHostReservedConnectionLost(sql)) throw failed()
    await sql`BEGIN`; transaction = true
    const [pendingRemoval] = await sql<{ pending: boolean }[]>`
      SELECT EXISTS (
        SELECT 1 FROM agent_host_destructive_publication_events AS prepared
        WHERE prepared.host_id = ${target.hostId}
          AND prepared.state = 'prepared'
          AND ${target.bindingId} = ANY(prepared.removal_binding_ids)
          AND NOT EXISTS (
            SELECT 1 FROM agent_host_destructive_publication_events AS terminal
    ...
    const existing = inserted[0] ?? (await sql<AdmissionRow[]>`
      SELECT sequence, host_id AS "hostId", binding_id AS "bindingId",
        execution_identity_digest AS "executionIdentityDigest", active_revision AS "firstRevisionId",
        first_desired_state_digest AS "firstDesiredStateDigest", admitted_at AS "admittedAt"
      FROM agent_host_binding_admissions
...
async function admissionIdentity(
  active: NonNullable<Awaited<ReturnType<AgentHostActiveCollectionReader['read']>>>,
  bindingId: string,
  ...
  const executionIdentityDigest = await createAgentAssetDigest(JSON.stringify({
    schemaVersion: 1,
    domain: 'boring-agent-host-admission-execution:v1',
  ...
  return Object.freeze({
    ...
    firstDesiredStateDigest: agentHostDigest(active.active.desiredStateDigest, 'active.desiredStateDigest'),
  })
}

export function createAgentHostAdmissionLedger(
  connection: AttestedAgentHostDatabaseConnection,
): AgentHostAdmissionLedger {
  ...
  const claim = async (sql: postgres.ReservedSql) => {
    const state: ReservedConnectionState = { connectionId: null, lost: false }; const token = `boring-agent-host-reservation:${++nextReservationClaim}`
  ...
  const key = (raw: AgentHostAdmissionFenceKey): AgentHostAdmissionFenceKey => Object.freeze({
    hostId: strictAgentHostId(raw.hostId, 'hostId'), bindingId: strictAgentHostRef(raw.bindingId, 'bindingId'),
  })
  const lockName = (value: AgentHostAdmissionFenceKey) => `boring-agent-host-admission:v1:${value.hostId.length}:${value.hostId}:${value.bindingId.length}:${value.bindingId}`

  const withBindingFences = async <T>(rawKeys: readonly AgentHostAdmissionFenceKey[], operation: (sql: postgres.ReservedSql) => Promise<T>): Promise<T> => {
    available()
    ...
    } catch (caught) { unsafe = isAgentHostReservedConnectionLost(sql) || connectionLost(caught); error = operationStarted ? caught : failed() }
    finally {
      let connectionUnsafe = isAgentHostReservedConnectionLost(sql) || unsafe || connectionLost(error)
  ...
  const listBindingIds = async (hostId: string, databaseRef: string): Promise<readonly string[]> => {
    available()
    const expectedHost = strictAgentHostId(hostId, 'hostId')
    if (strictAgentHostRef(databaseRef, 'databaseRef') !== live.databaseRef) throw failed()
    const sql = await live.sql.reserve().catch(() => { throw failed() })
    try {
      await claim(sql)
      ...
      `
      return Object.freeze(rows.map((entry) => strictAgentHostRef(entry.bindingId, 'admission.bindingId')))
    } catch (caught) { if (isAgentHostReservedConnectionLost(sql) || connectionLost(caught)) await abandon(); throw failed() } finally { if (!closed) try { releaseState(sql); sql.release() } catch { await abandon(); throw failed() } }
  }

  const admit = async (activeReader: AgentHostActiveCollectionReader, rawTarget: AgentHostAdmissionTarget): Promise<AgentHostBindingAdmission> => {
    try {

...
import {
  createAgentHostCaddyfileAuthorityReaderForPolicy,
  AGENT_HOST_CADDYFILE_AUTHORITY_POLICY,
  AGENT_HOST_CADDYFILE_MAX_BYTES,
  AGENT_HOST_CADDYFILE_PATH,
  type AgentHostCaddyfileAuthorityPolicy,
} from '../agentHostCaddyfileAuthority.js'
...
async function fixture() {
  const parent = await mkdtemp(path.join(os.tmpdir(), 'boring-agent-host-caddy-authority-'))
  ...
  const policy: AgentHostCaddyfileAuthorityPolicy = {
    directoryPath,
    directoryUid: UID,
    directoryGid: GID,
    directoryMode: 0o755,
...
function reader(policy: AgentHostCaddyfileAuthorityPolicy) {
  return createAgentHostCaddyfileAuthorityReaderForPolicy(policy)
}
...
async function expectUnavailable(action: Promise<unknown>): Promise<void> {
  ...
  expect(JSON.stringify(failure)).not.toMatch(new RegExp(`${CANARY}|boring-agent-host-caddy-authority-|Caddyfile`))
}
...
describe('AgentHost Caddyfile authority', () => {
  ...
  it('hard-codes the production path and root-owned immutable policy', () => {
    expect(AGENT_HOST_CADDYFILE_PATH).toBe('/opt/boring/agent-host/Caddyfile')
    expect(AGENT_HOST_CADDYFILE_AUTHORITY_POLICY).toEqual({
      directoryPath: '/opt/boring/agent-host',
      directoryUid: 0,
      directoryGid: 0,
      directoryMode: 0o755,
      fileUid: 0,
    ...
    expect(Object.isFrozen(AGENT_HOST_CADDYFILE_AUTHORITY_POLICY)).toBe(true)
  })

  it.each([
    ['directory owner', (p: AgentHostCaddyfileAuthorityPolicy) => ({ ...p, directoryUid: p.directoryUid + 1 })],
    ['directory group', (p: AgentHostCaddyfileAuthorityPolicy) => ({ ...p, directoryGid: p.directoryGid + 1 })],
    ['directory mode', (p: AgentHostCaddyfileAuthorityPolicy) => ({ ...p, directoryMode: 0o750 })],
    ['file owner', (p: AgentHostCaddyfileAuthorityPolicy) => ({ ...p, fileUid: p.fileUid + 1 })],
    ['file group', (p: AgentHostCaddyfileAuthorityPolicy) => ({ ...p, fileGid: p.fileGid + 1 })],
    ['file mode', (p: AgentHostCaddyfileAuthorityPolicy) => ({ ...p, fileMode: 0o440 })],
  ...
  it('rejects a non-regular, missing, empty, or oversized file', async () => {
    for (const mutation of ['directory', 'missing', 'empty', 'oversized']) {
      ...
      if (mutation === 'directory') await mkdir(h.file, { mode: 0o444 })
        ...
        const contents = mutation === 'empty' ? new Uint8Array() : new Uint8Array(AGENT_HOST_CADDYFILE_MAX_BYTES + 1)
  ...
  it('rejects noncanonical and hostile policy without leaking canaries', async () => {
    ...
    const hostile = new Proxy(h.policy, {
      get() {
        throw new AgentHostError(AgentHostErrorCode.COLLECTION_NOT_READY, { field: CANARY })
      },
    })
  ...
  it('has no writer API and reads only through anchored no-follow descriptors', async () => {
    const source = await readFile(new URL('../agentHostCaddyfileAuthority.ts', import.meta.url), 'utf8')

...
import { deriveD1SecretRefsEnvelope, digestD1Desired, createD1DesiredSnapshot, type D1DesiredSnapshotV1 } from '../d1RevisionCodec.js'
import { createD1RuntimeInputsIdentity, type D1RuntimeInputsAttestationV1 } from '../d1RuntimeInputs.js'
import {
  createD1BindingSecretMaterializer,
  createD1RuntimeInputsInspector,
  type D1BindingSecretProvider,
  type D1ProvidedBindingInspectionV1,
  type D1ResolvedBindingSecretsV1,
...
function attestation(secretRefs: readonly string[], secretFingerprint = sha('4')): D1RuntimeInputsAttestationV1 {
  return {
...
async function fixture(secretRefs: readonly string[] = ['credential-ref', 'secondary-ref'], ids: readonly string[] = ['insurance']) {
  ...
  const expected = await Promise.all(snapshot.plan.bindings.map((binding) => createD1RuntimeInputsIdentity(binding, attestation(secretRefs))))
...
function provided(secretRefs: readonly string[], bytes = (ref: string) => new TextEncoder().encode(`value:${ref}`), bindingId = 'insurance') {
  ...
  const inspection: D1ProvidedBindingInspectionV1 = {
  ...
  const resolved: D1ResolvedBindingSecretsV1 = { bindingId, secrets: values }
  return { inspection, resolved, values: values.map((entry) => entry.value) }
}
function provider(input: ReturnType<typeof provided>): D1BindingSecretProvider {
  return { inspect: async () => input.inspection, resolveSecrets: async () => input.resolved }
}
async function tmpfsRoot() { return mkdtemp(path.join(TMPFS_PARENT, 'boring-d1-secrets-')) }
function materializer(root: string, provider: D1BindingSecretProvider, fault?: (point: 'before-final-rename' | 'after-final-rename') => void | Promise<void>) {
  return createD1BindingSecretMaterializer({ root, ownerUid: OWNER_UID, appUid: APP_UID, appGid: APP_GID, provider, fault })
}

describe('D1 binding secret materializer', () => {
  it('publishes a canonical redacted manifest and ordinal secret files with exact metadata', async () => {
    ...
    expect(manifest).toEqual({
      schemaVersion: 1, domain: 'boring-d1-binding-secrets:v1', hostId: 'host-1', revisionId: 'r0000000001',
  ...
  it('compares exact runtime identity before creating a stage and redacts failures', async () => {
    ...
    const wrong = [await createD1RuntimeInputsIdentity(f.snapshot.plan.bindings[0]!, attestation(['credential-ref', 'secondary-ref'], sha('5')))]
    ...
    expect(error).toMatchObject({ code: D1HostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED, details: { field: 'runtimeInputs' } })
  ...
  it('inspects canonical metadata without resolving secret values', async () => {
    ...
    const inspected = await createD1RuntimeInputsInspector({
      inspect: async () => p.inspection,
      resolveSecrets: async () => { resolved = true; return p.resolved },
    })(f.snapshot)
    expect(inspected).toEqual([{ bindingId: 'insurance', attestation: attestation(['credential-ref', 'secondary-ref']) }])
  ...
  it('fails closed for provider omissions, extras, malformed values, and resource bounds', async () => {
    ...
    const cases: Array<{ source: D1BindingSecretProvider; code: string }> = [
      { source: provider(missing), code: D1HostErrorCode.SECRET_UNAVAILABLE },
      { source: provider(extra), code: D1HostErrorCode.COLLECTION_NOT_READY },
      {
        source: { ...provider(valid), inspect: async () => ({ ...valid.inspection, unexpected: true }) as D1ProvidedBindingInspectionV1 },
        code: D1HostErrorCode.COLLECTION_NOT_READY,
      },
      { source: provider(oversized), code: D1HostErrorCode.SECRET_UNAVAILABLE },
      { source: provider(empty), code: D1HostErrorCode.SECRET_UNAVAILABLE },
      {
        ...
        code: D1HostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED,
      },
    ]
  ...
  it('rejects non-tmpfs, unsafe, and symlinked runtime roots before touching the provider', async () => {
    const regular = await mkdtemp(path.join(os.tmpdir(), 'boring-d1-secrets-'))
    ...
    for (const root of [regular, unsafe, linked]) {
      ...
      const source: D1BindingSecretProvider = {
      ...
      expect(error).toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY, details: { field: 'materialize' } })
      expect(calls).toBe(0)
      expect(p.values.some((value) => value.some((byte) => byte !== 0))).toBe(true)
    }
  })
  ...
  it('rejects aliased provider buffers before copying and zeroes the transfer', async () => {
    ...
    await expect(materializer(root, source)(f.candidate, f.expected))
      .rejects.toMatchObject({ code: D1HostErrorCode.SECRET_UNAVAILABLE })
  ...
  it('captures only data properties and zeroes through the Uint8Array builtin', async () => {
    ...
    const source: D1BindingSecretProvider = {
      ...
      }) as unknown as D1ResolvedBindingSecretsV1,
    }
    ...
    })(extraFixture.candidate, extraFixture.expected)).rejects.toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY })
    expect([...extra]).toEqual([0])

    for (const mutate of [
      (resolved: D1ResolvedBindingSecretsV1) => ({ ...resolved, unexpected: true }),
      (resolved: D1ResolvedBindingSecretsV1) => ({ ...resolved, bindingId: '../invalid' }),
    ]) {
      ...
      await expect(materializer(outerRoot, {
        ...
        resolveSecrets: async () => mutate(outer.resolved) as D1ResolvedBindingSecretsV1,
      })(extraFixture.candidate, extraFixture.expected)).rejects.toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY })
  ...
  it('enforces collection file-count and aggregate-byte caps before staging', async () => {
    ...
    })(f.candidate, f.expected)).rejects.toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY })
  ...
  it('leaves a hidden stage on pre-rename failure and adopts an exact post-rename retry', async () => {
    ...
    expect(afterError).toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY })

...
const SOURCE_ENTRY: D1EntryInvocation = {
  ...
  args: ['--import', 'tsx', path.resolve('src/server/deployment/d1CommandEntry.ts')],
}
...
async function roots(hosts: readonly string[] = ['host-1']): Promise<Roots> {
  const base = await mkdtemp(path.join(os.tmpdir(), 'boring-d1-cli-'))
  ...
  return { base, lockRoot, stateRoot, env: { ...process.env, BORING_D1_OWNER_UID: String(UID), BORING_D1_STATE_ROOT: stateRoot, BORING_D1_LOCK_ROOT: lockRoot } }
}
...
function childScript(line = SUCCESS, delayMs = 0, setup = ''): D1EntryInvocation {
...
describe('D1 revision command boundary', () => {
  it('allows only the explicit production and CI local filesystem set', () => {
    for (const type of [0xef53, 0x58465342, 0x9123683e, 0x01021994, 0x794c7630]) expect(isSupportedLocalD1LockFilesystem(type)).toBe(true)
    for (const type of [0x6969, 0xff534d42, 0xfe534d42, 0x65735546, 0x517b, 0x00c36400]) expect(isSupportedLocalD1LockFilesystem(type)).toBe(false)
  })

  it('selects the source or built sibling from the wrapper location only and fails when it is missing', async () => {
    const source = resolveD1EntryInvocation(pathToFileURL(path.resolve('src/server/deployment/d1CommandWrapper.ts')).href)
    expect(source.args.at(-1)).toBe(path.resolve('src/server/deployment/d1CommandEntry.ts'))
    const built = await mkdtemp(path.join(os.tmpdir(), 'boring-d1-built-entry-')); const builtWrapper = path.join(built, 'd1CommandWrapper.js')
    await writeFile(builtWrapper, ''); await writeFile(path.join(built, 'd1CommandEntry.js'), '')
    expect(resolveD1EntryInvocation(pathToFileURL(builtWrapper).href).args.at(-1)).toBe(path.join(built, 'd1CommandEntry.js'))
    const fake = await mkdtemp(path.join(os.tmpdir(), 'boring-d1-entry-')); const wrapper = path.join(fake, 'd1CommandWrapper.js')
    await writeFile(wrapper, '')
    expect(() => resolveD1EntryInvocation(pathToFileURL(wrapper).href)).toThrowError(expect.objectContaining({ code: D1HostErrorCode.PLAN_INVALID, details: { field: 'entry' } }))
  })
  ...
  it('rejects missing, symlinked, hard-linked, wrong-mode, and wrong-owner lock configuration', async () => {
    ...
    expect((await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: missing.env, entry: childScript() })).exitCode).toBe(2)
    const wrongMode = await roots(); await chmod(path.join(wrongMode.lockRoot, 'host-1.lock'), 0o644)
    expect((await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: wrongMode.env, entry: childScript() })).exitCode).toBe(2)
    const hard = await roots(); await link(path.join(hard.lockRoot, 'host-1.lock'), path.join(hard.lockRoot, 'alias'))
    expect((await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: hard.env, entry: childScript() })).exitCode).toBe(2)
    const linked = await roots(); const alias = path.join(linked.base, 'locks-link'); await symlink(linked.lockRoot, alias)
    expect((await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: { ...linked.env, BORING_D1_LOCK_ROOT: alias }, entry: childScript() })).exitCode).toBe(2)
    const owner = await roots()
    expect((await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: { ...owner.env, BORING_D1_OWNER_UID: String(UID + 1) }, entry: childScript() })).exitCode).toBe(2)
  })
  ...
  it('holds one host lock across FD3 close, rejects a contender, and does not serialize other hosts', async () => {
    ...
    const first = runD1RevisionWrapper({
    ...
    const contender = await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: h.env, entry: childScript() })
    expect(contender).toEqual(expect.objectContaining({ exitCode: 3 })); expect(contender.line).toContain(D1HostErrorCode.REVISION_CONFLICT)
    const other = await runD1RevisionWrapper({ stdin: Readable.from([minimal('apply', 'host-2')]), env: h.env, entry: childScript() })
  ...
  it('does not construct dependencies for a missing direct FD3', async () => {
    ...
    const keys = ['BORING_D1_OWNER_UID', 'BORING_D1_STATE_ROOT', 'BORING_D1_LOCK_ROOT'] as const
  ...
  it('rejects a wrong-file FD3 before dependency construction', async () => {
    ...
    expect(result.code).toBe(2); expect(result.stdout).toContain(D1HostErrorCode.PLAN_INVALID)
    await expect(access(marker)).rejects.toMatchObject({ code: 'ENOENT' })
  }, 15_000)
  ...
  it('rejects a separately opened FD3 when another OFD owns the canonical lock', async () => {
    ...
    const result = await directEntry(h, marker, await open(file, constants.O_RDWR | constants.O_NOFOLLOW))
    expect(result.code).toBe(3); expect(result.stdout).toContain(D1HostErrorCode.REVISION_CONFLICT)
    await expect(access(marker)).rejects.toMatchObject({ code: 'ENOENT' })
  ...
  it('accepts only the fixed child envelope and redacts malformed output and stderr', async () => {
    ...
    for (const script of [
      ...
      const output = await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: h.env, entry: script })
      expect(output).toEqual({ line: '{"ok":false,"error":{"code":"D1_PUBLICATION_FAILED","details":{"field":"command"}}}\n', exitCode: 70 })
  ...
  it('kills an orphaned process group on normal leader exit before releasing the lock', async () => {
    ...
    const output = await runD1RevisionWrapper({ stdin: Readable.from([minimal()]), env: h.env, entry: childScript(SUCCESS, 0, setup) })
  ...
  it('bounds signal shutdown, kills a SIGTERM-ignoring group, then releases the lock', async () => {
    ...
    expect(result).toEqual({ code: 143, stdout: '{"ok":false,"error":{"code":"D1_PUBLICATION_FAILED","details":{"field":"signal"}}}\n' })
  ...
  it('reports a signal latched after leader exit while group cleanup is running', async () => {
    ...
    expect(result).toEqual({ code: 130, stdout: '{"ok":false,"error":{"code":"D1_PUBLICATION_FAILED","details":{"field":"signal"}}}\n' })
  ...
  it('keeps the host lock held by the entry after abrupt wrapper death', async () => {
    ...
    const wrapperUrl = pathToFileURL(path.resolve('src/server/deployment/d1CommandWrapper.ts')).href
    const harness = `import {runD1RevisionWrapper} from ${JSON.stringify(wrapperUrl)};const out=await runD1RevisionWrapper({entry:{command:process.execPath,args:['-e',${JSON.stringify(entryCode)},'--']}});process.stdout.write(out.line);process.exitCode=out.exitCode`

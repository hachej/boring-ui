...
async function desired(specs: readonly BindingSpec[] = [{ id: 'insurance' }], host: HostSpec = {}): Promise<D1DesiredSnapshotV1> {
  ...
  return createD1DesiredSnapshot({ schemaVersion: 1, hostId: 'host-1', expectedHostRevision: null, hostAppImageDigest: hostImage, runtimeProfileRef: runtimeProfile, databaseRef: host.database ?? 'postgres-eu', workspaceRootPolicyRef: workspacePolicy, sessionRootPolicyRef: sessionPolicy, bindings: planBindings }, resolvedBindings)
}
interface RuntimeVersionSpec { environment?: string; workspace?: string; session?: string; secret?: string }
function attestations(value: D1DesiredSnapshotV1, versions: RuntimeVersionSpec = {}) {
  return value.plan.bindings.map((binding) => ({
    ...
    } satisfies D1RuntimeInputsAttestationV1,
  }))
}
async function runtimeIdentities(value: D1DesiredSnapshotV1, versions: RuntimeVersionSpec = {}) {
  return Promise.all(attestations(value, versions).map(({ bindingId, attestation }) =>
    createD1RuntimeInputsIdentity(value.plan.bindings.find((binding) => binding.bindingId === bindingId)!, attestation)))
}
function observation(value: D1DesiredSnapshotV1, identities: readonly D1RuntimeInputsIdentityV1[]): D1ObservationV1 {
  const runtime = new Map(identities.map((identity) => [identity.bindingId, identity]))
  return { schemaVersion: 1, domain: 'boring-d1-observed:v1', bindings: value.resolvedBindings.map((binding) => ({ bindingId: binding.bindingId, ready: true, resolvedDigest: binding.resolvedDigest, runtimeInputs: runtime.get(binding.bindingId)! })) }
}

class FakeStore implements D1HostRevisionStore {
  active: D1ActiveEnvelopeV1 | null = null
  completes = new Map<string, D1StoredCompleteV1>()
  candidates = new Map<string, D1StoredCandidateV1>()
  observations = new Map<string, D1ObservationV1>()
  reserved = new Set<string>()
  audits: D1AuditRecordV1[] = []
  calls: string[] = []
  adapterErrors: Partial<Record<'inspect' | 'materialize' | 'preload', unknown>> = {}
  materializedOutput?: readonly D1RuntimeInputsInspectionV1[]
  ...
  async seed(value: D1DesiredSnapshotV1, revisionId = 'r0000000001', terminal = true, versions: RuntimeVersionSpec = {}) {
    ...
    const completion = await createD1CompleteEnvelope(revisionId, value, observed)
    const complete = Object.freeze({ revisionId, desired: value, desiredStateDigest, secretRefs: deriveD1SecretRefsEnvelope(value), observation: observed, completion })
  ...
  record(complete: D1StoredCompleteV1): D1AuditRecordV1 {
  ...
  async writeObservation(_host: string, revisionId: string, value: D1ObservationV1) {
    ...
    const candidate = this.candidates.get(revisionId); if (!candidate || this.observations.has(revisionId)) throw new D1HostError(D1HostErrorCode.COLLECTION_NOT_READY, { field: 'observation' })
    const canonical = await canonicalizeD1Observation(value, candidate.desired); this.observations.set(revisionId, canonical); return canonical
  }
  async writeComplete(_host: string, revisionId: string) {
    ...
    const candidate = this.candidates.get(revisionId)!; const observed = this.observations.get(revisionId)!; const completion = await createD1CompleteEnvelope(revisionId, candidate.desired, observed)
  ...
  async publishActive(_host: string, revisionId: string) {
    ...
    const complete = this.completes.get(revisionId); if (!complete) throw new D1ActivePublishError(false)
    if (this.active?.revisionId === revisionId && this.active.desiredStateDigest === complete.desiredStateDigest) return this.active
    if (this.active && Number(revisionId.slice(1)) <= Number(this.active.revisionId.slice(1))) throw new D1ActivePublishError(false)
    if (this.fault === 'publish-before') throw new D1ActivePublishError(false)
    ...
    if (this.fault === 'publish-after') throw new D1ActivePublishError(true)
  ...
  async appendAudit(_host: string, raw: D1AuditRecordV1) {
    const record = canonicalizeD1AuditRecord(raw); this.calls.push(`audit:${record.outcome}:${record.phase}`)
    ...
    if ('completionDigest' in record && (!complete || complete.desiredStateDigest !== record.desiredStateDigest || complete.completion.completionDigest !== record.completionDigest)) throw new D1HostError(D1HostErrorCode.PUBLICATION_FAILED, { field: 'audit' })
    if ((record.outcome === 'COMPLETE' || record.outcome === 'RECOVERY_REQUIRED') && (!this.active || this.active.revisionId !== record.revisionId || this.active.desiredStateDigest !== record.desiredStateDigest)) throw new D1HostError(D1HostErrorCode.PUBLICATION_FAILED, { field: 'audit' })
    this.audits.push(record)
  }
  async hasTerminalAudit(_host: string, active: D1ActiveEnvelopeV1) {
    ...
    return this.audits.some((record) => isD1TerminalAuditFor(record, active, complete.completion))
  }
}

function harness(
  store: FakeStore,
  next: D1DesiredSnapshotV1,
...
describe('D1 command engine', () => {
  ...
  it('requires duplicate-free exact sorted removal confirmations and blocks admitted removals', async () => {
    ...
    for (const confirmRemove of [undefined, ['travel', 'extra'], ['travel', 'travel']]) {
      ...
      await expect(h.engine.execute(apply(retained, 'r0000000001', { confirmRemove }))).rejects.toMatchObject({ code: D1HostErrorCode.DESTRUCTIVE_CONFIRMATION_REQUIRED })
  ...
  it('rejects rollback removal gates before reproduce or runtime inspection', async () => {
    ...
    for (const confirmRemove of [undefined, ['travel', 'extra'], ['travel', 'travel']]) {
      ...
      await expect(h.engine.execute({ kind: 'rollback', hostId: 'host-1', expectedHostRevision: 'r0000000002', targetRevision: 'r0000000001', confirmRemove })).rejects.toMatchObject({ code: D1HostErrorCode.DESTRUCTIVE_CONFIRMATION_REQUIRED })
      expect(h.calls).toEqual(['guard']); expect(store.calls).not.toContain('reserve')
    }
    const store = new FakeStore(); await store.seed(target, 'r0000000001'); await store.seed(current, 'r0000000002'); store.calls = []
    const h = harness(store, target, ['travel'])
    await expect(h.engine.execute({ kind: 'rollback', hostId: 'host-1', expectedHostRevision: 'r0000000002', targetRevision: 'r0000000001', confirmRemove: ['travel'] })).rejects.toMatchObject({ code: D1HostErrorCode.BINDING_ADMITTED })
  ...
  it.each([
    ...
    const h = harness(store, target)
    await expect(h.engine.execute({ kind: 'rollback', hostId: 'host-1', expectedHostRevision: 'r0000000002', targetRevision: 'r0000000001' })).rejects.toMatchObject({ code: D1HostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED })
  ...
  it.each([
    ...
    await expect(h.engine.execute(apply(value, 'r0000000001'))).rejects.toMatchObject({ code: D1HostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED })
  ...
  it('rejects malformed inspected binding sets before recovery or reservation', async () => {
    ...
    for (const inspected of [[], [base[0], base[0]], [...base, { ...base[0], bindingId: 'extra' }]]) {
      ...
      await expect(h.engine.execute(apply(value, 'r0000000001'))).rejects.toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY, details: { field: 'runtimeInputs' } })
  ...
  it('passes engine-owned canonical identities to preload and persists only an exact match', async () => {
    const value = await desired([{ id: 'insurance' }, { id: 'travel' }]); const store = new FakeStore(); let received: readonly D1RuntimeInputsIdentityV1[] = []
    ...
    expect(received[0]).toMatchObject({ domain: 'boring-d1-runtime-inputs:v1', digest: expect.stringMatching(/^sha256:/) })
  ...
  it('re-inspects after materialization and rejects an intervening provider rotation', async () => {
    ...
    await expect(h.engine.execute(apply(value, null))).rejects.toMatchObject({ code: D1HostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED, details: { field: 'runtimeInputs' } })
  ...
  it('rejects malformed materialized identity sets before reinspection or preload', async () => {
    ...
    for (const output of [[], [base[0], base[0]], [...base, { ...base[0], bindingId: 'extra' }]]) {
      ...
      await expect(h.engine.execute(apply(value, null))).rejects.toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY, details: { field: 'materialize' } })
  ...
  it.each([
    ...
    await expect(h.engine.execute(apply(value, null))).rejects.toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY })
    expect(store.audits.at(-1)).toMatchObject({ outcome: 'FAILED', phase }); expect(store.audits.at(-1)).not.toHaveProperty('completionDigest')
  })

  it.each([
    ['inspect', D1HostErrorCode.SECRET_UNAVAILABLE, 'secret'],
    ['materialize', D1HostErrorCode.ACTIVE_BINDING_RESTART_REQUIRED, 'runtimeInputs'],
    ['preload', D1HostErrorCode.COLLECTION_NOT_READY, 'readiness'],
  ] as const)('preserves whitelisted %s adapter failures with fixed redacted details', async (phase, code, field) => {
    const value = await desired(); const store = new FakeStore(); store.adapterErrors[phase] = new D1HostError(code, { field: '/secret/private' })
  ...
  it.each([
    ...
    const value = await desired(); const store = new FakeStore(); store.adapterErrors[phase] = new D1HostError(D1HostErrorCode.PLAN_INVALID, { field: '/secret/private' })
    const error = await harness(store, value).engine.execute(apply(value, null)).catch((caught) => caught)
    expect(error).toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY, details: { field } }); expect(JSON.stringify(error)).not.toMatch(/secret\/private/)
  })
  ...
  it.each([
    ...
    const error = await harness(store, value).engine.execute(apply(value, null)).catch((caught) => caught)
    expect(error).toMatchObject({ code: D1HostErrorCode.COLLECTION_NOT_READY, details: { field } }); expect(JSON.stringify(error)).not.toMatch(/secret\/private|adapter failure/)
  })
  ...
  it.each([
    ...
    const value = await desired(); const store = new FakeStore(); store.fault = fault; const h = harness(store, value)
    await expect(h.engine.execute(apply(value, null))).rejects.toMatchObject({ code: D1HostErrorCode.PUBLICATION_FAILED })
  ...
  it.each([
    ...
    await expect(harness(store, next).engine.execute(apply(next, seeded ? 'r0000000001' : null))).rejects.toMatchObject({ code: D1HostErrorCode.PUBLICATION_FAILED })
  ...
  it('rejects rollback reproduction mismatch before reservation and redacts adapter failures', async () => {
    ...
    await expect(mismatch.engine.execute({ kind: 'rollback', hostId: 'host-1', expectedHostRevision: 'r0000000002', targetRevision: 'r0000000001' })).rejects.toMatchObject({ code: D1HostErrorCode.ROLLBACK_TARGET_INVALID })
    expect(store.calls).not.toContain('reserve')

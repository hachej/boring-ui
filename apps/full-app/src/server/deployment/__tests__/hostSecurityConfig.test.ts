...
import {
  AGENT_HOST_SECURITY_CONFIG_ERROR,
  AGENT_HOST_SECURITY_ENV_POLICY,
  createAgentHostSecurityConfig,
...
const facts = () => ({
  ...
  agentHostId: 'eu-host-1', publicationOwnerUid: 0, agentMode: 'vercel-sandbox',
...
const env = () => ({
  ...
  NODE_ENV: 'production', HOST: '0.0.0.0', PORT: '3000', BORING_AGENT_HOST_ID: 'eu-host-1', BORING_AGENT_HOST_OWNER_UID: '0',
  ...
  DATABASE_URL_FILE: '/run/boring/agent-host/host-secrets/database-url', BETTER_AUTH_SECRET_FILE: '/run/boring/agent-host/host-secrets/better-auth-secret',
  ...
  BORING_MANAGED_AGENT_MCP_ENABLED: '0', BORING_AGENT_HOST_MAX_BINDINGS: '20', BORING_AGENT_HOST_MAX_BUNDLE_BYTES: '1000000',
  BORING_AGENT_HOST_MAX_TOTAL_BUNDLE_BYTES: '10000000', BORING_AGENT_HOST_MAX_CONCURRENT_PRELOADS: '4',
})
const rejects = async (rawEnv: unknown, effective: unknown) => {
  await expect(createAgentHostSecurityConfig(rawEnv, effective)).rejects.toMatchObject({ code: AGENT_HOST_SECURITY_CONFIG_ERROR })
}
const frozen = (value: unknown): boolean => !value || typeof value !== 'object'
  || (Object.isFrozen(value) && Object.values(value).every(frozen))

describe('AgentHost host security config', () => {
  it('canonicalizes order and duplicates into one deeply frozen deterministic identity', async () => {
    const first = await createAgentHostSecurityConfig(env(), facts())
    ...
    expect(new Set(Object.values(AGENT_HOST_SECURITY_ENV_POLICY))).toEqual(new Set(['fixed-exact']))
  })

  it('changes the digest when an allowed behavior value changes coherently', async () => {
    const before = await createAgentHostSecurityConfig(env(), facts())
    const authChanged = await createAgentHostSecurityConfig({ ...env(), BETTER_AUTH_URL: 'https://login.example.test' },
      { ...facts(), betterAuthUrl: 'https://login.example.test' })
    const mcpEnabled = await createAgentHostSecurityConfig({ ...env(), BORING_MCP_PROD_ENABLED: '1' },
  ...
  it.each([
    ['host', (value: ReturnType<typeof facts>) => ({ ...value, agentHostId: 'eu-host-2' })],
  ...
  ('rejects forbidden, unknown, and secret-bearing key %s without serializing bytes', async (key) => {
    try { await createAgentHostSecurityConfig({ ...env(), [key]: CANARY }, facts()); throw new Error('accepted') }
    catch (error) {
      expect(error).toMatchObject({ code: AGENT_HOST_SECURITY_CONFIG_ERROR })
  ...
  it('fails closed on conditional managed target, malformed shapes, extras, and bounds', async () => {
    ...
    await rejects({ ...env(), BORING_AGENT_HOST_MAX_BINDINGS: '0' }, { ...facts(), collectionPolicy: { ...facts().collectionPolicy, maxBindings: 0 } })

...
import { createAgentHostAdmissionLedger, mintAttestedAgentHostDatabaseConnection, type AgentHostAdmissionLedger } from '../admissionLedger.js'
import type { AgentHostActiveCollection } from '../activeCollectionReader.js'
import { createAgentHostDestructivePublicationJournalStore, type AgentHostDestructivePublicationIdentity, type AgentHostDestructivePublicationJournalStore } from '../destructivePublicationJournal.js'
import { AgentHostErrorCode } from '../agentHostPlan.js'
import { canonicalizeAgentHostObservation, createAgentHostCompleteEnvelope, createAgentHostDesiredSnapshot, deriveAgentHostSecretRefsEnvelope, digestAgentHostDesired, type AgentHostDesiredSnapshotV1 } from '../agentHostRevisionCodec.js'
...
import { AgentHostActivePublishError, type AgentHostRevisionStore, type AgentHostStoredCompleteV1 } from '../hostRevisionStore.js'
...
async function desired(hostId: string, bindingIds: readonly string[], databaseRef: string): Promise<AgentHostDesiredSnapshotV1> {
  ...
  return createAgentHostDesiredSnapshot({ schemaVersion: 1, hostId, expectedHostRevision: null, hostAppImageDigest: digest('a'), runtimeProfileRef: 'runsc-eu', databaseRef, workspaceRootPolicyRef: 'workspace-roots', sessionRootPolicyRef: 'session-roots', bindings: planBindings }, resolvedBindings)
}
async function stored(hostId: string, revisionId: string, bindings: readonly string[], databaseRef = 'postgres-eu'): Promise<AgentHostStoredCompleteV1> {
  const value = await desired(hostId, bindings, databaseRef)
  const observation = await canonicalizeAgentHostObservation({
    schemaVersion: 1, domain: 'boring-agent-host-observed:v1', bindings: await Promise.all(value.resolvedBindings.map(async (binding) => {
      const planned = value.plan.bindings.find((entry) => entry.bindingId === binding.bindingId)!
      return { bindingId: binding.bindingId, ready: true, resolvedDigest: binding.resolvedDigest, runtimeInputs: await createAgentHostRuntimeInputsIdentity(planned, {
  ...
  return Object.freeze({ revisionId, desired: value, desiredStateDigest, secretRefs: deriveAgentHostSecretRefsEnvelope(value), observation, completion: await createAgentHostCompleteEnvelope(revisionId, value, observation) })
}
class Revisions {
  readonly completes = new Map<string, AgentHostStoredCompleteV1>(); events: string[] = []; publishCount = 0
  ...
  constructor(readonly hostId: string, completes: readonly AgentHostStoredCompleteV1[]) {
  ...
  store = {
    ...
    publishActive: async (_hostId: string, revisionId: string) => {
      ...
      if (fault === 'before') throw new AgentHostActivePublishError(false)
      ...
      if (fault === 'after') throw new AgentHostActivePublishError(true)
      return this.active
    },
  } as unknown as AgentHostRevisionStore
}
function identity(revisions: Revisions, overrides: Partial<AgentHostDestructivePublicationIdentity> = {}): AgentHostDestructivePublicationIdentity {
...
async function harness(onclose?: (connectionId: number) => void, max = 8) {
  ...
  const ledger = createAgentHostAdmissionLedger(mintAttestedAgentHostDatabaseConnection('postgres-eu', client, { ownsClient: true }))
  const revisions = new Revisions(hostId, await Promise.all([stored(hostId, 'r0000000001', ['alpha', 'bravo', 'zulu']),
    stored(hostId, 'r0000000002', ['bravo']), stored(hostId, 'r0000000003', ['alpha'])])); const journal = createAgentHostDestructivePublicationJournalStore()
  return { hostId, client, ledger, revisions, journal, close: () => ledger.close() }
}
const reader = (revisions: Revisions): { read(): Promise<AgentHostActiveCollection | null> } => ({
  async read() {
...
async function operation(journal: AgentHostDestructivePublicationJournalStore, value: AgentHostDestructivePublicationIdentity) {
  const sql = await admin.reserve(); try { return await journal.readOperation(sql, value.operationId) } finally { sql.release() }
}
async function prepare(journal: AgentHostDestructivePublicationJournalStore, value: AgentHostDestructivePublicationIdentity) {
...
afterAll(async () => {
  if (!admin) return
  await admin`DELETE FROM agent_host_binding_admissions WHERE host_id LIKE ${`fenced-${RUN}-%`}`
  await admin.end()
})

describe('AgentHost fenced destructive publication', () => {
  it('orders fenced checks, committed prepare, transaction-free publication, committed terminal, then unlock', async () => {
    ...
    const ledger = { ...h.ledger, withBindingFences: async <T>(keys: Parameters<AgentHostAdmissionLedger['withBindingFences']>[0], run: (sql: postgres.ReservedSql) => Promise<T>) =>
      h.ledger.withBindingFences(keys, async (sql) => {
        ...
        const observed = new Proxy(sql, { apply(target, thisArg, args: [TemplateStringsArray, ...unknown[]]) {
          if (args[0].join('').includes('agent_host_binding_admissions')) events.push('admission-clear')
          return Reflect.apply(target, thisArg, args)
        } }) as postgres.ReservedSql
        try { return await run(observed) } finally { events.push('unlocking') }
      }) } as AgentHostAdmissionLedger
    const journal = {
      ...h.journal,
      appendPrepared: async (sql: postgres.ReservedSql, value: AgentHostDestructivePublicationIdentity) => { const result = await h.journal.appendPrepared(sql, value); events.push('prepared'); return result },
      appendTerminal: async (sql: postgres.ReservedSql, value: AgentHostDestructivePublicationIdentity, state: 'committed' | 'aborted') => { const result = await h.journal.appendTerminal(sql, value, state); events.push(`terminal:${state}`); return result },
    }
    ...
    await createAgentHostFencedDestructivePublication({ admissionLedger: ledger, journalStore: journal, revisionStore: h.revisions.store, publicationControl }).publish(value)
  ...
  it('blocks an admitted removal before journal or pointer effects', async () => {
    ...
    await expect(createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore: h.revisions.store }).publish(value))
      .rejects.toMatchObject({ code: AgentHostErrorCode.BINDING_ADMITTED, details: { bindingId: 'zulu' } })
  ...
  it('makes first- and last-key first-admission races lose after removal publication', async () => {
    for (const bindingId of ['alpha', 'zulu']) {
      ...
      const value = identity(h.revisions); const publisher = createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore: h.revisions.store })
      ...
      await published
      expect(await admitted).toMatchObject({ code: AgentHostErrorCode.ADMISSION_RECORD_FAILED })
  ...
  it('serializes overlapping removal sets without deadlock or stale-pointer publication', async () => {
    ...
    const publisher = createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore: h.revisions.store })
    ...
    await expect(second).rejects.toMatchObject({ code: AgentHostErrorCode.REVISION_CONFLICT })
    expect(h.revisions.active.revisionId).toBe('r0000000002'); await h.close()
  })
  ...
  it('leaves no false terminal across prepare, publication, terminal, and connection failures', async () => {
    for (const fault of ['prepare', 'prepare-rollback', 'prepare-commit', 'publish-before', 'publish-after', 'terminal', 'connection'] as const) {
      ...
      const ledger = { ...h.ledger, withBindingFences: <T>(keys: Parameters<AgentHostAdmissionLedger['withBindingFences']>[0], run: (sql: postgres.ReservedSql) => Promise<T>) =>
        h.ledger.withBindingFences(keys, async (sql) => {
          reserved = sql
      ...
      } as AgentHostDestructivePublicationJournalStore
      h.revisions.onPublish = async () => {
      ...
      const error = await createAgentHostFencedDestructivePublication({ admissionLedger: ledger, journalStore: journal, revisionStore: h.revisions.store }).publish(value).catch((caught) => caught)
      expect(error).toMatchObject({ code: AgentHostErrorCode.ROLLBACK_JOURNAL_FAILED, details: { field: 'rollbackJournal' } })
      ...
      if (fault === 'prepare-rollback' || fault === 'prepare-commit' || fault === 'connection') {
        expect(internalError).toMatchObject({ code: 'CONNECTION_CLOSED', message: 'AGENT_HOST_RESERVED_CONNECTION_LOST' })
      }
  ...
  it('rejects artifact and database drift before prepare', async () => {
    for (const fault of ['expected-digest', 'target-digest', 'desired-content', 'observation-content', 'secret-refs', 'expected-completion-revision', 'target-completion-revision', 'missing-target', 'incomplete-target', 'incomplete-digest', 'expected-database-ref', 'target-database-ref'] as const) {
      ...
      if (fault === 'incomplete-target') {
        const target = h.revisions.completes.get('r0000000002')!
        h.revisions.completes.set('r0000000002', { ...target, completion: undefined } as unknown as AgentHostStoredCompleteV1)
      }
      if (fault === 'incomplete-digest') {
        const target = h.revisions.completes.get('r0000000002')!
        h.revisions.completes.set('r0000000002', { ...target, completion: { ...target.completion, completionDigest: undefined } } as unknown as AgentHostStoredCompleteV1)
      }
      ...
      await expect(createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore: h.revisions.store }).publish(value)).rejects.toMatchObject({
        code: AgentHostErrorCode.ROLLBACK_TARGET_INVALID,
      })
      expect(await operation(h.journal, value)).toBeNull(); expect(h.revisions.active.revisionId).toBe('r0000000001'); await h.close()
    }
    const h = await harness(); const value = identity(h.revisions)
    const revisionStore = { ...h.revisions.store, readComplete: async () => { throw new Error('private read failure') } } as AgentHostRevisionStore
    const caught = await createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore }).publish(value).catch((error) => error)
    expect(caught).toMatchObject({ code: AgentHostErrorCode.ROLLBACK_JOURNAL_FAILED }); expect(JSON.stringify(caught)).not.toMatch(/private/)
  ...
  it('recovers a committed prepare under exact fences with no SQL transaction open during publication', async () => {
    ...
    const ledger = { ...h.ledger, withBindingFences: <T>(keys: Parameters<AgentHostAdmissionLedger['withBindingFences']>[0], run: (sql: postgres.ReservedSql) => Promise<T>) =>
      h.ledger.withBindingFences(keys, async (sql) => { reserved = sql; return run(sql) }) } as AgentHostAdmissionLedger
    h.revisions.onPublish = async () => {
      const [state] = await reserved!<{ assigned: boolean }[]>`SELECT txid_current_if_assigned() IS NOT NULL AS assigned`
      transactionOpen = state!.assigned
    }
    await createAgentHostFencedDestructivePublication({ admissionLedger: ledger, journalStore: h.journal, revisionStore: h.revisions.store }).recoverPending(h.hostId)
  ...
  it('preserves the client close callback, bounds a killed publisher, and lets recovery converge', async () => {
    ...
    const publishingLedger = { ...h.ledger,
      withBindingFences: <T>(keys: Parameters<AgentHostAdmissionLedger['withBindingFences']>[0], run: (sql: postgres.ReservedSql) => Promise<T>) =>
        h.ledger.withBindingFences(keys, async (sql) => { reserved = sql; return run(sql) }),
    } as AgentHostAdmissionLedger
    const recoveryLedger = createAgentHostAdmissionLedger(mintAttestedAgentHostDatabaseConnection('postgres-eu', postgres(DATABASE_URL), { ownsClient: true }))
    h.revisions.onPublish = async () => {
      const [backend] = await reserved!<{ pid: number }[]>`SELECT pg_backend_pid() AS pid`
      backendPid = backend!.pid; entered.resolve(); await release.promise
    }
    const publisher = createAgentHostFencedDestructivePublication({ admissionLedger: publishingLedger, journalStore: h.journal, revisionStore: h.revisions.store }); const publishing = publisher.publish(value).catch((error) => error); await entered.promise
    ...
    expect(publishError).toMatchObject({ code: AgentHostErrorCode.ROLLBACK_JOURNAL_FAILED }); expect(admissionError).toMatchObject({ code: AgentHostErrorCode.ADMISSION_RECORD_FAILED }); expect(closeCount).toBeGreaterThan(0)
    expect(await admin`SELECT 1 FROM agent_host_binding_admissions WHERE host_id = ${h.hostId} AND binding_id = 'alpha'`).toHaveLength(0)
    ...
    await createAgentHostFencedDestructivePublication({ admissionLedger: recoveryLedger, journalStore: h.journal, revisionStore: h.revisions.store }).recoverPending(h.hostId)
    expect((await operation(h.journal, value))?.terminal?.state).toBe('committed')
  ...
  it('finalizes an already-published target without republishing', async () => {
    ...
    await createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore: h.revisions.store }).recoverPending(h.hostId)
  ...
  it('rejects admission behind a durable prepare and aborts recovery for an already-persisted admission', async () => {
    ...
    const publisher = createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore: h.revisions.store })
    await expect(publisher.publish(value)).rejects.toMatchObject({ code: AgentHostErrorCode.ROLLBACK_JOURNAL_FAILED })
    h.revisions.onPublish = undefined
    await expect(h.ledger.admit(reader(h.revisions), target(h.hostId, 'alpha'))).rejects.toMatchObject({ code: AgentHostErrorCode.ADMISSION_RECORD_FAILED })
    await admin`INSERT INTO agent_host_binding_admissions (host_id, binding_id, active_revision) VALUES (${h.hostId}, 'alpha', ${value.expectedRevision})`
  ...
  it('processes stale discovery in journal sequence, skips new terminals, and requests sorted operation fences', async () => {
    ...
    await createAgentHostFencedDestructivePublication({ admissionLedger: ledger, journalStore: journal, revisionStore: h.revisions.store }).recoverPending(h.hostId)
  ...
  it('fails closed for artifact, history, and active-pointer drift without fabricating a terminal', async () => {
    for (const fault of ['wrapper', 'wrapper-incomplete', 'embedded', 'digest', 'foreign-host', 'foreign-database', 'missing', 'incomplete', 'bindings', 'third-pointer', 'history'] as const) {
      ...
      if (fault === 'wrapper-incomplete') h.revisions.completes.set(value.targetRevision, { ...targetRevision, observation: undefined } as unknown as AgentHostStoredCompleteV1)
      if (fault === 'embedded') h.revisions.completes.set(value.targetRevision, { ...targetRevision, completion: { ...targetRevision.completion, status: 'BROKEN' } } as unknown as AgentHostStoredCompleteV1)
      ...
      } as unknown as AgentHostStoredCompleteV1)
      ...
      const caught = await createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: journal, revisionStore: h.revisions.store }).recoverPending(h.hostId).catch((error) => error)
      expect(caught).toMatchObject({ code: AgentHostErrorCode.ROLLBACK_JOURNAL_FAILED })
  ...
  it('keeps publication and terminal ambiguity retryable until recovery converges', async () => {
    for (const fault of ['publication-before', 'publication-after', 'terminal', 'lost-terminal-response'] as const) {
      ...
      const ledger = fault === 'lost-terminal-response' ? { ...h.ledger,
        withBindingFences: <T>(keys: Parameters<AgentHostAdmissionLedger['withBindingFences']>[0], run: (sql: postgres.ReservedSql) => Promise<T>) => h.ledger.withBindingFences(keys, (sql) => run(new Proxy(sql, {
      ...
      } as AgentHostAdmissionLedger : h.ledger
      const recovering = createAgentHostFencedDestructivePublication({ admissionLedger: ledger, journalStore: journal, revisionStore: h.revisions.store })
      await expect(recovering.recoverPending(h.hostId)).rejects.toMatchObject({ code: AgentHostErrorCode.ROLLBACK_JOURNAL_FAILED })
      h.revisions.onPublish = undefined
      const retryLedger = fault === 'lost-terminal-response'
        ? createAgentHostAdmissionLedger(mintAttestedAgentHostDatabaseConnection('postgres-eu', postgres(DATABASE_URL), { ownsClient: true })) : h.ledger
      await createAgentHostFencedDestructivePublication({ admissionLedger: retryLedger, journalStore: h.journal, revisionStore: h.revisions.store }).recoverPending(h.hostId)
  ...
  it('makes first- and last-key admissions lose against recovered removal fences', async () => {
    for (const bindingId of ['alpha', 'zulu']) {
      ...
      const recovering = createAgentHostFencedDestructivePublication({ admissionLedger: h.ledger, journalStore: h.journal, revisionStore: h.revisions.store }).recoverPending(h.hostId)
      ...
      await recovering; expect(await admission).toMatchObject({ code: AgentHostErrorCode.ADMISSION_RECORD_FAILED })
      expect((await operation(h.journal, value))?.terminal?.state).toBe('committed'); await h.close()
    }
  })
})

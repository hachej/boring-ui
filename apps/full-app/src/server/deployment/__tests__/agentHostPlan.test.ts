...
describe('parseAgentHostPlan', () => {
  it('strictly validates and freezes a canonical lexical plan', () => {
    const parsed = parseAgentHostPlan(plan({ bindings: [
      binding('z'),
  ...
  it.each([
    ...
    try {
      parseAgentHostPlan(input)
      throw new Error('expected plan rejection')
    } catch (error) {
      expect(error).toMatchObject({ code: AgentHostErrorCode.PLAN_INVALID, details: { field } })
  ...
  it.each(['192.168.1.1', '127.1', '127.0.0.01', '0127.0.0.1', '0x7f.0.0.1'])(
    ...
    (hostname) => {
      expect(() => parseAgentHostPlan(plan({ bindings: [binding('a', { hostname })] })))
  ...
  it('admits only binding ids whose .env filename fits NAME_MAX', () => {
    ...
    expect(parseAgentHostPlan(plan({ bindings: [{ ...binding('a'), bindingId: longest }] })).bindings[0].bindingId).toBe(longest)
    expect(() => parseAgentHostPlan(plan({ bindings: [{ ...binding('a'), bindingId: `${longest}a` }] })))
      .toThrow(expect.objectContaining({ code: AgentHostErrorCode.PLAN_INVALID, details: { field: 'bindings[0].bindingId' } }))
  })

  it('admits only host ids that fit one filesystem path component', () => {
    expect(parseAgentHostPlan(plan({ hostId: 'a'.repeat(250) })).hostId).toBe('a'.repeat(250))
    expect(() => parseAgentHostPlan(plan({ hostId: 'a'.repeat(251) })))
      .toThrow(expect.objectContaining({ code: AgentHostErrorCode.PLAN_INVALID, details: { field: 'hostId' } }))
  })
})

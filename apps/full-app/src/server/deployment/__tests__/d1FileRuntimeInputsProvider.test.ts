...
function binding(secretRefs: readonly string[] = ['credential-ref', 'secondary-ref']): D1SiteBindingV1 {
  return {
...
function manifest(site: D1SiteBindingV1, valueGeneration = GENERATION_A, secretFingerprint = sha('4')) {
  return {
    schemaVersion: 1, domain: 'boring-d1-file-runtime-inputs:v1', hostId: 'host-1', bindingId: site.bindingId,
...
async function fixture(secretRefs: readonly string[] = ['credential-ref', 'secondary-ref'], parent = TMPFS_PARENT, includeValues = true): Promise<Fixture> {
  const base = await mkdtemp(path.join(parent, 'boring-d1-file-provider-')); await chmod(base, 0o700)
...
function provider(input: Fixture, ownerUid = OWNER_UID) {
  return createD1FileRuntimeInputsProvider({ hostId: 'host-1', ownerUid, metadataRoot: input.metadataRoot, valueRoot: input.valueRoot })
}
...
async function unavailable(operation: Promise<unknown>, input: Fixture): Promise<void> {
  ...
  expect(error).toMatchObject({ code: D1HostErrorCode.SECRET_UNAVAILABLE, details: { field: 'secret' } })
  expect(JSON.stringify(error)).not.toContain(input.base)
  expect(JSON.stringify(error)).not.toMatch(/credential-ref|canary:/)
}

describe('D1 file runtime-input provider', () => {
  ...
  it('does not touch the value root during inspect or for a zero-secret binding', async () => {
    ...
    const inspected = await createD1FileRuntimeInputsProvider({
  ...
  it('rejects unsafe metadata roots, files, owner policy, and links', async () => {
    ...
    await unavailable(createD1FileRuntimeInputsProvider({ hostId: 'host-1', ownerUid: OWNER_UID, metadataRoot: alias, valueRoot: linkedRoot.valueRoot }).inspect(linkedRoot.binding), linkedRoot)
    expect(() => provider(linkedRoot, OWNER_UID + 1)).toThrow(expect.objectContaining({ code: D1HostErrorCode.SECRET_UNAVAILABLE }))
  })
  ...
  it('keeps descendant reads anchored when the validated metadata root is replaced', async () => {
    ...
    const source = createD1FileRuntimeInputsProvider({

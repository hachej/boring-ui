...
import { captureD1CoreProofRevision, createD1CoreProofBindingIdDigest, createD1CoreProofOperationIdDigest, verifyD1CoreProof, type D1CoreProofBindingV1, type D1CoreProofRevisionV1 } from '../d1CoreProof.js'
import { captureD1DrFingerprint } from '../d1DrProof.js'
...
const bindingDigests = new Map(await Promise.all(['alpha', 'bravo', 'charlie', 'delta'].map(async (id) => [id, await createD1CoreProofBindingIdDigest(id)] as const)))
const rollbackOperationIdDigest = await createD1CoreProofOperationIdDigest('rollback-delta')
const limits = {
  maxBindings: 4,
  maxBundleBytes: 10,
...
async function desired(ids: readonly string[]): Promise<D1DesiredSnapshotV1> {
...
function plan(value: D1DesiredSnapshotV1): D1HostPlanV1 {
  return { ...value.plan, expectedHostRevision: null }
}
async function candidate(value: D1DesiredSnapshotV1, revisionId: string): Promise<D1StoredCandidateV1> {
  return {
    revisionId,
    desired: value,
    desiredStateDigest: await digestD1Desired(value),
    secretRefs: deriveD1SecretRefsEnvelope(value),
  }
}
async function inputs(value: D1DesiredSnapshotV1) {
  return Promise.all(
    value.plan.bindings.map((binding) =>
      createD1RuntimeInputsIdentity(binding, {
...
function handle(binding: D1ResolvedBindingV1): D1PreparedBindingHandle {
...
function proofBinding(id: string, value: string): D1CoreProofBindingV1 {
  return Object.freeze({
    bindingId: id,
...
function snapshot(revisionId: string, state: string, bindings: readonly D1CoreProofBindingV1[]): D1CoreProofRevisionV1 {
  return Object.freeze({
    revisionId,
...
function authorizationEvidence(bindingId: string, crossBindingId: string) {
  return {
    ...
    crossBinding: { outcome: 'denied', code: D1HostErrorCode.HOST_SCOPE_VIOLATION, effectsBefore: 1, effectsAfter: 1, admissionsBefore: 1, admissionsAfter: 1 },
  }
}
function evidence() {
  return {
    schemaVersion: 1,
    domain: 'boring-d1-core-proof:v1',
...
describe('D1-006 core proof', () => {
  it('captures stable-process three -> four -> exact rollback through the served collection seam', async () => {
    ...
    const controller = createD1CollectionController({
      limits,
    ...
    const capturedInitial = await captureD1CoreProofRevision(controller)
    ...
    const capturedAdd = await captureD1CoreProofRevision(controller)
    ...
    const capturedRollback = await captureD1CoreProofRevision(controller)
    ...
    const makeRoots = async () => {
      const root = await mkdtemp(join(tmpdir(), 'd1-core-dr-proof-'))
    ...
    const captureDr = () => captureD1DrFingerprint({ reader: controller, revisionReader, hostId: 'eu-host', ...restoredRoots, readRows: rows })
    ...
    expect(JSON.stringify(restoredDr)).not.toMatch(/d1-core-dr-proof|canary-secret|session\.jsonl/)
    await expect(captureD1DrFingerprint({ reader: controller,
      revisionReader: { readComplete: async (revisionId: string) => revisionId === 'r0000000002' ? null : revisionReader.readComplete(revisionId) },
      hostId: 'eu-host', ...restoredRoots, readRows: rows })).rejects.toMatchObject({ code: D1HostErrorCode.PROOF_INVALID })
    await expect(captureD1DrFingerprint({ reader: controller, revisionReader, hostId: 'eu-host', ...restoredRoots,
      sessionRoot: restoredRoots.workspaceRoot, readRows: rows })).rejects.toMatchObject({ code: D1HostErrorCode.PROOF_INVALID })
  ...
  it('accepts complete evidence even when the measured target is missed', async () => {
    expect(await verifyD1CoreProof(evidence(), isolationEvidence())).toEqual(
  ...
  it('allows bindings to reuse content-addressed definition and runtime assets', async () => {
    ...
    const shared = (revision: D1CoreProofRevisionV1): D1CoreProofRevisionV1 => ({ ...revision,
    ...
    await expect(verifyD1CoreProof(value, isolationEvidence())).resolves.toMatchObject({ status: 'pass' })
  })
  ...
  it.each([
    ...
    const value = structuredClone(evidence())
    mutate(value)
    await expect(verifyD1CoreProof(value, isolationEvidence())).rejects.toMatchObject({
        code: D1HostErrorCode.PROOF_INVALID,
        details: { field: 'proof' },
    })
  })
  ...
  it('rejects isolation evidence whose canonical digest does not verify', async () => {
    ...
    await expect(verifyD1CoreProof(evidence(), hostile)).rejects.toMatchObject({ code: D1HostErrorCode.PROOF_INVALID })
  })
  ...
  it('never echoes a secret value or raw path in proof errors', async () => {
    ...
    const error = await (async () => {
      try {
        await verifyD1CoreProof(hostile, isolationEvidence())
      } catch (caught) {
        return caught
      }
    })()
    ...
    expect(JSON.stringify(error)).toBe('{"code":"D1_PROOF_INVALID","details":{"field":"proof"},"name":"D1HostError"}')

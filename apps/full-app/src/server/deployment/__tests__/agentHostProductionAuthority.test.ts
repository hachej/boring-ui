...
import type { AgentHostCollectionController } from '../bootCollection.js'
import type { AgentHostPendingPublicationV1 } from '../agentHostPublicationControl.js'
import { createAgentHostProductionAuthority } from '../agentHostProductionAuthority.js'
...
const binding: AgentHostSiteBindingV1 = { bindingId: 'one', hostname: 'one.example.test', workspaceId: 'workspace-one', defaultDeploymentId: 'deployment-one',
...
let input: Awaited<ReturnType<typeof createAgentHostRuntimeInputsIdentity>>
...
const candidate = { revisionId: targetActive.revisionId, desired, desiredStateDigest: targetActive.desiredStateDigest, secretRefs: {} } as unknown as AgentHostStoredCandidateV1
const complete = (active: typeof oldActive | typeof targetActive): AgentHostStoredCompleteV1 => ({ ...candidate, revisionId: active.revisionId,
  desiredStateDigest: active.desiredStateDigest, observation: { bindings: [{ runtimeInputs: input }] }, completion: {} } as never)

beforeEach(async () => { input = await createAgentHostRuntimeInputsIdentity(binding, { environment: { versionFingerprint: digest('1') },
...
function harness(start: typeof oldActive | typeof targetActive | null, initiallyServed: typeof oldActive | typeof targetActive | null = null) {
  let durable = start; let served = initiallyServed; let pending: AgentHostPendingPublicationV1 | null = { schemaVersion: 1, operationId: 'operation-1',
  ...
  const controller = { resolver: { resolvePlan: vi.fn(async () => desired), reproduce }, preload, serve,
    ...
    settleRetirement: vi.fn(), discardPrepared } as unknown as AgentHostCollectionController
  ...
  const authority = createAgentHostProductionAuthority({ hostId: 'host-1', ownerUid: 0, dependencies: { store, servedCollection: controller,
  ...
  return { authority, preload, serve, reproduce, discardPrepared, setDurable(value: typeof oldActive | typeof targetActive | null) { durable = value },
    setPending(value: AgentHostPendingPublicationV1 | null) { pending = value }, served: () => served }
}

describe('AgentHost production publication authority', () => {
  it('prepares without changing served state, then commits only the durable target', async () => {
    ...
    await expect(h.authority.commit('operation-1')).rejects.toMatchObject({ code: AgentHostErrorCode.PUBLICATION_FAILED })
  ...
  it('recovers only an exact reproduction of the durable complete revision', async () => {
    ...
    await expect(drifted.authority.recover()).rejects.toMatchObject({ code: AgentHostErrorCode.PUBLICATION_FAILED })
  })
  ...
  it('fails closed on an unrelated durable tuple', async () => {
    ...
    await expect(harness(unrelated as never).authority.recover()).rejects.toMatchObject({ code: AgentHostErrorCode.PUBLICATION_FAILED })
  })
})

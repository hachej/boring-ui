...
import { assertContainersAbsent, parsePinnedImage, targetRunArgs, validateInspect } from '../../../../scripts/agent-host-docker-boundary-proof.js'

const digest = 'a'.repeat(64)
const image = `registry.example/full-app@sha256:${digest}`

describe('AgentHost Docker boundary proof contract', () => {
  it('requires an explicit immutable image reference', () => {
    expect(parsePinnedImage(['--image', image], {})).toBe(image)
    expect(parsePinnedImage(['--', '--image', image], {})).toBe(image)
    expect(parsePinnedImage([], { AGENT_HOST_DOCKER_PROOF_IMAGE: `sha256:${digest}` })).toBe(`sha256:${digest}`)
  ...
  it('runs the actual entrypoint with one read-only host mount and no user override', () => {
    ...
    expect(args).toContain('type=bind,src=/dev/shm/proof/host-a,dst=/run/boring/agent-host,readonly')
  ...
  it('accepts only the exact redacted host mount outside the checkout', () => {
    const record = [{ Config: { Cmd: ['node'], Env: [], Labels: {} }, Mounts: [{ Type: 'bind', Source: '/dev/shm/proof/host-a', Destination: '/run/boring/agent-host', RW: false }] }]

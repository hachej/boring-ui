...
const binding: AgentHostSiteBindingV1 = { bindingId: 'insurance', hostname: 'insurance.example.test', workspaceId: 'workspace:insurance',
...
async function artifact(overrides: Record<string, unknown> = {}) {
  ...
  return { schemaVersion: 1, domain: 'boring-agent-host-agent-artifact:v1', hostId: 'host-1', bindingId: binding.bindingId,
...
async function inbox(value?: Awaited<ReturnType<typeof artifact>>) {
  ...
  const parent = await mkdtemp(path.join(os.tmpdir(), 'agent-host-agent-artifacts-')); const root = path.join(parent, 'artifacts')
...
const load = (root: string, extra: Record<string, unknown> = {}) => loadAgentHostAgentArtifactInputs({
  hostId: 'host-1', ownerUid: process.geteuid!(), root, limits, inputs: [{ binding, compositionDigest: digest('c') }], ...extra,
})

describe('AgentHost immutable agent artifact inbox', () => {
  ...
  it('accepts the injected byte cap exactly and rejects cap plus one', async () => {
    ...
    await expect(load(h.root, { limits: { ...limits, maxBundleBytes: size - 1 } })).rejects.toMatchObject({ code: AgentHostErrorCode.PUBLICATION_FAILED })
  })
  ...
  it('rejects filesystem drift, changed reads, oversize input, and tampered identities', async () => {
    ...
    for (const mutate of cases) {
      ...
      await expect(load(current.root, extra ?? {})).rejects.toMatchObject({ code: AgentHostErrorCode.PUBLICATION_FAILED })
    }
    for (const overrides of [{ bundleRef: '../bundle' }, { workspaceAllocationRef: 'stale' }, { workspaceCompositionDigest: digest('d') }]) {
      await expect(load((await inbox(await artifact(overrides))).root)).rejects.toMatchObject({ code: AgentHostErrorCode.PUBLICATION_FAILED })
    }
  ...
  it('rejects an input tree owned by a different uid than the command policy', async () => {
    ...
    try { await expect(load(h.root, { ownerUid: policyUid })).rejects.toMatchObject({ code: AgentHostErrorCode.PUBLICATION_FAILED }) }

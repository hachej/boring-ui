export function assertProductionAgentModeIsSafe(env: NodeJS.ProcessEnv = process.env) {
  ...
  if (mode !== 'vercel-sandbox' && mode !== 'blaxel') {
    throw new Error(
      `BORING_AGENT_MODE=${mode ?? '<unset>'} is not allowed in production full-app. ` +
        'Set BORING_AGENT_MODE=vercel-sandbox or blaxel, or set BORING_ALLOW_UNSAFE_AGENT_MODE=1 only for an explicitly approved deployment.',
    )
  }
}

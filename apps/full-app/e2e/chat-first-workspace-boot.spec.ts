...
test('authenticated workspace route renders chat before tree/session warmup resolves', async ({ page, baseURL }) => {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (path === '/api/v1/tree' || path === '/api/v1/agents/default/sessions') {
      delayed.push(path)
      return new Promise<void>(() => {})
    }
    if (path === '/api/v1/agents/default/ready-status') return route.fulfill({ status: 200, contentType: 'text/event-stream', body: 'event: status\ndata: {"state":"ready","sandboxReady":true,"harnessReady":true}\n\n' })
  ...
  expect(delayed).toEqual(expect.arrayContaining(['/api/v1/tree', '/api/v1/agents/default/sessions']))
})

...
test('authenticated workspace can submit chat while runtime dependencies are still preparing', async ({ page, baseURL }) => {
  let chatSubmitted: { sessionId?: string; message?: string } | null = null
  await page.route('**/*', async (route) => {
    ...
    if (path === '/api/v1/agent/sessions' && request.method() === 'GET') return route.fulfill(json([]))
    ...
    if (path === '/api/v1/agent/chat' && request.method() === 'POST') {
      chatSubmitted = request.postDataJSON() as { sessionId?: string; message?: string }
      return route.fulfill({
        status: 200,
        contentType: 'text/event-stream',
        body: 'event: done\ndata: {}\n\n',

...
const COMMAND_PALETTE_CHECKPOINTS = new Set(["closed", "open", "commands"])
...
export async function validateUiReviewManifest(root: string, manifest: UiReviewManifest): Promise<void> {
  ...
  if (manifest.scenarioId !== "command-palette") throw new Error("UI_REVIEW_MANIFEST_SCENARIO_INVALID")
  if (manifest.rubricVersion !== UI_REVIEW_RUBRIC_VERSION) throw new Error("UI_REVIEW_MANIFEST_RUBRIC_INVALID")

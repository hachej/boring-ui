...
export const UI_REVIEW_OWNER_SPOT_CHECKS = [
...
export async function createExecutionPacket(input: {
  ...
  if (input.manifest.scenarioId !== "command-palette") throw new Error("UI_IMPROVEMENT_SCENARIO_INVALID")
  ...
  validateCommandPaletteHardGateReport(input.hardGates, input.manifest)
  validateUiCriticReport(input.critic, input.manifest)
  validateCalibrationRecord(input.calibration)
  ...
  const packet: UiExecutionPacketV1 = {
    ...
    handoff: {
      channel: "existing-inbox-ask_user",
      reportPath: "report.html",
      ownerSpotChecks: [...UI_REVIEW_OWNER_SPOT_CHECKS],
    },
  }
  validateExecutionPacket(packet, input.manifest)
...
export async function validateUiImprovementRun(input: {
  ...
  await validateUiReviewManifest(input.root, manifest)
  ...
  validateExecutionPacket(packet, manifest)
  await validateExecutionPacketEvidence(input.root, packet)
...
export function validateCalibrationRecord(raw: unknown): asserts raw is UiCalibrationRecordV1 {
  ...
  if (raw.schemaVersion !== 1 || raw.scenarioId !== "command-palette" || !["pending", "approved", "request-changes"].includes(raw.ownerDisposition)) throw new Error("UI_CALIBRATION_CONTRACT_INVALID")
...
export function validateExecutionPacket(raw: unknown, manifest: UiReviewManifest): asserts raw is UiExecutionPacketV1 {
  ...
  if (raw.schemaVersion !== 1 || raw.kind !== "ui-improvement-execution-packet" || raw.scenarioId !== "command-palette" || raw.sourceRunId !== manifest.runId) throw new Error("UI_EXECUTION_PACKET_IDENTITY_INVALID")
  ...
  if (!isRecord(raw.handoff) || Object.keys(raw.handoff).sort().join(",") !== "channel,ownerSpotChecks,reportPath" || raw.handoff.channel !== "existing-inbox-ask_user" || raw.handoff.reportPath !== "report.html" || !Array.isArray(raw.handoff.ownerSpotChecks) || raw.handoff.ownerSpotChecks.join("|") !== UI_REVIEW_OWNER_SPOT_CHECKS.join("|")) throw new Error("UI_EXECUTION_PACKET_HANDOFF_INVALID")
}

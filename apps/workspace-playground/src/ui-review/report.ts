...
import { UI_REVIEW_OWNER_SPOT_CHECKS } from "./improvement"
...
export function renderUiReviewHtml(input: {
  ...
  const spotChecks = UI_REVIEW_OWNER_SPOT_CHECKS.map((step) => `<li>${escapeHtml(step)}</li>`).join("")
...
export function renderUiReviewMarkdown(input: {
  ...
  return [
    ...
    "",
    "## Owner spot-check",
    ...UI_REVIEW_OWNER_SPOT_CHECKS.map((step, index) => `${index + 1}. ${step}`),
    "",

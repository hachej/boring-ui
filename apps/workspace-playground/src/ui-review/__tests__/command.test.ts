...
function parse(args: string[]): unknown {
  ...
  if (result.status !== 0) throw new Error(result.stderr.match(/UI_REVIEW_(?:SCENARIO|COMMAND)_INVALID[^\n]*/)?.[0] ?? result.stderr)
  return JSON.parse(result.stdout)
}

describe("UI review command", () => {
  it("accepts explicit improve and an optional local baseline", () => {
    expect(parse(["--", "improve", "command-palette", "--critic=fixture", "--baseline-dir", "/tmp/before"])).toMatchObject({
      mode: "improve",
  ...
  it.each([
    ...
    expect(() => parse(args)).toThrow(/UI_REVIEW_(?:SCENARIO|COMMAND)_INVALID/)
  })
})

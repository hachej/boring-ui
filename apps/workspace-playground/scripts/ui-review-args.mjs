export function parseUiReviewArgs(argv, env = process.env) {
  ...
  let mode = "review"
  let scenario = values.get("scenario") ?? "command-palette"
  if (positional.length > 0) {
    if (positional.length !== 2 || (positional[0] !== "review" && positional[0] !== "improve") || values.has("scenario")) {
      throw new Error("UI_REVIEW_COMMAND_INVALID:expected review|improve command-palette")
    }
    mode = positional[0]
    scenario = positional[1]
  }
  if (scenario !== "command-palette") throw new Error(`UI_REVIEW_SCENARIO_INVALID:${scenario}`)
  return {
    mode,
    scenario,

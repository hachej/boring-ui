...
test.describe("workspace-playground inbox demo", () => {
  test("mounts plugin-owned demo items in the app-left Inbox", async ({ page }) => {
    ...
    await expect(page.getByText("No artifacts attached")).toHaveCount(0)
    const firstInboxRow = page.getByRole("button", { name: /Pick the deploy target for the release smoke/ }).first()

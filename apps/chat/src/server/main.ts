...
export async function startCoreServer(port = 5320) {
  const config = await loadConfig({
    env: {
      CORE_STORES: 'local',
      MAIL_TRANSPORT_URL: 'console://',
      MAIL_FROM: 'chat@local',
      PORT: String(port),
      BETTER_AUTH_URL: `http://localhost:${port}`,
      LOG_LEVEL: 'info',
      APP_ID,
      APP_NAME: 'Boring Chat',
    },
  ...
  app.post('/auth/sign-up/email', async (_request, reply) => {
    reply.status(403)
    return { error: { message: 'Sign-up disabled in chat dev mode' } }
  })
  ...
  app.log.info({ port, defaultWorkspaceId: ws.id, devUser: DEV_USER_EMAIL }, 'chat core server ready')

  return { app, defaultWorkspaceId: ws.id }
}

if (process.argv[1] && import.meta.url.endsWith(process.argv[1].replace(/\\/g, '/'))) {
  const port = Number(process.env.CORE_PORT) || 5320

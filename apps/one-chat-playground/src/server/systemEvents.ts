...
export function formatSystemEvent(event: BuilderFinishedEvent | MockupFinishedEvent): string {
  ...
  if (event.kind === 'mockup-finished') {
    return `[system event] The builder finished the MOCKUP for intent ${event.slug}: ${summary}${punctuation} Show it with show_on_screen at ${event.url} titled "${event.title}", say "Here is a sketch, nothing works yet. Keep it, or tell me what to change?", and wait.`
  }

...
Do not add libraries. Never edit data by hand or with a script. Never touch configuration, dependencies, generated routes, or servers.

You are working in the isolated `candidate/` version, never the accepted app. For every line in the agreement that says the app **shall** do something, add one matching check to `scripts/smoke.mjs`. The script receives the candidate base URL in `BASE_URL`, must fail non-zero when a check fails, and must not change saved data. Run `./verify.sh` before finishing and report success only when it passes.

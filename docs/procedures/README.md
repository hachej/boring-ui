...
| --- | --- |
| risk-based delivery, human-review boundaries (>500 package lines), states, rollout | [`boring-loop.md`](boring-loop.md) |
| mandatory cross-package abstraction review | [`coding-invariants.md`](coding-invariants.md#cross-package-abstraction-review-hard-gate) |
...
| revision-bound proof and Playwright before/after UI video | [`proof-of-work.md`](proof-of-work.md) |
...
Use `ask_user` for protected-boundary human decisions; use a GitHub issue/PR
comment when unavailable. Routine changes take the verified automatic path once
its controls are enabled; docs adoption alone does not enable automation.
...
| `exec` | Drive one ready artifact through implementation, proof, abstraction review, and automatic delivery or protected-boundary owner handoff. |
...
`visual-review.md` owns UI scenario execution; risk/merge routing stays in
`boring-loop.md` and video evidence in `proof-of-work.md`. `local-signoff.md` is
compatibility/reference only, not independent merge authority.

...
The Orchestrator does not keep timers. It calls the `supervise` tool (start/stop/status, 30s..60m). The host persists one epic-labelled entry per session (`<state>/supervision.json`), fires ticks, skips a tick while the session is busy instead of queueing it, and on boot re-arms every persisted entry whose registry epic is still active. A crash therefore costs at most one interval. The stale-claim rule below is unchanged.
...
- **State**: `<stateRoot>/epics.json` is the runtime epic registry and `<stateRoot>/session-bindings.json` maps every Factory session to its epic. Supervision, demos, leases, and snapshots carry or key by the same epic key. Atomic writes protect all registries.
- **Recovery**: boot loads and validates the registry, re-arms supervision for every active entry, cleans expired demos, and warms each active epic snapshot. The adopt endpoint reattaches shared sessions and safely copies native transcripts from the former per-epic session roots into the hub namespace; it rejects cross-epic binding collisions and preserves the legacy source files.

...
| [`agent-mail-mcp-study.md`](agent-mail-mcp-study.md) | Study of the OSS `mcp_agent_mail` mailbox server as a model for per-job 1:1/1:n agent messaging. Verdict: third shape (mailbox), maps onto the ratified Thread + addressed posts; ideas only (license rider). | **CURRENT EVIDENCE** (2026-08-29) |
| [`job-channel-addressed-posts-proposal.md`](job-channel-addressed-posts-proposal.md) | Proposal derived from it: Tier 1 addressed posts inside the Thread (gate-free, input to the post-shape-spike engine plan); Tier 2 subscriber inboxes (gated, input to the post-P1 relay-vs-blackboard ruling). | **PROPOSAL** — not dispatchable |

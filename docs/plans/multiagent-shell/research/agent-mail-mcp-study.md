...
benefit of" them, and "use" includes analyzing/indexing. Treat as **ideas-only**; any code lift is an
...
`send_message`, yet a validator rejects `@all` with "doesn't support broadcasting", and issue #149 shows
broadcast degrading as stale agents pile up). A "thread" is a tag, not a shared object: two agents on one
...
Discovery = `list_agents`/`whois`. **Polling only** — the README's wake-up answer is the *commercial*
Companion app ("Message Stacks" fanning scheduled prompts into tmux panes). Identity = per-agent
...
| File reservations vs D25/D28 | **Wrong layer for us.** Their leases exist because agents own separate checkouts; ours share one canonical workspace with per-seat authority (D28, north-star 2026-08-25). Keep only an advisory "intends to edit X" **system marker**; enforcement, if ever, lives in Environment admission, not git hooks. |

...
**One Host-owned envelope authority, one SQLite file** (the existing `requestLedgerPath` DB). Request remains the existing accepted-work record. Turn, Effect, and Attention are three additive projections inside that same ledger authority; they are not another task store, workflow engine, or identity system. `SqliteAgentRequestLedger` grows the required projection/history tables and additive methods; `InMemoryAgentRequestLedger` mirrors them for `inMemoryRequestLedgerMode` hosts.

#### Durable nested tool execution: replay, admission, and settlement are different contracts

Durable event replay belongs to the Agent-owned session record/event store. It records model-visible tool calls/results and delivers committed events after reconnect or restart. Reading or replaying those events must never invoke a model, tool handler, or external effect. An `observe` tool needs durable call/result events but no Effect row.

Turn admission and nested effect settlement are Host-owned envelope facts. A turn uses the existing five-part `RequestKey`; **RunId is only a branded projection of that key**. A nested effect is addressed by `(RunId, effectId)`, where `effectId = toolCallId` for tool calls. This is a child coordinate, not a child Run or independently minted identifier.

For effect-bearing tools, the host commits Effect `in-flight` before calling the handler, settles the Effect before appending the model-visible tool-result event, and uses compare-and-set terminal recording. Matching outcome digests are idempotent; contradictory settlement raises a conflict alarm and preserves the first terminal record. Settlement is exactly-once recording, not a claim of exactly-once world effects. An unresolved ordinary effect becomes `outcome-unknown` and is never automatically redispatched; only a registration-declared idempotent effect may reconcile/retry under the same `(RunId, effectId)`.

Until A3 lands, ordinary tools—including disposable sandbox lifecycle calls—remain direct calls with no durable per-tool replay claim. Do not add a private tool-specific accepted-work framework as an interim substitute.
...
| **Turn** (new — Sol P0) | `RunId` (one turn per admitted prompt/follow-up) | `sessionRef`, `incarnation`, `state: started \| ended \| error \| outcome-unknown`, `startedSeq`, `endedSeq`, `startedAt`, `updatedAt` | written from the `agent-start` / `agent-end` / `error` events the host already observes for every session (`createAgentHost.ts:517-521`); reconcile: `started` from a prior incarnation → `outcome-unknown`. **A4 reconstructs activity from Turn rows only.** |
| **Effect** (new) | `(RunId, effectId)`; `effectId = toolCallId` for tool effects, deterministic operation coordinate for gateway effects | `effectClass: 'observe' \| 'propose' \| 'mutate' \| 'external-effect' \| 'pause'`, `idempotent: boolean`, `state: admitted \| in-flight \| settled \| outcome-unknown \| paused`, `outcomeDigest`, `receipt` | CAS by exact prior state; `settled` is idempotent on matching digest (D-c) |
| **Attention** (new; C5 pause record, tool-independent) | `attentionId` (= today's `questionId` for wire compat) + FK `(RunId, effectId)` | `sessionRef {agentTypeId, sessionId}`, `kind: question \| approval \| review \| notice`, `status: ready \| answered \| cancelled \| expired \| superseded` (`abandoned` kept read-only for legacy rows), `ownerPrincipalId`, `expiresAt`, `riskTier`, `answer {values, resolvedBy, resolvedAt}`, `resume: {state: pending \| resumed \| outcome-unknown, resumeRequestId, error?}`, payload (title/context/schema/artifacts) | `ready → answered/cancelled/expired/superseded` CAS; answer is idempotent on identical values (same `resolvedBy`), conflict otherwise |

Identity rules: `RunId := RequestKey` — Turn, Effect, and Attention carry the *prompt command's* request key, never a fresh run/task/execution id. `sessionRef`, tool name, effect class, and host incarnation are attributes, never identities. `resumeRequestId` is deterministic: `attention:<attentionId>:resume` so double-answer, double-boot, or two clients produce one ledger row. `abandoned` is never written by any code path after A3; it only means "owner/superseded" via the `superseded` status.

The current-state tables are CAS projections. Every admission, start, pause, settlement, rejection, and `outcome-unknown` transition is also appended to immutable envelope history in the same SQLite transaction. This satisfies R2 auditability without introducing a second ledger.

#### Effect admission and classification on tools

- Tools declare `effect?: EffectClass` on `AgentTool` (`packages/agent/src/shared/tool.ts`); unknown → `external-effect` (fail-closed: crash mid-call = `outcome-unknown`, never re-run). Read-only tools (`read/grep/find/ls/list`) declare `observe`; `ask_user` declares `pause`. `observe` writes no Effect row (free, VISION.md:45); its model-visible call/result events still append to the Agent-owned session record for replay.
...
- Boot reconciliation (`createAgentHost` after ledger open): claim new incarnation; for each non-terminal row from a prior incarnation: `pause` effect with attention `ready|answered` → keep (turn is parked); declared-idempotent → reset to `pending-admission` (re-executable on replay under the same `(RunId, effectId)`); everything else → `outcome-unknown` (D-c rule, ARCHITECTURE-PLAN.md:196-210).

#### Durable tool execution crash matrix

| Crash boundary | Durable evidence | Recovery |
|---|---|---|
| Before turn admission commit | No Run | Caller may retry the same RequestKey |
| After admission, before proven turn dispatch | Admitted Run only | Never mint another Run; dispatch only when the pre-dispatch fence proves execution never began, otherwise terminalize explicitly |
| After turn start, before Effect admission | Started Run, no child Effect | Do not automatically resume the model turn; reconcile the Run honestly |
| After Effect admission, before `in-flight` commit | Effect admitted | Handler is proven uncalled; transition under the same child coordinate, never invent a replacement Effect |
| After `in-flight`, before handler return | World effect may have occurred | `outcome-unknown`, except explicit declared-idempotent reconciliation |
| After handler return, before settlement commit | World effect may have occurred | `outcome-unknown`; never infer success from process memory |
| After settlement, before tool-result event append | Settled digest and receipt | Append/project exactly one result under `(RunId, effectId)` without invoking the handler |
| After event append, before fanout/client acknowledgement | Settled Effect and durable event | Replay only; matching settle/append retries are no-ops |
| Contradictory second settlement | Existing different digest | Conflict alarm; preserve the first terminal record |
...
3. **Unit/conformance** — `requestLedger.test.ts`: append-only transition history; reopen with new incarnation → in-flight → outcome-unknown except paused; Turn `started` → outcome-unknown; attention CAS; digest-idempotent settle; contradictory digest alarm. The bodies for `gatewayConformance.ts:675` ("replays receipts and create tombstones across restart") and `:676` (the complete crash matrix above) are written in a **`level-d` test file behind a new fixture capability `restart(): Promise<fixture>`** (same file A4 uses for `:677`); the skips at `:674-677` and the `'B' | 'D'` type widening are **P1-C's**, uniformly. Handler-spy tests prove `in-flight` commits before entry, settlement commits before tool-result append, settled-before-event recovery emits exactly one result without invoking the handler, and ordinary effect handlers are never called twice. Mutation controls move each boundary in the unsafe direction and must fail.
...
**A3a = 2 sessions** (`9p50.4`): S1 ledger extension (Turn + Effect + Attention projections and immutable transition history) + incarnation/reconcile + follow-up replay + child-effect admission + level-d case bodies; S2 lossless ledger-backed ask-user + #1384 salvage + #1348 e2e green. **A3b = 1 session** (`9p50.8`): resume via the A2 seam extension + crash matrix + hermetic real-pi continuation test + #1413 test — the least trustworthy estimate in the plan (pi behavior unverified); it bounces to a split, not a retry, if the spike shows a transcript-rewrite path. Non-goals: remote/cross-host transport and deployment of C6; Artifacts/Delivery/cost; metering durability; activity index (A4); stream store / default-on (P1-B/C); cross-process live tail; approval RBAC; channel delivery beyond request-ID keyed bridge ops; inbox UI redesign. This slice implements local C6-compatible admission and nested settlement semantics and must not fork their identity or ledger.

...
| [runtime-plugin-local-dev-and-rpc](./runtime-plugin-local-dev-and-rpc/)                    | split CLI runtime-plugin DX roadmap: health, file-backed paginated reads, links, dependency imports, deferred writes | proposal split into smaller plans |
...
| 4. **Data access / display / update + nav** (files/DBs)                | runtime-plugin-local-dev-and-rpc split plans: file-backed paginated reads first, `WorkspaceLink` nav, DB/writes deferred                                                                                        | ⏳ redesigned as smaller phases |
...
| 9. Verify / ship / deploy                                              | agent-generation, end-to-end-fix; CLI `verify-plugin`; app-setup SKILL. **Agent self-test of a pane → runtime-plugin-local-dev-and-rpc health plan** (the missing rung between `verify-plugin` and a human)      | ◐ static covered; pane self-test designed                           |
...
3. **Generic data access timing.** The local-dev-and-rpc roadmap now starts with enhancing
 the existing file endpoint via `view=records` pagination before any generic `data.v1.*`/DuckDB
 service. **Decide:** ship file-backed reads first, then reopen DB querying only when needed.
...
- Ship the local-dev-and-rpc split phases in order: health, file-backed reads, links, already-installed imports.

...
The original lifecycle source named here, `runtimeBindingLifecycle.ts`, was
private and unused and has since been removed. OB2 must attach observations to
the active agent-owned `EnvironmentLeaseManager` in
`packages/agent/src/server/agent-host/environmentLease.ts`: successful creation
maps to `loaded`, completed retirement to `not-loaded`, creation/retirement
failures to `error`, and no observation to `unknown`. These observations remain
planned OB2 work; the cleanup does not add lifecycle instrumentation.
...
- Session route/list:
  [`packages/agent/src/server/agent-host/__tests__/httpProjection.test.ts`](../../../packages/agent/src/server/agent-host/__tests__/httpProjection.test.ts).
- Admin composition:

...
| **P6-D/P6-R** (definition + resolution — **v1 gate**) | separate versioned definition/deployment schemas; immutable verified-bundle registry distinct from resolved-agent registry; deterministic definition/deployment/resolved digests; immutable session identity snapshot; requirements validated against active authority | affected agent/core/CLI tests |
...
| **D1** (dedicated delivery — **v1 gate**) | remote target verifies/materializes bundle without source checkout; fenced journal survives step crashes/concurrency; timed <=15-minute golden path; zero platform-source edits; no-op reapply; prior complete deployment rollback reproduces resolved digest; no secret output | provisioning smoke |

...
| **P7** (multi-agent/inspection) | two agents, same `sessionId`, no shared binding/transcript/catalog; `agentId` in binding scope key + `sessionNamespace`; per-agent readiness/catalogs; session search scoped by workspace+agent; scrubbed agent-list endpoint + per-agent inspection endpoint | `pnpm --filter @hachej/boring-agent run test` · `run test:e2e` |

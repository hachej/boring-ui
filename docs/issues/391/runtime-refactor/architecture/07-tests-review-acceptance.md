...
| **P1** (headless core) | pure smoke + boundary invariants; one per-session admission path; caller request-id dedupe/conflict; actor/origin persistence; deterministic duplicate-tool failure; exact-once agent-local disposal and bounded caches | `pnpm --filter @hachej/boring-agent run test` · `run test:e2e` · `run lint:invariants` · `pnpm check:agent-isolation` |
| **R0/M1** (managed MCP tracer) | bearer verifier/principal authorization; required subject-scoped caller idempotency key dedupes before quota and survives changed tool-call id; byte caps: brief 32 KiB, key 128 B, progress 4 KiB + 128/64 KiB retention, poll/final 96 KiB, Markdown 256 KiB, result 384 KiB; no path/secret | affected host tests + authenticated stock-client smoke |
...
| **P2** (sandbox pkg/providers + bash mode resolution) | package/import gates plus real EU runsc/systrap provider evidence: digest-pinned OCI, netns/nftables denial, resource limits, secret absence, exact cleanup; mocks alone insufficient | package build/typecheck/invariants/tests · composer typechecks · `audit:imports` · real-target runsc smoke |
...
| **P5a** (v1 provisioning/secrets) | existing engine/runners moved to boring-bash/server with agent origin removed; host orchestration; per-requirement readiness/health; deterministic fingerprint; host-side secret brokerage; non-dev governance roots fail closed. SDK services/remote-worker generality are P5b. | affected bash/host/agent checks |
...
| **P6-D/P6-R** (definition + resolution — **v1 gate**) | separate versioned definition/deployment schemas; immutable verified-bundle registry distinct from resolved-agent registry; deterministic definition/deployment/resolved digests include activated-plugin and source-labeled static-prompt identity; retained generations reproduce pinned host-app/plugin/prompt artifacts; immutable session identity snapshot; requirements validated against active authority | affected agent/core/CLI tests |
| **P6b** (post-v1 child-app/Macro scoping) | blocked on #376; never gates P8 | checks defined when scheduled |
| **A1** (agent authoring — **v1 gate**) | import-free contained directory compile to self-contained bundle; deterministic definition+asset digest; local scripted turn through P6-R. If R0/M1 exists, its hygiene bead is required before P8; only proven absence skips it. | CLI/core smoke |
| **D1** (dedicated delivery — **v1 gate**) | remote target verifies/materializes bundle plus pinned host-app/plugin/static-prompt identity without source checkout; exact HTTPS host serves bounded landing; existing member sign-in enters only the bound managed workspace; dedicated scope covers every workspace-bearing API/front selector, post-signup provisioning, account deletion, and ownership mutation; reserved-host/no-pointer ingress fails inactive before generic routing; readiness is opaque or P5a-authenticated and rejects fabrication/replay; first external publication follows pointer CAS; workspace selects deployed `default` agent; fenced journal survives step crashes/concurrency; timed <=15-minute golden path; zero platform-source edits; no-op reapply; prior complete deployment rollback reproduces host-app/plugin/prompt and resolved identity; no secret output | provisioning smoke + focused full-app journey |
...
| **P8** (v1 verification/cleanup) | zero removal markers; P1, T1/T2, P2/P3, E1, P5a, P6-D/P6-R, A1, and D1 green; timed product proof recorded. P4/E2/X1/P5b/P6 expansion/P7/M2/D2/S3/S4 explicitly do not gate. | `pnpm lint:invariants` · marker/import gates · v1 golden-path smoke |
...
| **D2** (shared subdomain tenancy — factory sidecar follow-up, not P8 gate) | one shared EU deployment hot-registers subdomain tenants from `WorkspaceAgentsDeclaration`; authenticated host adapter creates trusted `TenantContext`; unknown hosts/unauthorized principals fail closed; cross-tenant isolation conformance proves no sessions/files/pending-inputs/search/artifacts/governance leakage; no broker secret crosses tenant boundary | affected package build/typecheck/test · fake-provider smoke · `TenantIsolationConformance` · `pnpm audit:imports` |

...
| **P8** (verification/cleanup) | **zero `TODO(remove:*)` markers repo-wide**; all 00 invariants green; no old-path importer; README documents the four-part surface contract; **every delivered runtime lane green EXCEPT P6b, M1, M2, D1, and S4; M2 may land after P8 as a committed follow-up** (all P1–P7, T1–T2, E1–E2, **X1**, S1–S3, Phase 5, P6a must be green; P8 verifies P6b/M2/D1/S4 follow-up or status tracking and never waits on those lanes landing) | `pnpm lint:invariants` (incl. marker gate) · `node scripts/check-no-remove-markers.mjs` · `pnpm run test` · `pnpm audit:imports` |
...
| **S3** (control-plane UX) | Fleet page lists every declared agent from `GET /api/v1/agents` and enriches/drills down via `/info`; cross-surface observation (workspace attaches to a Slack-born session by `sessionId`); central approval answering | `pnpm --filter @hachej/boring-workspace run test` · `pnpm --filter workspace-playground run test:e2e` |
| **S4** (agent onboarding — onboarding follow-up, not P8 gate) | Fleet drill-down shows definition readiness, demo URL status, provisioning status, and missing policy refs; unknown refs fail closed; no authoring controls; no secret/policy/model key leakage | `pnpm --filter @hachej/boring-workspace run typecheck` · `run test` · `run lint:plugin-invariants` · `pnpm audit:imports` |

...
| **P2** (proposed v1 runsc minimum) | landed sandbox package/publish boundary plus real EU runsc/systrap evidence: isolation, resource limits, secret absence, exact cleanup, and no fallback; full provider/mode migration is post-v1 | affected package/import checks · real-target runsc smoke |
...
| **P5a** (proposed v1 minimum) | only D1-consumed authenticated runsc readiness, redacted fingerprint, host-side secret brokerage, and fail-closed non-dev facts; engine relocation, E1 lifetimes, SDK/services, and remote-mount generality are post-v1 | affected runsc/host/D1 checks |
...
| **D1** (proposed v1 gate) | target materializes the verified bundle and immutable host artifact without checkout access; exact HTTPS landing and existing-member auth reach one managed workspace whose existing composer selects deployed `default`; authenticated EU runsc readiness, selector/lifecycle denials, and idempotent apply; complete redacted site snapshot/digest covers hostname, bounded landing, auth/membership/owner and workspace/default bindings, roots/storage/runtime, host artifact, composition, definition/deployment, and secret ref names/status only; P8 changes site-level values, then rollback restores every prior field/digest and reproduces P6-R identity with no secret value | provisioning smoke + focused full-app journey |
...
| **P8** (proposed v1 verification) | reduced v1 markers/invariants; P1, P6-D/A1, narrow P2/P5a, stateless P6-R, and D1 green; timed workspace-first proof recorded. T1/T2/full P3/E1 and every later lane do not gate. | applicable invariant/import gates · v1 golden-path smoke |

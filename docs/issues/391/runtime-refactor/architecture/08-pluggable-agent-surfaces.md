...
10. **No retro-compat, no speculative abstraction.** All `@hachej/*` consumers are in-repo: importers migrate in the same PR, transitional code carries `TODO(remove:<bead-id>)` with a same-phase deletion bead, no abstraction without two real consumers (binding policy: `../INDEX.md` "Simplicity & no-compat policy"). Exceptions that MUST stay compatible: on-disk pi session JSONL, the landed #416 shared contracts, server↔front within one release train.

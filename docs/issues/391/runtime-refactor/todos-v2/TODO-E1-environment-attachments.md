...
- Notes: Normalize `subpath` with the same rules `readonlyProjectionOperations.normalizeProjectionPath` uses (reject `..`/`.`/null-byte); compute the child root, then hand a `ReadonlyProjectionHandle { filesystem, projectionRoot: childRoot }` to `createReadonlyProjectionOperations`. All ops then inherit `assertInsideProjection` jailing for free. Never filter paths on the consumer side.

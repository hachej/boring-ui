...
- Notes:
  ...
  - **In `@hachej/boring-agent` shared (minimal core-facing):** the agent-side injection type **IS** the existing operation-bearing binding array — `ResolvedEnvironments { bindings: RuntimeFilesystemBinding[] }`, where `RuntimeFilesystemBinding` is the **landed agent shape** `{ filesystem, access, operations }` (`packages/agent/src/server/runtime/mode.ts`). **There is NO `PreparedEnvironmentAttachment { handle: unknown }` — it is deleted.** The agent never receives an opaque handle; it receives prepared, operation-bearing bindings. `FilesystemId`/`FilesystemAccess` are the agent's own local string-literal aliases matching `RuntimeFilesystemBinding`. boring-bash's `resolveAttachments` **imports the agent-defined `ResolvedEnvironments` type-only** and returns it — wrapping prepare + operations construction (see BBE1-002).

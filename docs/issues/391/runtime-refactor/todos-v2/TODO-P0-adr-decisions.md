...
- `docs/issues/391/runtime-refactor/00-global-isa.md` — intent/strategy/architecture, package ownership table, non-negotiable invariants 1–14, open-decisions list.
- `docs/issues/391/runtime-refactor/08-pluggable-agent-surfaces.md` — **the 7 decisions this phase ratifies** live in its "Decisions this file locks" section (§ near the end).
- `docs/issues/391/runtime-refactor/06-migration-phases.md` — "Phase 0" section is the deliverable list this TODO expands.
- `docs/issues/391/runtime-refactor/09-environments-attachable.md` — backing detail for decision 7 (attachable environments).
...
- A new locked decision (the v2 runtime-free + surface-agnostic ADR) is merged into `docs/DECISIONS.md` in the existing 4-field format, and the 7 locked decisions from `08` are each ratified (recorded or cross-referenced) with a status of `decided` / `deferred`.
...
- Do NOT mark any of the 7 decisions `open` — `08` already locks them; record them as `decided` (or `deferred` for the ones `00` lists as still-open, e.g. surface addressing-store location).
...
- **Implementation notes:** Use the 4-field table. In **What**, cite the package-ownership table from `00-global-isa.md`. In **Re-evaluate when**, reference the AI-SDK v6 migration (the trigger `08` names for revisiting the wire protocol). Link the entry to `docs/issues/391/runtime-refactor/README.md` and `08-pluggable-agent-surfaces.md`.
...
- **Files to touch/create:** create `docs/issues/391/runtime-refactor/todos-v2/_issue-391-comment.md` (draft comment body); no code.
- **Implementation notes:** The draft must: link `docs/issues/391/runtime-refactor/README.md` and `08-pluggable-agent-surfaces.md`; state that the legacy monolith plan is superseded (preserved snapshot at `docs/issues/391/runtime-refactor/legacy-monolith-source.md`); list the v2 phase set from `06-migration-phases.md` (Phases 0–8 + tracks T/S/E); use "directly owned vs materially advanced if acceptance lands" language from `00-global-isa.md`. If `gh` is available, post it (`gh issue comment 391 --body-file …`) and record the URL; otherwise leave the file for a human to post.
- **Tests to add:** none.
- **Acceptance:** A single copy-paste-ready comment body exists in-repo that reconciles #391 with the v2 pack and contains no phase drift versus `06-migration-phases.md`.
...
- **Files to touch:** `docs/issues/391/runtime-refactor/00-global-isa.md` (open-decisions section — decisions 1, 2, 6 are already resolved in-file; ensure each resolved item points to `08`). Confirm `README.md` already states `08` supersedes surface decisions (it does — verify wording is consistent).
...
- A reviewer must confirm all 7 `08` decisions are recorded with a status and a source pointer, and that §7e's supersession note does not weaken the no-split-brain guarantee for boring-bash-active runtimes.

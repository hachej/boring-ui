...
- **Files to create**:
  - `packages/agent/src/shared/transport.ts` (new): the minimal contract (AI-SDK `ChatTransport`-shaped):
    ...
    - **`originSurface?: string` — session-create provenance metadata, DEFINED HERE (T2):** the session-create path (`sendMessages` on a NEW session / `POST …/agents/:agentId/sessions`) carries an **optional `originSurface?: string`** written by the calling adapter and stored on the session record at `create()`. **Producers:** the **workspace adapter writes `'workspace'`**, the **S1 Slack adapter writes `'slack'`** (`TODO-S1` BBS1-002), and **embeds write their own** (e.g. `'embed'`). It is display/provenance metadata only — **never an addressing key** (two-handles rule). The field is defined on this session-create contract in T2; **S3's origin-surface badge + filter (`TODO-S3` BBS3-002) is its consumer**, and `SessionSummary.originSurface` (S3) mirrors it.

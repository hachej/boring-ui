...
- **Notes (resolve `00` open decision 4 — record the choice in the file header)**: recommend **path prefix `/api/v1/agents/:agentId`** for new agent-scoped routes (explicit, cache-key-friendly, matches the `/info` endpoint and the eve `/eve/v1/*` analog) while keeping the existing unprefixed routes bound to the default agent for back-compat within this release train (server+front ship together — `08` decision 9). Do not add both a header and a path form for the same route.

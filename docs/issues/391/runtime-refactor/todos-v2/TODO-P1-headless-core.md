...
- **Tests to add:** unit test that `createAgent({ runtime: 'none', harnessFactory: fakeHarness })` constructs without Fastify and exposes exactly the **seven** members `start`, `stream`, `send`, `resolveInput`, `sessions`, `readiness`, `dispose`; `send` yields at least one event from a fake harness turn via the live tail (fuller smoke is BBP1-006); a `stream(sessionId,{startIndex})` call with an offset older than the live buffer throws `AgentNotImplementedError`/`ERR_NOT_IMPLEMENTED_UNTIL_T1`.
- **Acceptance:** `createAgent()` is importable and usable with no Fastify, no env reads; the **seven-member** API (`start`, `stream`, `send`, `resolveInput`, `sessions`, `readiness`, `dispose`) exists; `send` drives one turn through a fake harness.

...
- Do NOT invent a new streaming/event protocol — reuse `HarnessPiChatService`. The indexed `AgentEvent` envelope + `stream` (replay+live-tail) internals are Phase T1; in P1 `stream` (and `resolveInput`) are typed stubs throwing `AgentNotImplementedError` / `ERR_NOT_IMPLEMENTED_UNTIL_T1` (see BBP1-002).
...
- **Implementation notes:**
  ...
  - `agent.stream(sessionId, { startIndex }): AsyncIterable<AgentEvent>` — the read primitive (replay-from-offset + live tail; replaces a separate `replay()`). **Stub in P1** — the durable offset-addressed event log is T1. Throw a typed `AgentNotImplementedError` with code `ERR_NOT_IMPLEMENTED_UNTIL_T1` (do not silently delegate to `PiChatReplayBuffer` — downstream packs must not build on stub behavior; see the Do-NOT below). Provide the exact signature so T1 fills it in without an API break. Cancelling the returned iterator must never cancel the turn (document this on the method even while stubbed).
  - `agent.send(input: AgentSendInput): AsyncIterable<AgentEvent>` — documented convenience only: `const { sessionId, startIndex } = await start(input); yield* stream(sessionId, { startIndex })`. Because `stream` is a T1 stub, `send` is likewise not fully functional until T1; keep it defined as the composition, add no independent semantics.
  ...
  - **Typed stub error:** define `AgentNotImplementedError` (code `ERR_NOT_IMPLEMENTED_UNTIL_T1`) in the shared/type-only surface; both `stream` (replay-before-T1) and `resolveInput` throw it. **Do NOT** ship any downstream pack (T2, S1, P3, …) that depends on stub behavior — they block on T1 filling these in.

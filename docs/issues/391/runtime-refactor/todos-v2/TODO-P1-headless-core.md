...
- `docs/issues/391/runtime-refactor/01-agent-core-runtime-free.md` — the pure-mode contract, `AgentFeature`/`AgentEnvironment` shapes, the pi-harness audit questions, required tests.
- `docs/issues/391/runtime-refactor/08-pluggable-agent-surfaces.md` — the `createAgent()` API surface (`send`/`resolveInput`/`sessions`/`replay`/`readiness`/`dispose`), the two-handles rule, the "façade has no Fastify import / no env reads / no file discovery" rule.
...
- `createAgent()` exported from `@hachej/boring-agent/server`, Fastify-free, returning `{ send, resolveInput, sessions, replay, readiness, dispose }`.
...
- No new plugin registry. `AgentFeature` is a façade over the existing tool/route/systemPrompt/capability seams (`buildHarnessAgentTools`, `buildFilesystemAgentTools`, `mergeTools({ checkReadiness })`, `registerCapabilitiesContributor`) — see `01-agent-core-runtime-free.md` "Public core contracts".
...
- Do NOT invent a new streaming/event protocol — reuse `HarnessPiChatService`. The indexed `AgentEvent` envelope + `replay` internals are Phase T1; in P1 `replay` is a stub (see BBP1-002).
...
- **Description:** Add `createAgent(config): Agent` to `@hachej/boring-agent/server`, returning `{ send, resolveInput, sessions, replay, readiness, dispose }`. Fastify-free. Config shape derived from what `createAgentApp` currently reads (BBP1-001), passed in — never read from env inside the façade.
...
- **Implementation notes:**
  - Derive `AgentConfig` from `CreateAgentAppOptions` minus HTTP/Fastify concerns. Concretely it carries: `harnessFactory?` (default `createPiCodingAgentHarness`), `runtime: RuntimeModeAdapter | 'none'`, `features?: AgentFeature[]`, `tools?: AgentTool[]` (extra app-owned tools), `sessions?` override, `systemPromptAppend?`, `systemPromptDynamic?`, `telemetry?`, `metering?`, `sessionStorageRoot?` (see BBP1-004), and the resolved non-env values that were previously env-read (template path, workspace/session roots) — all supplied by the caller.
  - `agent.send(input, ctx)` wraps `HarnessPiChatService` (`packages/agent/src/server/pi-chat/harnessPiChatService.ts`): build the service once from `{ harness, sessionStore: harness.sessions, workdir, workspace?, metering }`, drive a prompt, and expose the event stream. In P1 the yielded events may be the existing `PiChatEvent`/chunk stream; the indexed `AgentEvent` envelope is T1. Keep the async-iterable/subscriber shape so T1 can wrap it without an API break.
  ...
  - `agent.resolveInput(...)` — **stub in P1** (real approvals path is T1). Provide the method signature `(sessionId, requestId, response)` and throw a `NOT_IMPLEMENTED`-style error, or route to the existing follow-up path if trivially available. Document the T1 hand-off.
  - `agent.replay(sessionId, { startIndex })` — **stub in P1**. The durable event log is T1. Provide the signature and a clear "requires T1" error, or delegate to `PiChatReplayBuffer` read-only if it already buffers (it does per session live-channel; check `pi-chat/piChatReplayBuffer.ts`).
...
- **Tests to add:** unit test that `createAgent({ runtime: 'none', harnessFactory: fakeHarness })` constructs without Fastify and exposes all six members; `send` yields at least one event from a fake harness turn (fuller smoke is BBP1-006).
...
- **Implementation notes:**
  ...
  - `registerAgentRoutes` is the hard one: its per-request `RuntimeBinding` LRU (`createRuntimeBinding`, `getOrCreateRuntimeBinding`, `MAX_RUNTIME_BINDINGS`) plus background provisioning must be preserved. Model each workspace binding as a `createAgent()` instance (or a shared factory) keyed by `scope.key`. The two-handles rule (`00` invariant 12): `x-boring-workspace-id → SessionCtx` mapping stays adapter-side; the façade only sees `sessionId`/`SessionCtx`.
...
- **Description:** Add the pure composition path (`runtime: 'none'`, `features: []`) and separate transcript/session storage (`sessionStorageRoot`) from workspace file roots.
...
- Do not start Track T1 (event envelope / real `replay` / `resolveInput`) until BBP1-002..006 are merged; T1 depends on the stub seams landing here.

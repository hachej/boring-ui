...
- `plugins/ask-user/src/front/index.tsx` — `definePlugin` registration; a `QuestionForm` pane (`ASK_USER_PANEL_ID = "ask-user.questions"`, placement `workspace-page`) **and** an **`InboxOverlay`** (`plugins/ask-user/src/front/inbox/{InboxOverlay,InboxDetailPanel,InboxFilterBar,InboxRow,InboxSection}.tsx`) — a pending-questions inbox **across sessions** with filter/detail. Mounted as an app-left overlay (`BoringFrontAppLeftOverlayProps`).
...
- Plugin authoring API `@hachej/boring-workspace/plugin`: `definePlugin({ id, panels, workspaceSources, commands, catalogs, appLeftOverlay, setup })` (`packages/workspace/src/shared/plugins/frontFactory.ts`).
...
- `api.registerCatalog(...)` → `packages/workspace/src/front/plugin/CatalogRegistry.ts`; `api.registerPanelCommand(...)` → `.../CommandRegistry.ts` (command palette). App-left overlay chrome: `packages/workspace/src/shared/plugins/appLeftOverlayChrome.ts`.
...
- Readiness source: `packages/agent/src/server/runtime/readyStatus.ts` (`ReadyStatusTracker.getReadiness()`) surfaced at `GET /api/v1/ready-status` (SSE; `packages/agent/src/server/http/routes/readyStatus.ts`). P7 scopes this per `(workspaceId, agentId)`.
...
- `docs/issues/391/runtime-refactor/08-pluggable-agent-surfaces.md` § "The steering surface" — the workspace is the **control plane**; steering = the workspace consuming the same **public contracts, with more of them** (the `/info` endpoint), never private core hooks. Also § "The headless façade: `createAgent()`" (the core public API is `agent.start()` receipt + `agent.stream()` replay/tail + `resolveInput`) and § "Human-in-the-loop" (one approval channel; any surface holding the `sessionId` answers via `resolveInput`).
- `docs/issues/391/runtime-refactor/00-global-isa.md` § "North star" — eve-class UX steered from the workspace: author → configure → deploy → converse anywhere → inspect/observe. **Agent-as-directory authoring is explicitly deferred** until `AgentRegistry`/Phase 6–7 (no-speculative-abstraction policy) — see Do NOT.
- `docs/issues/391/runtime-refactor/06-migration-phases.md` § Phase 7 v2 ("agent inspection endpoint … consumed by workspace panels"), § Phase T1 (durable indexed event stream + on-stream approvals + `resolveInput`), § Phase T2 (front transport refit to `sessionId`-only public contract).
- `docs/issues/391/runtime-refactor/todos-v2/TODO-P7-multi-agent-inspection.md` — **BBP7-005 ships `GET /api/v1/agents/:agentId/info`** (`{ agentId, model, tools, readiness, channels, environments }`; public, private-hook-free, no secret leak; modeled on `models.ts`). **This is the entire endpoint S3 consumes.** BBP7-002 defines the `/api/v1/agents/:agentId/...` path prefix and `AgentRegistry` addressing; BBP7-004 ships the scoped session-search API S3's session filter calls.
- BINDING policy: `docs/issues/391/runtime-refactor/todos-v2/README.md` "Simplicity & no-compat policy" — no shims, no abstraction without two real consumers, `TODO(remove:<bead-id>)` regime, migrate every importer in the same PR. House format: `TODO-P2-bash-package-providers.md`.

### Depends on

- **T2** (`TODO-T2-transport-adapters.md`): the `sessionId`-only public transport (`RemotePiSession`/`usePiSessions`/`PiChatPanel` refit) + DS `startIndex` replay. Cross-surface transcript viewing (BBS3-002) attaches the refit stack by `sessionId`.
- **P7** (`TODO-P7-multi-agent-inspection.md`): `GET /api/v1/agents/:agentId/info` (BBP7-005), the `/api/v1/agents/:agentId` addressing + `AgentRegistry` (BBP7-002), and the scoped session-search route `GET /api/v1/agents/:agentId/sessions/search` (BBP7-004). **If `/info` and the agent-scoped routes are not present, STOP and report** — S3 consumes them, it does not build them.
- **T1** (`TODO-T1-durable-events-approvals.md`, via T2/P7): the durable pending-input-request store + `resolveInput` path the central inbox (BBS3-003) reads/writes. The ask-user server store is what BBS3-003 migrates onto this path.

## Goal / exit criteria

The workspace is the eve-class control plane over public contracts only:

1. An **agent list + inspect panel** exists (new), registered through the existing panel/source registry, rendering `GET /api/v1/agents/:agentId/info` (model, tools, readiness, channels, environments) read-only for every declared agent. No private core hook, no secret shown.
...
- **Public contracts only.** S3 consumes `agent.start`/`agent.stream`/`resolveInput` (in-process) and the HTTP surfaces `GET /api/v1/agents/:agentId/info`, `GET /api/v1/agents/:agentId/sessions/search`, the pi-chat event routes, and `resolveInput`/`POST …/input`. **No import of core internals, no private hook** (`08`/`00` steering rule; the endpoint is the entire private-hook-free surface).
- **Register through the existing system.** New panels via `api.registerPanel({ placement: "workspace-page" })` (`PanelRegistry`); new left-rail entries via `api.registerWorkspaceSource` (`WorkspaceSourceRegistry`); overlays via the app-left-overlay chrome. Do not add a new registry or a competing plugin host.
- **One approval channel** (`00` invariant 13; `08` HITL). The central inbox routes onto T1 `resolveInput` — it does not add a second approval mechanism. ask-user's own pending store/publisher is migrated onto T1, not paralleled.
...
### BBS3-001 — Agent list + inspect panel (NEW; consumes `GET /api/v1/agents/:agentId/info`) · size M
- **Title**: A read-only workspace panel listing declared agents and inspecting one agent's model/tools/readiness/channels/environments.
- **Status**: the **genuinely new** control-plane surface. Everything else in S3 is a delta on an existing component; this panel is new — but it registers through the existing panel system and reuses the existing inspector/readiness idioms.
- **Files create**: a small front plugin (e.g. `packages/workspace/src/plugins/agentInspectorPlugin/front/` or a new `plugins/agent-inspector/` — match the repo's plugin-placement convention; verify against how `filesystemPlugin` and `ask-user` are located and pick the lighter one). Contains: an `AgentInspectorPanel` (agent picker → `/info` fetch → read-only sections for model/tools/readiness/channels/environments, modeled on `DebugDrawer`'s tabbed read-only inspector), and an `/info` client.
- **Files touch**: register via `definePlugin({ panels: [{ id: "agent-inspector", label: "Agents", component, placement: "workspace-page" }] })` and/or `registerWorkspaceSource` for a left-rail entry (`packages/workspace/src/shared/plugins/frontFactory.ts` API → `PanelRegistry`/`WorkspaceSourceRegistry`). Reuse the model-label helpers from `chatModelLabels`/`chatPanelComposerControls.tsx` for provider/model display; reuse the readiness snapshot shape from `runtime/readyStatus.ts`.
- **Notes**: Fetch `GET /api/v1/agents/:agentId/info` per agent (list of declared agents from the P7 `AgentRegistry` addressing / the workspace's agent set). Read-only — **no** edit/configure controls (authoring is out of scope). Never render a secret/handle field; assert the payload is already scrubbed (BBP7-005). If a live readiness stream is wanted, subscribe to the existing `GET /api/v1/ready-status` (agent-scoped per P7) rather than polling `/info`.
- **Tests**: `AgentInspectorPanel.test.tsx` — renders model/tools/readiness/channels/environments from a mocked `/info`; a reviewer agent shows readonly env + no bash tool; a pure concierge shows no environments; asserts no key/secret field is rendered; the panel registers under the existing `PanelRegistry`/`WorkspaceSourceRegistry` (not a new host).
- **Acceptance**: `06` Phase 7 v2 "agent inspection endpoint … consumed by workspace panels"; panel is public-contract-only and registered through the existing system.
...
- **Files touch**:
  - `packages/agent/src/shared/session.ts` — add `originSurface?: "workspace" | "slack" | "embed" | "cli" | string` (and, if not already threaded, `agentId?`) to `SessionSummary`. The session store populates it at `create()` from the creating surface (the field is written by T2/S1 adapters; S3 consumes it). Keep it optional so existing JSONL sessions load unchanged (default `"workspace"`).
...
- **Files touch**:
  ...
  - `plugins/ask-user/src/front/runtime.ts` + `providerHooks.ts` — source pending requests from the T1 durable pending store (via the public `resolveInput`/pending-requests read) instead of the ask-user-only `askUserStore`. Answering an item calls `agent.resolveInput(sessionId, requestId, response)` (in-process) / `POST …/input` (HTTP).
  - `plugins/ask-user/src/server/{askUserRuntime,questionsBridge,askUserStore,askUserStatePublisher}.ts` — route the ask-user question path onto the T1 on-stream request + pending-request store (T1 migrates the ask-user tool onto the single channel). Mark any bridging code with `// TODO(remove:BBS3-003)` + a same-phase deletion of the ask-user-only pending publisher once T1 owns it.
...
- **Tests**: inbox test — pending requests from two sessions on two origin surfaces appear in one inbox; answering an item calls `resolveInput` with the right `(sessionId, requestId)`; an ask-user question and a tool approval render in the same list; the ask-user-only pending publisher is deleted (no second channel) — assert via the `TODO(remove:*)` regime.
...
- Agent inspect panel consumes `GET /api/v1/agents/:agentId/info` only, is read-only (no authoring), renders no secret/handle, and registers through the existing `PanelRegistry`/`WorkspaceSourceRegistry` (not a new host).

...
- **Public contracts only.** S3 consumes `agent.start`/`agent.stream`/`resolveInput` (in-process) and the HTTP surfaces `GET /api/v1/agents/:agentId/info`, `GET /api/v1/agents/:agentId/sessions/search`, the **T2 public event transport** (`GET /api/v1/agents/:agentId/sessions/:sessionId/events/stream` + the `ChatTransport` contract from `TODO-T2` — NOT the legacy `…/pi-chat/:sessionId/*` routes, which are deleted at the T2 cutover), and `resolveInput`/`POST /api/v1/agents/:agentId/sessions/:sessionId/input`. **No import of core internals, no private hook** (`08`/`00` steering rule; the endpoint is the entire private-hook-free surface).
...
- **Files touch**:
  - `packages/agent/src/shared/session.ts` — add `originSurface?: "workspace" | "slack" | "embed" | "cli" | string` (and, if not already threaded, `agentId?`) to `SessionSummary`, mirroring the `originSurface` field **defined on the T2 session-create contract (`TODO-T2` BBT2-001)**. The session store populates it at `create()` from the creating surface — the T2/S1 **adapters** write it: the workspace adapter writes `'workspace'`, the S1 Slack adapter writes `'slack'` (`TODO-S1` BBS1-002), embeds write `'embed'`; **S3 only consumes it**. Keep it optional so existing JSONL sessions load unchanged (default `"workspace"`).

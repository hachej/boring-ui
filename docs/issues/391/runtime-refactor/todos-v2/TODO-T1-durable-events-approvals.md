...
- `packages/agent/src/shared/chat/piChatEvent.ts` — `PiChatEvent` union, each variant carries `seq: number`. **The boring stream unit is `PiChatEvent`, not a raw AI-SDK `UIMessageChunk`.** The plan's `AgentEvent.chunk: UIMessageChunk` maps onto boring as `chunk: PiChatEvent` (the mapped/enriched event already emitted today). Preserve that; do not re-plumb to raw pi chunks in T1.
- `packages/agent/src/shared/harness.ts` — `AgentHarness`, `SendMessageInput`, `RunContext`. The header comment at L79 ("Resume is NOT a harness concern … The HTTP route owns cursor buffering + replay; harness stays reconnect-unaware") must be **updated**: T1 introduces `agent.replay()` as an in-process API. Add `replay` to the façade, not to `AgentHarness` (the harness stays reconnect-unaware; the store + service own replay).
...
2. `agent.replay(sessionId, { startIndex })` (in-process) + DS-compliant `GET`/`HEAD` stream routes (catch-up from offset, SSE + long-poll) behind a Fastify↔WHATWG bridge.
3. SSE drop + reconnect replays losslessly (proven by conformance test; front cutover is T2).
4. `needsApproval` on `AgentTool`; approval-request emitted on-stream; pending request persisted; turn parks (`session.waiting`) and resumes; `resolveInput(sessionId, requestId, response)` in-process + `POST …/input` route.
5. ask-user + any permission prompts ride the single approval path — no second channel.
6. Harness conformance additions pass: envelope ordering, replay-from-index, approval park/resume across process restart.
...
- Do NOT add a second persistence format for approvals — persist pending requests in the same SQLite DB as the event stream (or the session store), not a new JSON file.
...
### BBT1-003 — DS-compliant GET/HEAD routes + `agent.replay()`  · size L
...
- **Files to touch/create**:
  ...
  - `packages/agent/src/server/createAgent.ts` (touch): `resolveInput(sessionId, requestId, response)` — look up the in-memory waiter; if the process restarted and the waiter is gone but the pending row exists, mark resolved so the resumed turn (BBT1-006) can pick it up.
  - `packages/agent/src/server/http/routes/eventStream.ts` or a sibling `input.ts` (new): `POST /api/v1/agent/sessions/:sessionId/input` `{ requestId, response }` → `agent.resolveInput(...)`.
...
- **Migration (precise)**:
  ...
  2. Response shape: `resolveInput(sessionId, requestId, { values })` maps to the existing `AskUserAnswer.values`; reuse `formatAskUserResult` so the model-visible text is unchanged.
  ...
  4. Delete/deprecate the second channel: `AskUserStatePublisher`, `askUserBridgeHandlers` (`ask-user.v1.*`), `questionsRoutes`, and `FileAskUserStore` become unused once pending-request persistence (BBT1-004) is the source of truth. Remove them in the same PR only if the front is cut over; otherwise leave a `@deprecated` shim and file a cleanup bead (Phase 8).
...
- **Acceptance**: 08 "Existing permission prompts and the ask-user plugin migrate onto this path; no second approval channel." Questions pane still works end-to-end; no `ask-user.v1.*` bridge traffic in the happy path.
...
- **Cases**:
  ...
  - **Approval park/resume across process restart**: emit a `needsApproval` tool → request persisted → **dispose the service and rebuild it against the same SQLite file** → `resolveInput(requestId)` resumes the parked turn to completion (proves durability, 08 "Durable across process restarts once the session event log lands").

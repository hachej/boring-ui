...
- **Files to touch/create**:
  ...
  - `packages/agent/src/server/createAgent.ts` (touch): `resolveInput(sessionId: string, requestId: string, response: ResolveInputResponse)` — look up the in-memory waiter; if the process restarted and the waiter is gone but the pending row exists, mark resolved so the resumed turn (BBT1-006) can pick it up.

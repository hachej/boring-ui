...
- **Locked route family (from day one):** every new T1 HTTP route lives under the canonical agent-scoped prefix `/api/v1/agents/:agentId/...` — `GET`/`HEAD` `/api/v1/agents/:agentId/sessions/:sessionId/events/stream`, `POST /api/v1/agents/:agentId/sessions/:sessionId/input`, `GET /api/v1/agents/:agentId/sessions/:sessionId/input`, and `GET /api/v1/agents/:agentId/pending-inputs`. Use a **canonical `default` agentId** until the P7 `AgentRegistry` lands; **P7 then only adds agentId resolution — no path migration**. `agentId` is a routing-scope path segment resolved in the route; it never enters the store/replay signature or `sessionStreamPath` (still keyed by `sessionId` only). This is the P7-locked family (`00` open decision 4) — do not ship T1 routes under an unprefixed `/api/v1/agent/...` shape that P7 would have to migrate.
...
- **Files to create/touch**:
  - `packages/agent/src/shared/events.ts` (new): 
    ...
    ```
  - `packages/agent/src/server/pi-chat/harnessPiChatService.ts` (touch): inject an optional `EventStreamStore` (constructor option `eventStore?`). In `publishChannelEvent`, **durably append first, then fan out**: `await this.eventStore?.appendEvent(sessionStreamPath(sessionId), toAgentEvent(sessionId, event))` and only after that commits `channel.buffer.publish(event)` to the in-memory live subscribers. Ensure the stream is created lazily in `buildChannel` (`await eventStore.createStream(path)`). Keep the in-memory live fan-out fast (the single awaited append is the only added latency).
  - The SQLite `EventStreamStore` offset is the **sole** `AgentEvent.eventIndex`; `PiChatEvent.seq` stays only inside `chunk` (internal to the payload, never surfaced as the index), and the tap keys `appendEventOnce(path, key=String(seq), …)` on that internal `seq` purely for idempotency across a service restart replaying the buffer.
...
- **Files to touch/create**:
  ...
  - **Pending-request read API (named — S3's approval inbox and P7 consume this exact name):** add `agent.pendingInputs({ sessionId? }): Promise<PendingInputRequest[]>` on `createAgent.ts`. It returns the currently-parked input/approval requests, **scoped** (by `sessionId` when given, otherwise all sessions visible to the caller's `SessionCtx`) and **redacted** (tool params scrubbed of secret/credential material with the same redaction the transcript uses). Back it by the `boring_pending_requests` table (this bead's persistence) so it survives restarts and reflects rows a different process parked. `PendingInputRequest` shape: `{ sessionId, requestId, kind: 'approval' | 'input', toolName?, toolCallId?, schema?, createdAt }` — **no raw params, no secrets**. Add the HTTP mirror on the adapter (locked route family; `:agentId` canonical `default` until P7): `GET /api/v1/agents/:agentId/sessions/:sessionId/input` (scoped) and `GET /api/v1/agents/:agentId/pending-inputs` (cross-session, for the inbox), both returning the same redacted `PendingInputRequest[]`. This is the single named pending-request listing API — do not leave it unspecified.
...
- **Tests**: `approval.test.ts` — tool with `needsApproval: true` emits a `data-approval-request` on the stream, turn parks (status `waiting`), `resolveInput` with `approve` lets `execute` run and the turn completes; `deny`/abort cancels. A second subscriber sees the same request and can resolve it (proves "answered from another client"). **`pendingInputs.test.ts`** — while a turn is parked, `agent.pendingInputs({ sessionId })` returns the parked request (redacted: no raw params/secrets); `agent.pendingInputs({})` lists it across sessions; after `resolveInput` clears the row the list is empty; the request survives a store rebuild against the same SQLite file (matches BBT1-006 durable-survival). The `GET …/input` and `GET …/pending-inputs` routes return the same redacted payload.
- **Acceptance**: 06 Phase T1 exit — "an approval issued in one client can be answered from another client holding the same session." The named `agent.pendingInputs({ sessionId? })` read API (+ its `GET` HTTP mirror) exists, is redacted and durable, and is the API S3's approval inbox and P7 consume.

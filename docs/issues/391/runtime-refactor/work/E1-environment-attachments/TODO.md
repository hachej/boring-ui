...
- Description: Make the agent core **own** the `ResolvedEnvironments` core-facing type — `{ bindings: RuntimeFilesystemBinding[] }` (defined in `@hachej/boring-agent` shared, BBE1-001; there is **no** `PreparedEnvironmentAttachment`) — and prove it never imports — type **or** value — from `@hachej/boring-bash`. Cross-package type edges are explicit and one-way by purpose: boring-bash → agent for the core-facing resolved binding type, and boring-bash → boring-sandbox/shared for the authoritative `ProviderCapabilities` alias/pick. The agent imports neither boring-bash nor boring-sandbox.

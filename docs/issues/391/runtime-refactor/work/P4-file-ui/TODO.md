...
- File mentions and file-related slash commands are capability-gated composer providers shipped by `boring-bash/plugin`; pure-mode front ships no `/api/v1/files/search` dependency, `@files` note path, upload affordance, or filesystem vocabulary.
...
- Composer providers move as part of the bash capability bundle: file mention provider, file-related slash commands, `@files` note generation, and upload affordances are registered only when the host attaches the bash/filesystem capability. Agent front keeps provider-neutral composer/picker primitives only.
...
- **Files touch/move:** introduce a small provider-neutral composer contribution seam in the public workspace/agent front boundary (name free to implementation; e.g. `registerComposerProvider` / `composerProviders` analogous to `toolRenderers`) and move the file provider out of `packages/agent/src/front/useComposerPickers.ts`, `packages/agent/src/front/primitives/mention-picker.tsx`, `packages/agent/src/front/chatSubmit.ts`, and `packages/agent/src/front/chat/components/PiChatComposerSurface.tsx` into `packages/boring-bash/src/plugin/composer/`. The moved provider owns file search (`/api/v1/files/search`), file mention token selection, `@files: ...` model-note generation, file-related slash commands, and upload affordances (`/api/v1/files/upload`) for fs-bearing hosts.
- **Notes:** This implements the repo-local #26 direction cited in `docs/plans/archive/pi-native-chat-ui-rewrite-plan.md`: move `@file` mentions from agent into a workspace composer provider because workspace/bash own file context. Keep the agent front provider-neutral: it may render a generic mention/slash picker supplied by providers, but it must not know that `@` means files, must not call `/api/v1/files/search`, and must not synthesize `@files:` in pure mode. The provider is capability-gated by attached bash/filesystem capability; missing capability means no provider, not a disabled file UI.
...
- **Acceptance:** file mentions, file slash commands, upload affordances, and `@files:` enrichment exist only when `boring-bash/plugin` is attached; pure-mode front has zero filesystem vocabulary in the composer.
...
- File mention/slash composer providers are capability-gated; pure-mode front has no `/api/v1/files/search` dependency, upload affordance, `@files:` note, or file slash command.

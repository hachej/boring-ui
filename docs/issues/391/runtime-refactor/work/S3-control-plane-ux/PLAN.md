...
## Deliverables
- A **Fleet page** (new), registered through the existing workspace-page registration, listing every declared agent (model/tools/readiness/environments from `GET /api/v1/agents/:agentId/info`) read-only, with a per-agent drill-down (that agent's sessions, pending approvals, environments). Fleet-widget extension point deferred to the farm epic.

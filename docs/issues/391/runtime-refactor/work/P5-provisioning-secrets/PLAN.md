...
the Phase 2 provider capability matrix (`ProviderCapabilities`); it runs parallel
...
`Bash*` declarative data shape live in `@hachej/boring-bash/shared`; the host wires
them. `@hachej/boring-agent` keeps **zero value imports** from boring-bash. The v2

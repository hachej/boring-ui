...
- **P2** `@hachej/boring-sandbox/providers` + `providers/matrix.ts` `ProviderCapabilities` (in `boring-sandbox/shared`) exist (the normalizer validates requirements against them). The concrete providers live in `@hachej/boring-sandbox` (00 open decision 3 RESOLVED; 08 decision 11), not `boring-bash/providers`; `resolveMode` lives in `@hachej/boring-bash/modes`. If the boring-sandbox providers/matrix are absent, STOP and report — do not fork a second capability model.
...
- **Files create:** `packages/boring-bash/src/shared/bashRequirement.ts` (front-safe data shape — no `node:*`/`Buffer`; **this is the single home for every `Bash*` declarative data shape** — `BashRequirement`, `BashHealthCheckSpec` (used by BBP5-004), `BashManagedServiceRequirement` (used by BBP5-006), `BashSdkArchiveSpec` (used by BBP5-005), and their sub-types. `packages/agent` owns only the **generic runners** that consume these normalized inputs; it never owns a `Bash*` data shape); `packages/boring-bash/src/server/provisioning/resolveBashProvisioningRequirements.ts` (the normalizer; server-scoped, may use `node:*`); `packages/boring-bash/src/server/provisioning/__tests__/resolveBashProvisioningRequirements.test.ts`.
...
- **Files create:** `packages/agent/src/server/workspace/provisioning/healthCheck.ts` — the health-check **runner only** (a generic runner over the normalized `BashHealthCheckSpec` input) + `__tests__/`. **The `BashHealthCheckSpec` data shape lives in `@hachej/boring-bash/shared` (co-located with `BashRequirement`, BBP5-001), NOT in `packages/agent`** — the runner imports it **type-only**. `packages/agent` owns only the runner, never the `Bash*` data shape.
- **Files touch:** `provisioning/types.ts` (add `readiness?.healthCheck` passthrough — a **type-only** reference to the boring-bash-owned `BashHealthCheckSpec`), `registerAgentRoutes.ts` (gate `runtime:<id>` ready on health pass).
- **Notes:** `BashHealthCheckSpec` (declared in `@hachej/boring-bash/shared`, BBP5-001) per `../03` (command/http probe, `timeoutMs`, retry/backoff). Health failure → `CapabilityState='failed'` with a stable `errorCode`, retryable where appropriate; it gates the dependent tool/panel, never the aggregate unless required. Redact any secret-shaped values from health output before logging.
...
- **Files create:** `packages/agent/src/server/workspace/provisioning/sdkArchive.ts` — the SDK-archive **runner/installer only** (install-from-packed-artifact over the normalized `BashSdkArchiveSpec` input) + `__tests__/`. **The `BashSdkArchiveSpec` data shape lives in `@hachej/boring-bash/shared` (co-located with `BashRequirement`, BBP5-001), NOT in `packages/agent`** — `sdkArchive.ts` imports it **type-only**. `packages/agent` owns only the runner, never the `Bash*` data shape.
- **Files touch:** `provisioning/types.ts` (`RuntimeProvisioningContribution.sdkArchives` — a **type-only** reference to the boring-bash-owned `BashSdkArchiveSpec`), `provisionWorkspaceRuntime.ts` (add an `sdk archives` phase between node and python, following the `runPhase` pattern), `fingerprint.ts` (fold archive id + content hash into the fingerprint).
- **Notes:** `BashSdkArchiveSpec` (declared in `@hachej/boring-bash/shared`, BBP5-001; the runner imports it type-only). Reuse `packArtifact.ts` — archives enter the runtime as packed/copied artifacts via `adapter.resolveInstallSource`/`copyFromHost`, **never** as ad-hoc host paths. Rewrite any resulting env/path entries to sandbox-visible paths. Preserve Vercel pack-artifact no-host-leak rules.
...
- **Files create:** `packages/agent/src/server/workspace/provisioning/managedService.ts` — the managed-service **supervisor/runner only** (start/health/port-grant/teardown, over the normalized `BashManagedServiceRequirement` input) + `__tests__/`. **The `BashManagedServiceRequirement` data shape lives in `@hachej/boring-bash/shared` (co-located with `BashRequirement`, BBP5-001), NOT in `packages/agent`** — the supervisor imports it **type-only**; agent owns only the runner.
- **Files touch:** `provisioning/types.ts` (**type-only** reference to the boring-bash-owned `BashManagedServiceRequirement`), `registerAgentRoutes.ts` (surface service status `not-started|starting|ready|failed` into readiness; gate dependent tools/panels), the host callers (BBP5-002) to pass service requirements through.
- **Notes:** `BashManagedServiceRequirement` (declared in `@hachej/boring-bash/shared`, BBP5-001) per `../03` (`id`, `command`, `cwd?`, `ports[{port, purpose:'iframe'|'api'|'preview', public?}]`, `healthCheck?`, `teardown:'kill-process-tree'|'provider-default'`). Services require `exec` + `servicePorts` capability; deny otherwise with a stable code. No public exposure without an explicit port/proxy/iframe grant. Services run inside the selected source-of-truth/runtime view. Teardown on workspace/plugin dispose is deterministic. **Managed services receive only non-secret status/handles** (service status, port/proxy handles, grant ids) — a managed service is a sandboxed environment, so **no raw brokered secret ever enters its env**, grant or no grant. Raw brokered secrets are consumed **only by trusted-core tools running outside any sandboxed environment** (BBP5-007); a BBP5-007 grant authorizes such a trusted-core tool's *use* of a secret, it never opens an env-injection path into a service. Capture service logs with truncation/redaction, linked to the requirement id.

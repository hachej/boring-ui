...
- Headless `systemPromptMode` uses Pi's supported `DefaultResourceLoader` with the sealed cwd as both `cwd` and `agentDir`, one `SettingsManager.inMemory()` shared with `createAgentSession()`, explicit empty prompt sources plus literal prompt override callbacks, and every ambient-discovery flag disabled. The empty sources short-circuit Pi's prompt-file discovery before the overrides run. It does not evaluate static or hot-reloadable path/package resources; only trusted in-process extension factories supplied by the harness code or host remain enabled.
...
| Model registry and auth files | Credentials allowed; Pi settings default sealed. | `AuthStorage.create()` and model registry setup remain the Pi-owned credential trust boundary. Headless mode calls `readConfiguredDefaultModel({ allowPiSettings: false })`, preserving explicit `BORING_AGENT_DEFAULT_MODEL`, provider/id, Infomaniak, and custom-provider defaults while skipping `readPiSettingsDefaultModel()` and its file-backed global settings lookup. The pure `/api/v1/agent/models` route applies the same narrow policy. The same in-memory settings manager and sealed `agentDir` reach `createAgentSession()`. An explicit available `input.model` still wins; otherwise Pi may restore session state or choose from the credential-backed registry using empty in-memory settings. Workspace model/default behavior is unchanged. |
| System prompt cwd/workspace/file-tool leakage | Sealed for pure mode. | `systemPromptMode: 'headless'` supplies explicit empty `systemPrompt`/`appendSystemPrompt` sources so Pi does not probe prompt files, then supplies the minimal base and explicit host append through literal override callbacks. A path-like host fragment therefore remains literal. The mode also omits workspace/Python addenda and strips Pi's cwd line before the model turn. The snapshot test asserts no host cwd, sealed cwd, `Current working directory:`, `Workspace paths`, `AGENTS.md`, `SYSTEM.md`, `APPEND_SYSTEM.md`, `read/edit/write`, `find/grep/ls`, or `uv pip install`. |
...
- `packages/agent/src/server/http/routes/__tests__/models.test.ts`
  - `keeps the host default when the profile disables only Pi settings fallback`
- `packages/agent/src/server/models/__tests__/modelConfig.test.ts`
  - explicit env and provider defaults survive `allowPiSettings: false`, while a Pi settings-only default does not

...
- Add the pure `runtime: 'none'` path (no bash bundle spread into `tools`). Pure-mode rejection of non-empty attachments is **TEMPORARY P1 behavior**; T2 owns the capability split in `BBT2-007` (`none | direct | workspace`) and replaces the blanket rejection.
...
`AgentRouteBindingProfile` (`packages/agent/src/server/agentRouteBindingProfile.ts`) is the narrow HTTP-adapter binding seam for route registration over already-composed runtime/core values. It may carry adapter cleanup hooks such as `beforeRegister` and `dispose`; those are legitimate Fastify registration/close mechanics. It must not own capability/lifecycle **policy**: runtime selection, tool contribution, readiness meaning, attachment acceptance, and environment policy remain in host/core composition and later phase owners, not in the profile object.

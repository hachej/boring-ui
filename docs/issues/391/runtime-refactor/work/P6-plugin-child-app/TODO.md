...
- **Files create:** `packages/workspace/src/shared/agents/agentDefinitionDeclaration.ts` (import-free declaration schema + validator) + `__tests__/`. The canonical schema is `WorkspaceAgentsDeclaration { agents: AgentDefinitionDeclaration[]; defaultAgentId: string }`. `AgentDefinitionDeclaration` includes:

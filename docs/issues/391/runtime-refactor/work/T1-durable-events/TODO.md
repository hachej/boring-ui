...
- **Locked route family (from day one):** every new T1 HTTP route lives under the canonical agent-scoped prefix `/api/v1/agents/:agentId/...`. **T1 owns and defines this whole write+read family from day one** — the **canonical write routes** `POST /api/v1/agents/:agentId/sessions` (session creation, no `sessionId`), `POST /api/v1/agents/:agentId/sessions/:sessionId/prompt` (turn start / follow-up), `POST /api/v1/agents/:agentId/sessions/:sessionId/interrupt` (abort current turn), `POST /api/v1/agents/:agentId/sessions/:sessionId/stop` (end/close session), plus the **read/approval routes** `GET`/`HEAD` `/api/v1/agents/:agentId/sessions/:sessionId/events/stream`, `POST /api/v1/agents/:agentId/sessions/:sessionId/input`, `GET /api/v1/agents/:agentId/sessions/:sessionId/input`, and `GET /api/v1/agents/:agentId/pending-inputs`. The write routes map 1:1 onto the façade: `sessions`→`start` (new session), `prompt`→`start` (follow-up), `interrupt`→`interrupt`, `stop`→`stop`. Use a **canonical `default` agentId** until the P7 `AgentRegistry` lands; **P7 then only adds agentId resolution — no path migration**. `agentId` is a routing-scope path segment resolved in the route; it never enters the store/replay signature or `sessionStreamPath` (still keyed by `sessionId` only). This is the P7-locked family (`00` open decision 4) — do not ship T1 routes under an unprefixed `/api/v1/agent/...` shape that P7 would have to migrate. The legacy `…/pi-chat/:sessionId/{prompt,interrupt,stop}` routes stay live beside these until the T2 cutover deletes them (`TODO-T2` BBT2-006); T2's `dsHttpTransport` targets **only** this canonical family.
...
- **Two authorities, kept separate (state plainly):** the SQLite `EventStreamStore` is the **replay authority** — the durable, offset-addressed log that `stream(sessionId, { startIndex })`/the DS routes serve. It is **separate from** the pi session JSONL, which remains the **conversation-state authority** for harness rehydration (existing user sessions must keep loading — do not fold it into the event store). **Pending approval requests live in the event-stream SQLite DB**, never in the pi session JSONL and never in a new JSON file.
...
- **Files to create/touch**:
  ...
  - `packages/agent/src/server/http/routes/eventStream.ts` (new): Fastify route `GET`/`HEAD` `/api/v1/agents/:agentId/sessions/:sessionId/events/stream` (locked route family; `:agentId` is a canonical `default` until P7) that builds a WHATWG `Request` from the Fastify request, calls `handleStreamRead`/`handleStreamHead(store, sessionStreamPath(sessionId))` (store keyed by `sessionId` only — `agentId` is routing scope, not a store key), and pipes the WHATWG `Response` (incl. `ReadableStream` body for SSE) back onto `reply.raw`. Provide a small `fastifyToWebRequest(req)` + `webResponseToFastify(res, reply)` bridge (Node has `Request`/`Response`/`ReadableStream` globals on v22).
...
- **Files to touch/create**:
  ...
  - **Pending-request read API (named — S3's approval inbox and P7 consume this exact name):** add `pendingInputs(ctx: SessionCtx, opts?: { sessionId? }): Promise<PendingInputRequest[]>` to the **existing `sessions` member** (the `SessionStore`) exposed by `createAgent.ts`, callable as `agent.sessions.pendingInputs(ctx, opts?)`. **The façade stays exactly nine members — `pendingInputs` is NOT a new top-level façade member; it hangs off `sessions`.** **Scoping is explicit — the caller's tenancy `ctx: SessionCtx` is the first, required argument** of `agent.sessions.pendingInputs(ctx, …)`; this makes cross-tenant scope a mechanical property of the signature rather than an ambient assumption. (`start`/`send` take an `AgentSendInput`, not a `SessionCtx`; the HTTP adapters are what resolve `x-boring-workspace-id → SessionCtx` and pass it explicitly as `ctx`.) It returns the currently-parked input/approval requests **scoped to `ctx`** — narrowed further to a single `sessionId` when `opts.sessionId` is given, otherwise all sessions **visible to that `ctx`** (never another tenant's) — and **redacted** (tool params scrubbed of secret/credential material with the same redaction the transcript uses). A request parked under one `SessionCtx` MUST NOT appear for a different `ctx`. Back it by the `boring_pending_requests` table (this bead's persistence) so it survives restarts and reflects rows a different process parked. `PendingInputRequest` shape: `{ sessionId, requestId, kind: 'approval' | 'input', toolName?, toolCallId?, schema?, createdAt }` — **no raw params, no secrets**. Add the HTTP mirror on the adapter (locked route family; `:agentId` canonical `default` until P7): `GET /api/v1/agents/:agentId/sessions/:sessionId/input` (scoped) and `GET /api/v1/agents/:agentId/pending-inputs` (cross-session within the request's resolved `SessionCtx`, for the inbox), both returning the same redacted `PendingInputRequest[]`; the HTTP adapter resolves `x-boring-workspace-id → SessionCtx` and passes it as `ctx`. This is the single named pending-request listing API — do not leave it unspecified.

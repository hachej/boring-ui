...
- `createAgent()`/façade must stay Fastify-free (Phase 1 invariant, `06` Phase 1). The DS store + `stream(sessionId, { startIndex })` live in agent server core; only the DS route file imports Fastify.

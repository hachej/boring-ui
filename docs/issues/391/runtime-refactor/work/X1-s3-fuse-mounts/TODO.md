...
- Current #416 binding shapes already include `FilesystemBinding.mountPath` in `packages/boring-bash/src/shared/index.ts`; E1 generalizes this into entries carrying host-supplied `mountPath` for `EnvironmentAttachment` resolution. X1 consumes the E1 entry `mountPath` whose backing is a mount process, not a local dir. If E1's `Environment`/`EnvironmentAttachment`/`AttachedEnvironmentRuntime`/`ResolvedEnvironment` contracts are absent, STOP+report rather than inventing a parallel environment seam.
...
- Benchmark baseline is real data from `/home/ubuntu/projects/x1-bench/report.md` (`2026-07-05 12:22 UTC`): warm `rg` on the mount `0.18s` (local `0.02s`), append-100 `0.05s` (local `0.01s`), `git init+commit` `5.51s` vs local `1.17s` (`4.7x local`), sequential write 50 MiB `4.40s` (local `0.41s`). BBX1-009 encodes these as the initial locked thresholds. The report's readonly/backend-down semantic rows are **not authoritative** because the harness had PATH/ordering bugs; BBX1-007 owns re-verifying readonly rejection, backend-down errno/recovery, and the six Decision-12 smoke semantics.
...
- [ ] The six Decision-12 smoke tests pass; the rclone-FUSE-vs-local benchmark (BBX1-009) exists with recorded numeric thresholds from the real `2026-07-05 12:22 UTC` baseline: warm `rg <= 0.18s`, append-100 `<= 0.05s`, `git init+commit <= 4.7x local`, sequential write 50 MiB `<= 4.40s`.
...
- **Locked initial thresholds from real baseline (`/home/ubuntu/projects/x1-bench/report.md`, `2026-07-05 12:22 UTC`):** warm `rg <= 0.18s` (mount `0.18s`, local `0.02s`); append-100 `<= 0.05s` (mount `0.05s`, local `0.01s`); `git init+commit <= 4.7x local` (mount `5.51s`, local `1.17s`); sequential write 50 MiB `<= 4.40s` (mount `4.40s`, local `0.41s`). If CI noise requires tolerance, the PR must include the raw run, the adjusted threshold, and reviewer approval; do not silently widen thresholds.
...
- **Tests:** the benchmark is the artifact; CI runs it against MinIO and fails if a locked threshold regresses. Thresholds are encoded in `bench:mounts`, not left as PR prose.
...
- BBX1-009 `bench:mounts` exists and encodes the locked baseline thresholds: warm `rg <= 0.18s`, append-100 `<= 0.05s`, `git init+commit <= 4.7x local`, sequential write 50 MiB `<= 4.40s`.

...
- Current #416 binding shapes already include `FilesystemBinding.mountPath` in `packages/boring-bash/src/shared/index.ts`; E1 generalizes this into entries carrying host-supplied `mountPath` for `EnvironmentAttachment` resolution. X1 consumes the E1 entry `mountPath` whose backing is a mount process, not a local dir. If E1's `Environment`/`EnvironmentAttachment`/`ResolvedEnvironments` contracts are absent, STOP+report rather than inventing a parallel environment seam.

...
- [09-environments-attachable.md](../../architecture/09-environments-attachable.md) — the `Environment`/`EnvironmentAttachment` plus `AttachedEnvironmentRuntime[]`/`ResolvedEnvironment[]` model; host-supplied `mountPath` per attachment; the no-leak conformance suite runs against every delivered mount.

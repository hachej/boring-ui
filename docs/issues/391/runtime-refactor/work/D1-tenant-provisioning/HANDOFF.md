...
- [ ] Invite, embedded/browser, Boring MCP, signed WorkspaceBridge runtime/
      ...
      exact. The four POST actions keep their existing route limiters;
      `GET /sources` uses the same Fastify route mechanism but skips unscoped
      requests. Under trusted scope all five charge every authenticated valid/
      malformed/conflicting/foreign/nonmember request using only `request.user.id`
      plus frozen `requestScope.workspaceId` before the shared admission helper,
      which is the first route preHandler after limiting and precedes workspace/
      member/user-store/provider/transport effects. No manual D1 limiter or second
      budget store is added, and raw selectors never key or bypass a budget.

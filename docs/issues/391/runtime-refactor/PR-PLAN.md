...
| D1-004c3 — Boring MCP scope admission | Boring MCP binding + full-app wrapper/tests | <= 280 net lines; 25 min | global auth/unauthenticated 401 stay first; the four already-limited POST actions charge every authenticated valid/malformed/conflicting/foreign/nonmember request by `request.user.id` + frozen `requestScope.workspaceId` before admission; `GET /sources` keeps no route limiter and admits first; no D1 limiter; raw selectors never key/bypass budget; denials precede workspace/member/user-store/provider/transport effects |

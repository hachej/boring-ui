...
| 4 | D1-004a3a -> a3b -> a4a/a4b -> b1 -> b2a/b2b -> c/d -> D1-005 -> D1-006 | **priority 1; through D1-004b1/#698 landed; D1-004b2a active** | Land atomic managed-member and account-deletion authority, selector/admission fences, atomic N-binding boot, then the three-agent EU proof. |
...
| D1-004b2a — atomic managed-member mutations — **ACTIVE** | workspace-store contract; Local/Postgres stores; `members.ts`; focused route/store/conformance tests | <= 300 net lines; 30 min | scoped add is atomic create-if-absent; owner demotion/removal precondition shares the mutation transaction/lock; deterministic both-order race proof; allowed member operations and generic behavior unchanged |
| D1-004b2b — atomic managed-owner account deletion | `deleteUserCompletely.ts`, account route, focused transaction/route tests | <= 250 net lines; 25 min | trusted scoped workspace is checked under the existing serializable deletion transaction before effects; every existing owner rejects; non-owner/generic deletion unchanged; deterministic both-order race proof |
| D1-004c — remaining selector conformance | inventory invite resolve/accept and full-app agent/MCP/plugin/pane/WorkspaceBridge selectors first; split one PR per route family if over budget | <= 400 net lines per PR; 25 min | every selector rejects a foreign workspace before lookup/effects; only agent/deployment-bearing selectors derive the default deployment from scope; no generic policy framework |
...
**D1 total: 16 dispatch units (D1-001…003; D1-004a1, a2, a3a, a3b, a4a, a4b, b1, b2a, b2b, c, d;
D1-005; D1-006) per the accepted D1-R0 contract and its reality recut.**
D1-001…004b1 are landed, D1-004b2a is active, and the remaining units are ordered.
...
P1 readiness (#642), P6-R (#647), D1-R0 (#649), and D1-001 through D1-004b1
(#652–#654, #660, #662, #665, #667, #672, #675–#680, #684, #685, #690, #692, #694, #695, #698) are landed. D1-004b2a is the active priority-1 slice. P2 recut #641

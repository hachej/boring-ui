...
| D1-004a4a — landing/root seam — **LANDED #694** | bounded `d1Landing.ts`; narrow optional core root-handler seam; focused tests | landed | revalidated/escaped bounded landing, fixed same-origin auth return, drift/read failure redacted 503, authenticated root delegates to unchanged SPA; no public internal ids |
| D1-004a4b — readiness/production activation — **LANDED #695** | `d1ServerWiring.ts`, `d1Readiness.ts`; small host-scope/core type changes; literal-IPv4 Compose health probe/topology proof; `main.ts`; focused tests | landed | owner UID from process-level `core.env`; exact owner/process/proxy inputs; one shared reader; exact raw-IPv4 health/readiness boundary; bounded encoded-path denial; Linux UID range; early D1 activation; generic mode unchanged |
...
| D1-004c3 — Boring MCP scope admission — **LANDED #708** | Boring MCP binding + full-app wrapper/tests | landed | global auth/unauthenticated 401 stay first; four POSTs keep existing route limiters; `GET /sources` uses the same Fastify mechanism but skips unscoped requests; all five scoped routes charge authenticated valid/malformed/conflicting/foreign/nonmember traffic by `request.user.id` + frozen `requestScope.workspaceId` before shared admission; no manual D1 limiter/second store; raw selectors never key/bypass; denials precede effects |
| D1-004c4 — WorkspaceBridge runtime-claim admission — **LANDED #711** | two-phase runtime-token verification + core host callback + workspace HTTP runtime/refresh routes/tests | landed | signature/claims -> host assertion -> registry/definition capability authorization; before runtime/store/idempotency/recordUse/handler/mint; D1 error escapes as HTTP 421; browser remains c2-owned |

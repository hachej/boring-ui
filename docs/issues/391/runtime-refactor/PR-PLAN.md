...
| D1-004c3 — Boring MCP scope admission | Boring MCP binding + full-app wrapper/tests | <= 280 net lines; 25 min | global auth and its unauthenticated 401 stay first; generic key/status exact when unscoped; scoped route limiter keys only on `request.user.id` + frozen `requestScope.workspaceId` and charges every authenticated valid/invalid/foreign/nonmember request before admission; raw selectors never key or bypass budget; selector denial precedes app lookups/effects |

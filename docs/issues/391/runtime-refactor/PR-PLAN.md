...
| 4 | D1-004a3a -> a3b -> a4a/a4b -> b1 -> b2a/b2b -> c1-c5 -> d1/d2/d3 -> e -> D1-005a/005b/005c -> D1-006 | **priority 1; through D1-004c5/#713 landed; D1-004d1 active** | Land durable admission, complete real first-effect coverage, direct Bridge-operation admission, recoverable rollback fencing, approved policy, observed host attestation, atomic N-binding publication, then the EU proof. |
...
| --- | --- | --- | --- |
| D1-001 — plan and composition identity — **LANDED #652** | `deployment/agentHostPlan.ts`, `workspaceComposition.ts`, focused tests, descriptor exports and host composition wiring | landed | strict plan validation, canonical redacted composition snapshot/digest, final requirement inventory validation, exact stable errors; no filesystem mutation, Compose, routing, or CLI |
| D1-002 — immutable revision store and local CLI — **LANDED #653, #654, #660, #662, #665** | `deployment/agentHostRevisionCodec.ts`, `hostRevisionStore.ts`, `agentHostCommand.ts`, `agentHostCommandCliProtocol.ts`, `agentHostCommandEntry.ts`, wrapper/lock policy, focused tests | landed as reviewed micro-PRs | OS lock, expected-revision CAS, immutable candidate/COMPLETE records, atomic pointer, audit, destructive confirmation, rollback-as-new-revision; no HTTP management route |
| D1-003 — stable-process Compose and runtime-input boundary — **LANDED #667, #672, #675–#680** | `deploy/agent-host/compose.yml`, binding env/revision store, secure tmpfs materializer, fixed file provider, production composition, Docker proof | landed as reviewed micro-PRs | one ingress plus one full-collection core process; external `databaseRef`; durable roots; deterministic per-binding env; generation-addressed tmpfs values; exact-host RO mount; no force-recreate, raw-secret config, sibling-host mount, or source-checkout mount |
| D1-004a1 — explicit proxy policy — **LANDED #684** | core config/schema/shared types + `createCoreApp.ts`; Fly + self-host legacy opt-ins; deterministic agent-host edge network; new `edgeNetworkPreflight.ts` wired before `composeAdapter.ts` effects; focused tests | landed as three reviewed commits | absent/null proxy policy disables trust; only `apps/full-app/fly.toml` and the self-host deploy/env templates get a conspicuous temporary `legacy-unsafe` opt-in; agent-host requires an exact ingress CIDR and bounded hop count; pre-apply rejects non-default host-route or foreign-Docker-network overlap as redacted `AGENT_HOST_COLLECTION_NOT_READY/edgeNetwork` while allowing an exact owned agent-host network match; forwarded host remains unusable until exact-peer/chain replacement proof |
...
| D1-004a3a — canonical ingress artifact/proof — **LANDED #690** | `deploy/agent-host/{Caddyfile,compose.yml}`; exact approved Caddy digest in `composeAdapter.ts`; `apps/full-app/package.json`; proof script + adapter/topology/proof tests | landed | read-only config; production image equals `caddy@sha256:af5fdcd76f2db5e4e974ee92f96ee8c0fc3edb55bd4ba5032547cbf3f65e486d`; exact `caddy run` command; all RFC Forwarded forms absent at backend; absent/hostile/repeated/comma XFH becomes one raw original-authority value |
| D1-004a3b — trusted host scope — **LANDED #692** | new `deployment/hostSurface.ts`; optional core request scope; minimal pre-auth wiring; focused tests | landed | exact direct or Caddy peer/XFH/XFF authority resolves against the active collection before auth; raw ambiguity, hidden hops, unknown host, IP aliases, and untrusted forwarding fail generic 421; hostname grants nothing |
| D1-004a4a — landing/root seam — **LANDED #694** | bounded `agentHostLanding.ts`; narrow optional core root-handler seam; focused tests | landed | revalidated/escaped bounded landing, fixed same-origin auth return, drift/read failure redacted 503, authenticated root delegates to unchanged SPA; no public internal ids |
| D1-004a4b — readiness/production activation — **LANDED #695** | `agentHostServerWiring.ts`, `agentHostReadiness.ts`; small host-scope/core type changes; literal-IPv4 Compose health probe/topology proof; `main.ts`; focused tests | landed | owner UID from process-level `core.env`; exact owner/process/proxy inputs; one shared reader; exact raw-IPv4 health/readiness boundary; bounded encoded-path denial; Linux UID range; early agent-host activation; generic mode unchanged |
| D1-004b1 — workspace authority/signup — **LANDED #698** | request-workspace helper; `workspaces.ts`, member guard, auth proxy/composition, `postSignupHook.ts`, errors/tests | landed | foreign path 421 before stores; membership before lookup; one bound list; create/delete/default provisioning denied; auth proxy strips caller scope and installs trusted scope; post-signup invite only for exact bound workspace; generic mode unchanged |
| D1-004b2a — atomic managed-member mutations — **LANDED #700** | workspace-store contract; Local/Postgres stores; `members.ts`; focused route/store/conformance tests | landed | scoped add is atomic create-if-absent; owner demotion/removal precondition shares the mutation transaction/lock; deterministic both-order race proof; allowed member operations and generic behavior unchanged |
| D1-004b2b — atomic managed-owner account deletion — **LANDED #701** | `deleteUserCompletely.ts`, account route, focused transaction/route tests | landed | trusted scoped workspace is checked under the existing serializable deletion transaction before effects; every existing owner rejects; non-owner/generic deletion unchanged; deterministic both-order race proof |
| D1-004c1 — public invite scope — **LANDED #704** | core legacy/public invite routes + focused tests | landed | legacy path scope rejects before token lookup; public token may do one read-only hash lookup, then foreign scope returns the same non-enumerating 404 `invite_not_found` as an unknown token before status/counters/workspace/member/mutation/audit; transport rate accounting is outside application effects |
| D1-004c2 — embedded/browser selector convergence — **LANDED #705** | core server selector helper, pane status, agent/UI/automation/browser-Bridge tests | landed | inspect every header/query/body workspace selector; absent derives scope, malformed/conflicting/foreign returns 421 before membership/root/runtime/store/dispatcher; generic precedence exact |
| D1-004c3 — Boring MCP scope admission — **LANDED #708** | Boring MCP binding + full-app wrapper/tests | landed | global auth/unauthenticated 401 stay first; four POSTs keep existing route limiters; `GET /sources` uses the same Fastify mechanism but skips unscoped requests; all five scoped routes charge authenticated valid/malformed/conflicting/foreign/nonmember traffic by `request.user.id` + frozen `requestScope.workspaceId` before shared admission; no manual agent-host limiter/second store; raw selectors never key/bypass; denials precede effects |
| D1-004c4 — WorkspaceBridge runtime-claim admission — **LANDED #711** | two-phase runtime-token verification + core host callback + workspace HTTP runtime/refresh routes/tests | landed | signature/claims -> host assertion -> registry/definition capability authorization; before runtime/store/idempotency/recordUse/handler/mint; agent-host error escapes as HTTP 421; browser remains c2-owned |
| D1-004c5 — managed-agent MCP configured-target admission — **LANDED #713** | full-app managed-agent MCP route/tests | landed | after bearer auth but before storage/controller/store/dispatcher/stream, request scope must equal configured workspace; dispatcher receives trusted scope/default deployment; generic absent-scope exact |
| D1-004d1 — durable admission ledger and session fence — **ACTIVE** | core Drizzle migration/schema; `deployment/admissionLedger.ts`; root-CLI destructive-diff read; focused schema/store/real-PG tests | <= 400 net lines; 30 min | insert/read-only rows behind injected nonserializable `AttestedAgentHostDatabaseConnection`; one reserved postgres.js handle runs every transaction and advisory lock/unlock command; exact active binding/workspace/default-deployment recheck; additive revision may advance with an identical triple; missing/mismatched capability fails core/CLI closed and ref equality never opens production; no pooled-Drizzle escape, provider registry, update/delete API, or connection attestation |
| D1-004d2 — real agent first-effect admission | shared Pi service decorator; surgical `createAgent.ts` facade delegation; agent-host full-app wiring; slash/reload hooks; focused route/composition tests | <= 400 net lines; 30 min | admit before durable session create/delete, prompt/followup/control, and slash execution; `Agent.sessions.create()` delegates through admitted `runtime.service.createSession` and proves commit before store write, while delete stays service-delegated; reload admits after trusted binding auth but before reprovision, `beforeReload`, `reloadSession`, or any other effect; reads/list/subscribe, preload/all-ready, and cache population do not admit; completeness proof enumerates every current mutation; admission results are never cached; generic composition exact |
| D1-004d3 — direct WorkspaceBridge operation admission | runtime operation authorization seam; agent-host callback; completeness/order tests | <= 220 net lines; 20 min | every direct operation counts as use after scope/op/capability but before idempotency/store/handler; refresh does not admit; one shared ledger hook and zero downstream effects on failure |
| D1-004e — recoverable rollback fence | append-only rollback-journal migration/schema; `agentHostPlan.ts` stable error; new `deployment/destructivePublicationJournal.ts`; root-command integration; real-PG/pointer recovery tests | <= 350 net lines; 30 min | sorted removal-set session locks; durable prepared event -> pointer -> committed event; recover/finalize/resume/abort after every crash boundary; first/last-key + overlapping-set/no-deadlock proof |
| D1-005a — approved release and intended policy | new `deployment/{approvedHostRelease,hostSecurityConfig}.ts`; landed identity reuse; focused tests | <= 300 net lines; 25 min | strict nonsecret env schema; secret-bearing env rejects; approved core/ingress plus exact non-root migration process (`10001:10001`, no web entrypoint/privilege), Caddyfile, auth/browser/agent-host/MCP policy; no Docker mutation |
| D1-005b — observed host execution attestation | new `deployment/{verifyRunningHostArtifact,migrationRunner}.ts`; `composeAdapter.ts` recut; stopped migration/core/ingress seam; D1-003a reuse; focused tests | <= 400 net lines; 30 min | deterministic host/revision migration id resumes by exact state/id and cleans only after durable redacted zero-exit record; create/inspect core before exact-id start; create/inspect ingress stopped; capability binds ids; no unapproved execution/public port |
| D1-005c — collection preload and atomic publication | new `deployment/{bootCollection,preloadSignal}.ts`; root command-wrapper + `main.ts` integration; focused tests | <= 400 net lines; 30 min | live 005b capability; compare one root-owned expected `{ databaseRef, databaseName, serverAddress, serverPort }` to values queried on the reserved live handle and mint fresh `AttestedAgentHostDatabaseConnection` for production core/CLI; no provider registry or database-identity table; N P6-R; candidate==preload; preload/all-ready zero rows; pointer then exact ingress id; first use vs rollback shares 004d1 sorted-set fence; failed preload zero rows; unused-add rollback works, used-add removal rejects; stable N+1 |
| D1-006 — EU-host proof and runbook | `work/D1-tenant-provisioning/AGENT-HOST-RUNBOOK.md`, narrow proof script under `scripts/`, `golden-path.json` evidence path (owned by P8) | <= 300 net lines; 20 min | boot/add-agent/apply/rollback/cleanup commands; reproduce the landed edge-network overlap guard and exact-owned-network reuse on the EU host; three distinct agents/workspaces/hostnames in one EU deployment; three independent P6-R digests; setup-to-first-success timing; idempotent additive apply; N+1 continuity; exact rollback as a new revision; secret canary |

**D1 total: 25 dispatch units (D1-001…003; D1-004a1, a2, a3a, a3b, a4a, a4b, b1, b2a, b2b, c1, c2, c3, c4, c5, d1, d2, d3, e;
D1-005a, D1-005b, D1-005c; D1-006) per the accepted D1-R0 contract and its reality recut.**
D1-001…004c5 are landed, D1-004d1 is active, and the remaining units are ordered.
The agent-host runtime is the repeatable multi-agent Docker host lane. Its generic
...
P1 readiness (#642), P6-R (#647), D1-R0 (#649), and D1-001 through D1-004c5
(#652–#654, #660, #662, #665, #667, #672, #675–#680, #684, #685, #690, #692, #694, #695, #698, #700, #701, #704, #705, #708, #711, #713) are landed. D1-004d1 is the active priority-1 slice. P2 recut #641

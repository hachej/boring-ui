...
| D1-004c3 — Boring MCP scope admission | Boring MCP binding + full-app wrapper/tests | <= 280 net lines; 25 min | generic key/status exact when unscoped; scoped onRequest key uses only user/IP + trusted workspace and charges all invalid traffic before admission; raw selectors never key budget; selector denial precedes app lookups/effects |

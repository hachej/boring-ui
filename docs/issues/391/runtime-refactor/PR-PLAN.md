...
| 3 | D1-R0 + D1-001…004a1 | **merged; ancestry verified through [#684](https://github.com/hachej/boring-ui/pull/684)** | Exact D1 PRs: #649, #652–#654, #660, #662, #665, #667, #672, #675–#680, and #684. Plan/identity, revision lifecycle, stable Compose, secure runtime inputs, Docker boundary proof, exact proxy policy, and the edge-network guard are complete. |
| 4 | D1-004a2 -> a3a/a3b/a4 -> b/c/d -> D1-005 -> D1-006 | **priority 1; D1-004a2 active** | Land the mounted reader, canonical ingress proof, Host scope, landing/readiness, authority/admission fences, atomic N-binding boot, then the three-agent EU proof. |
...
| D1-004a2 — mounted active reader — **ACTIVE** | new `deployment/activeCollectionReader.ts`; focused tests | <= 400 net lines; 25 min | exact-DAC read-only active→COMPLETE validation through existing codecs; no mutation store, cache, watcher, P6-R, or secret read |
| D1-004a3a — canonical ingress artifact/proof | `deploy/d1/{Caddyfile,compose.yml}`; approved exact Caddy digest in `composeAdapter.ts`; `apps/full-app/package.json`; proof script + adapter/topology/proof tests | <= 400 net lines; 25 min target | read-only config; production image must equal the Docker-proven `caddy@sha256:...`; present RFC Forwarded gets exact redacted `400 D1_HOST_SCOPE_VIOLATION`; absent/hostile/repeated/comma XFH reaches the backend as one raw original-authority value; until then no D1 scope consumes forwarded authority and direct-only mode does not complete a3 |
...
D1-001…004a1 are landed, D1-004a2 is active, and the remaining units are ordered.
...
P1 readiness (#642), P6-R (#647), D1-R0 (#649), and D1-001 through D1-004a1
(#652–#654, #660, #662, #665, #667, #672, #675–#680, #684) are landed. D1-004a2 is the active priority-1 slice. P2 recut #641

...
- stop, interruption, and shutdown clear timers; Agent reload is rejected while capture is active.
...
12. **Feature remains default-off and CLI-owned.** CLI statically adds the front plugin once; the plugin consumes browser readiness metadata and contributes its own commands/UI only when the server confirms local flag-on readiness. `boring-ui workspaces` rejects flag-on; non-CLI applications treat the environment variable as inert and gain no routes/UI.
...
- **Path:** One app/internal `plugins/live-transcription/` package statically added by `packages/cli/src/front/App.tsx`; the plugin contributes its commands, composer surfaces, message cards, and lifecycle behavior through generic host seams. The local server composition supplies authority/upstream policy and the session-bound host capability. Direct WhisperLiveKit V0 adapter; no STT provider interface.
...
- **Audio seam:** deterministic resampling/downmix/frame tests for common 44.1/48 kHz browser input; bounded queues; overflow terminal behavior; cleanup counters return to zero. Short-dictation tests prove in-memory MediaRecorder state, bounded base64 validation, loopback REST forwarding, returned draft text, and no durable audio path.
- **Provider seam:** scripted `mode=full` WhisperLiveKit JSON covers speaker labels, timestamps, duplicate snapshots, malformed/oversized events, backlog drain, upstream close, and terminal finalization. Slice 1 updates `docs/issues/912/spikes/whisperlivekit/gh912-wlk-contract-proof.mjs` to inspect the active Diart/CPU patch path rather than treating Sortformer proof as sufficient.
...
- Browser audio becomes mono signed PCM16 little-endian at 16 kHz. One-frame ACK flow control, socket high-water marks, payload/JSON limits, and duration/transcript caps bound Boring-owned queues; malformed, oversized, or backpressured input interrupts. The composer always shows the active microphone mode and elapsed time; short stop inserts text into the draft, while live stop completes Markdown.
...
- **Exact commands:**
  ...
  /path/to/cpu-venv/bin/python docs/issues/912/spikes/whisperlivekit/gh912-wlk-cpu-stream-probe.py /path/to/consented-french-fixture.wav
  ```

...
| attention/presence/working-state adjustments | adapt | one Workspace-owned addressed AgentHost activity stream via `WorkspaceAgentFront.tsx` and `front/sessionActivity.ts` | focused server + UI tests |

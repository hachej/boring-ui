...
> as `createAgentHost()`, `EmbeddedAgentGateway`, and the `connectSession`
> connection object. The `AgentApplication`/`start()`/`AgentInvocation`
> contract below is superseded planning vocabulary; any remaining
> invocation-shaped need must build behind the shipped Gateway, not beside it.
> See `docs/issues/909/plan.md`.]

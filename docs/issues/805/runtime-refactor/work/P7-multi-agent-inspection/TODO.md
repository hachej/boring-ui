...
- Plan: `docs/issues/805/plan.md` § "Phase 7 — Multi-agent routing/session/search" (v1 body + v2 additions: surface adapters address agents via `agentId`; the `GET /api/v1/agents` list endpoint and `GET /api/v1/agents/:agentId/info` inspection endpoint). Exit: v1 + "two surfaces bound to two agents in one workspace do not collide".
...
- BINDING policy: `docs/issues/805/plan.md` "Simplicity & no-compat policy" — no shims, no abstraction without two real consumers, `TODO(remove:<bead-id>)` regime, migrate every importer in the same PR.

### Depends on

- **P6-R stateless resolved-value contract:** [`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md)
...
- **T2** ([`../T2-transport/TODO.md`](../../../../807/runtime-refactor/work/T2-transport/TODO.md)): `sessionId`-only public transport; the platform-addressing invariant guard. Surface `agentId` binding (BBP7-007) rides the two-handles boundary T2 formalized.
...
Make [`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md) Phase 7 + [`../../architecture/05-multi-agent-sessions-hooks.md`](../../../../391/runtime-refactor/architecture/05-multi-agent-sessions-hooks.md) § "Tests" checkable:
...
- Do NOT build the control-plane UI panels — that is [`../S3-control-plane-ux/TODO.md`](../../../../809/runtime-refactor/work/S3-control-plane-ux/TODO.md) (S3 consumes the `/info` endpoint this bead ships).
...
- **Notes**: Info shape (stable public contract, eve `/eve/v1/info` analog):
  ...
  ```
  **Never** emit secrets, broker credentials, provider key material, or environment handles (`00` invariant 14; `models.ts` safety posture). S3 ([`../S3-control-plane-ux/TODO.md`](../../../../809/runtime-refactor/work/S3-control-plane-ux/TODO.md)) is the consumer — these endpoints are the entire private-hook-free steering surface.
...
- **Acceptance**: [`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md) Phase 7 "agent inspection endpoint … consumed by workspace panels"; the Fleet page has a public source for every declared agent id; no private core hooks, no secret leak.
...
- **Notes**: `agentId` stays surface-side metadata that selects the routing scope — it is NOT passed as platform addressing into a core signature (BBP7-001 resolves it into `RuntimeScope`). One surface cannot address two agents on one continuation key ([`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md): a Slack channel or embed binds to one `agentId` per addressing entry).
...
- **Title**: The Phase 7 exit criterion as an executable test.
  ...
  - **Notes**: This is [`../../architecture/05-multi-agent-sessions-hooks.md`](../../../../391/runtime-refactor/architecture/05-multi-agent-sessions-hooks.md) "Isolation test" + [`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md) Phase 7 exit. UUID global uniqueness is a generation invariant, not an authorization/storage-key assumption. Reuse the T2 interleaved-transport harness.
...
Matches [`../../PR-PLAN.md`](../../../../391/runtime-refactor/PR-PLAN.md) P7 rows exactly:

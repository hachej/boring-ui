...
Add a workspace/company owner-only admin surface in core, reachable from the user menu, then stack two policy features on top:

1. **Admin layout:** `/w/:id/admin` page with tabs and a user-menu entry shown only when the current workspace role is `owner`.
...
- Company FS stack:
  - PR #432 preserves explicit filesystem identity in chat file references.
  - PR #446 adds named runtime filesystem bindings; tools/routes accept `filesystem: 'company_context'`, route read/list/find/grep/stat through `RuntimeFilesystemBindingOperations`, and keep mutations on the user workspace unless the binding permits them.
  - PR #454 enforces readonly company viewers and sanitizes denied company-context reads as 403/404 without leaking path existence.
  - The key seam is `RuntimeBundle.filesystemBindings?: RuntimeFilesystemBinding[]`, consumed by file/tree routes and filesystem tools. Policy should filter/omit the `company_context` binding for the current request/session, not create separate company-specific tools.
...
- company context binding behavior remains whatever the open company-fs stack advertises today;
- model list remains the existing registry-derived list;
- credits/billing admission remains unchanged.

## Acceptance

- A workspace owner can open **Company admin** from the user menu and see two tabs: **Context access** and **Model control**.
...
- Add a user-menu entry guarded by `useWorkspaceRole() === 'owner'` and current workspace id.
- Add read-only placeholder copy and test coverage for owner visibility/non-owner hiding, route rendering, and tab switching.
...
- In the core→agent runtime binding composition, add a request/user-scoped policy layer for `company_context` operations so list/find/grep/read/stat return only paths matching the current user's compiled allow rules. Do **not** bake one user's rules into a cached runtime bundle; include a cross-user cache/leak regression test.
- For denied reads/stat, return sanitized forbidden/not-found consistent with #454.
...
- Agent/workspace integration tests using a fake `company_context` binding: allowed path passes, denied path is sanitized, mutation remains readonly.

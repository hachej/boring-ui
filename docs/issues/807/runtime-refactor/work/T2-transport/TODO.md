...
Match `INDEX.md` Phase T2 exit criteria:
...
- **Files to touch**:
  ...
  - `docs/issues/391/runtime-refactor/architecture/08-pluggable-agent-surfaces.md` is the spec; add a short doc `packages/agent/docs/transport.md` (new) recording: the `ChatTransport` contract, and that `x-boring-workspace-id → SessionCtx` mapping lives in `getRequestContext` (`server/http/routes/piChat.ts`) — the pattern every surface adapter (Slack `conversationKey→sessionId`, Excel workbook→sessionId) replicates. Public agent APIs accept `sessionId` only.
...
- **Acceptance**: [`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md) Phase T2 exit — "a headless Node consumer drives the same session interleaved with the UI." Script runs clean; interleaved test green.
...
- **Precondition (hard)**: land this **last in the same T2 PR stack**, only after BBT2-003's DS transport passes `runTransportConformance` **and** the workspace playground runs unmodified on the refit (per the no-parallel-implementations rule, `INDEX.md` rule 4). Until then the legacy path may coexist (the additive DS routes are the flag, per README "Versioning & flagging").
...
- Public contract keyed by `sessionId` only; every adapter binds that address to
  ...
  (`transport.md`).
- Headless Node consumer interleaves with the UI against one shared `createAgent()` session ([`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md) Phase T2 exit).

...
transport direction, grants, provider registration, or product UI. The premises-first PR #1409 is merged; its canonical `docs/vision/`,
non-scheduling `docs/roadmap/`, split premise Beads, moved paths, and merge
contract remain authoritative. #1415 appends planning records only. Regardless of merge
...
**Planning result:** the live capability proof is complete and Owner Gate 1 is
**APPROVED** with the exact durable-store, shared-transport, tracker-ownership,
and explicit-relink decisions recorded below. Implementation remains deferred
on DIRECTION placement, provider reconciliation proof, and each slice's named
technical predecessors. Discovery-only 900.1 must reland as three small serial PRs: shared
...
cross-subject duplicate toolkit accounts, and require the owner-approved
explicit relink. Silent migration or account adoption is forbidden. Project/key/algorithm rotation defines discovery,
...
The approved durable binding is the host control-plane store with versioned
`secretResolverHandle -> credential revision` resolution. Raw keys, user
settings, and plugin-local JSON are never cleanup authority. Provider
creation-reconciliation evidence remains a technical prerequisite; the runtime
must not claim orphan freedom from a local record alone.
...
1. Apply the approved Gate 1 decisions to the Definition-of-Ready Beads and
   retain the clean independent T1 review record.
...
### Gate 1 decision record and dispatch prerequisites

Owner Gate 1 approved the host control-plane durable store with versioned opaque
secret handles, shared curated+catalog transport hardening, Architecture-Steward
ownership of the exact C2 tracker, and explicit account relink. This planning
slice does not make 900.1 `ready-for-agent`; dispatch still requires:

1. provider-supported creation idempotency/reconciliation or finite-TTL operator
   proof against the approved host store;
2. independent T1 review remains clean on both artifacts;
3. the existing 900.1a-c Definition-of-Ready Beads are undeferred only after
   DIRECTION placement and their external gates are recorded; and
4. bead text matches this canonical plan for pre-POST create intent/provider
...
# The current branch already contains merged #1409. Verify that conflict
# resolution preserved its ratified layers and rows while adding this plan.
for path in ["docs/vision/README.md", "docs/roadmap/README.md", "docs/plans/agent-runtime/gateway/plan.md", "docs/plans/agent-runtime/fleet-and-environments/plan.md"]:
    subprocess.check_call(["git", "cat-file", "-e", f"HEAD:{path}"])
current_ids = {
    r["id"] for r in map(json.loads, Path(".beads/issues.jsonl").read_text().splitlines())
}
assert {"wt-391-forward-9p50.2", "wt-391-forward-shell-ngfs.14.2"} <= current_ids
...
bv --robot-insights | jq -e '.Cycles == null'
git diff --check
...
All eight implementation beads exist in `.beads/issues.jsonl` and remain
**DEFERRED** after Gate 1 approval until DIRECTION placement and their named
technical predecessors clear. Each carries WHAT, exact proof commands,
...
**Blocked by:** provider reconciliation mechanism and explicit DIRECTION
placement. No Bead predecessor. Owner Gate 1 selected the host control-plane
store, versioned opaque secret handles, and shared curated/catalog transport
hardening; the combined plan records those choices together with caller-signal
scope, Session-ID bounds, and the complete create-gap contract.
...
owner-approved explicit relink, minimal live account parser with
...
**Blocked by:** `wt-391-forward-gh900-composio-exact-describe-0ijm`. Owner
Gate 1 selected inventory, quarantine, and explicit relink; the combined plan
records that choice plus 5-page/100-row/500-total/15-second ceilings and
fail-closed partial scans. The Bead remains deferred on its predecessor and
DIRECTION placement.
...
The owner assigned the Architecture Steward as the C2 predecessor tracker.
No exact tracker IDs currently identify C2's complete canonical predecessor
closure, so no false Beads edges were created. The Architecture Steward must
identify implemented nodes for at least P-1/upstream,
...
## Approved Gate 1 decisions and remaining external gates

1. **Approved — durable authority:** use the host control-plane durable store
   with versioned opaque `secretResolverHandle` values. Raw keys, user settings,
   and plugin-local JSON are forbidden as cleanup authority. Provider-supported
   idempotent create, authoritative intent lookup/listing, or finite-TTL operator
   proof remains required before implementation can claim reconciliation.
2. **Approved — shared transport hardening:** harden curated and catalog
   Composio Session consumers together. Product semantics remain unchanged;
   arbitrary HTTPS Session endpoints are rejected before key forwarding.
3. **Approved owner — C2 predecessor tracking:** the Architecture Steward owns
   the exact frozen-DAG Bead/PR/conformance mapping and C5×C6 handoff references;
   #900 creates no substitutes. Missing exact references remain an external
   implementation blocker.
4. **Approved — account transition:** inventory and quarantine legacy/new
   subject conflicts, then require explicit account relink. Never silently
   migrate, promote, or select an account.
...
Owner Gate 1 is approved for decisions 1–4. No implementation slice becomes
`ready-for-agent` in this planning Bead: DIRECTION placement, provider
reconciliation proof, exact external C2 references, and each slice's technical
predecessors remain mandatory. After #1409, its sole `DIRECTION.md` dispatch queue is the global placement
...
**Historical plan-review verdict:** **BLOCKER** at the reviewed PR revision.
Owner Gate 1 later approved the host control-plane durable store, shared
transport hardening, Architecture-Steward tracker ownership, and explicit
account relink. Those decisions are now integrated without rewriting the
historical review record. Exact provider-reconciliation evidence, external C2
references, DIRECTION placement, and slice prerequisites remain implementation
blockers; no implementation Bead is made ready by this planning slice.

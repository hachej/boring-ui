...
- `docs/issues/391/runtime-refactor/architecture/09-environments-attachable.md` — attachments plus auth-gated contributions/methodless facts; **`mountPath` is host-supplied**; credential/no-leak rules apply to every mount.
...
- BINDING policy: `docs/issues/808/plan.md` "Simplicity & no-compat policy" — no shims, no abstraction without two real consumers, `TODO(remove:<bead-id>)` regime, migrate every importer in the same PR.
...
- **P5** ([`../P5-provisioning-secrets/TODO.md`](../../../../805/runtime-refactor/work/P5-provisioning-secrets/TODO.md)): the **capability-fact + secrets-broker** machinery — the remote-worker handshake pattern (`reported | unknown`, fail closed) and the host-side secret-brokering rule (BBP5-007: brokered secrets are host-side handles, never enter a sandbox). X1's `mounts.fuseS3` capability fact and its STS-broker reuse that machinery; the sandbox receives a directory handle, never a secret (00 invariant 14).
- **E1** ([`../E1-environment-attachments/TODO.md`](../../../../805/runtime-refactor/work/E1-environment-attachments/TODO.md)): attachments, auth-gated contributions, methodless facts, and host-supplied `mountPath`. X1 consumes those contracts; if absent, **STOP and report**.

### Current reality this extends (verify before coding)

- In this prep worktree, `packages/boring-sandbox` is absent; P2 BBP2-000 creates it. There is no product mount/FUSE implementation in `packages/`, `plugins/`, or `apps/`; existing `rclone`/FUSE mentions are planning/reference docs. X1 is greenfield inside `@hachej/boring-sandbox` **after P2 exists**.
- Current bwrap code is still in `packages/agent/src/server/sandbox/bwrap/buildBwrapArgs.ts`. P2 BBP2-003 moves the bwrap provider/arg builder to `packages/boring-sandbox/src/providers/bwrap/*`; after P2, X1 reuses that moved builder for `--bind`/`--ro-bind`. If the moved builder is absent, STOP+report P2 missing; do not fork a second bwrap arg path.
- Current #416 binding shapes already include `FilesystemBinding.mountPath`; E1 generalizes this into host-supplied attachment entries and auth-gated contributions. X1 consumes that seam with a mount-process backing. If absent, STOP+report.
- The reusable readonly no-leak suite already exists at `packages/boring-bash/src/server/testing/readonlyProjectionConformance.ts` and exports `checkReadonlyProjectionConformance`. X1 adds a readonly S3 subject to that suite; it does not fork or rename the suite.
- Benchmark data from `/home/ubuntu/projects/x1-bench/report.md` (`2026-07-05 12:22 UTC`) is **provisional** because the report records PATH/ordering defects. BBX1-007 owns corrected readonly/backend-down semantics; BBX1-009 reruns the performance methodology and publishes raw evidence before any numeric threshold is locked.
...
- [ ] The six Decision-12 smoke tests pass; BBX1-009 publishes corrected rclone-FUSE-vs-local raw results and methodology. Numeric thresholds remain provisional until a reviewed follow-up locks them.
...
- **Benchmark-semantics boundary:** the `/home/ubuntu/projects/x1-bench/report.md` semantic rows (`readonly-write-rejected: FAIL(wrote!)`, backend-down cached reads may still serve, recovery needs remount) were produced by a harness with PATH/ordering bugs and are **not** accepted as product semantics. BBX1-007 owns the authoritative smoke re-verification for readonly write rejection, backend-down errno/recovery, and all Decision-12 smoke semantics.
...
- **Files create:** `packages/boring-sandbox/src/mounts/__tests__/fusePerf.bench.ts` (+ a `bench:mounts` script in `packages/boring-sandbox/package.json`) — a repeatable benchmark of the **default rclone `--vfs-cache-mode full` mount over MinIO** under real agent edit/build loops: warm `rg` over a source tree, append/edit→save→re-read, `git init+commit`, sequential write 50 MiB, plus `npm i`/`uv pip install` or an incremental build once the harness can run it with caches on local NVMe. Report wall-clock + syscall/metadata counts vs a **local-NVMe baseline** for the same workload, and vs the **caches-on-local-NVMe** variant (Decision 8) so the cache-placement win is measured, not assumed.
- **Provisional baseline only:** retain the earlier warm `rg`, append, git, and sequential-write numbers for comparison, but do not encode them as pass/fail thresholds. Publish corrected raw runs, environment, ordering, and variance; a reviewed follow-up owns locking thresholds.
- **Notes:** This bead exists because **Open Risk #1 (10-sandbox-deployment-eu.md): rclone/FUSE over S3 may be too slow for edit/build loops** — it must be *proven*, not asserted. A run that misses the threshold is a signal to move that workload off the FUSE mount (caches to local NVMe / selective sync), not to ship blind. Also feeds the deployment capacity model (10 "prove with `npm i`/`uv pip install`/`rg`/build"). **Do not claim readonly/backend-down semantics from this benchmark bead**: the baseline report says those semantic checks need re-verification because the harness had PATH/ordering bugs; BBX1-007 owns those smoke semantics.
- **Tests:** the benchmark is the artifact; CI runs it against MinIO and preserves raw results. Until the corrected baseline is reviewed, CI checks harness completion and semantics rather than provisional numeric thresholds.
- **Acceptance:** a numeric rclone-FUSE-vs-local benchmark exists for the edit/build loop; `bench:mounts` enforces warm `rg <= 0.18s`, append-100 `<= 0.05s`, `git init+commit <= 4.7x local`, sequential write 50 MiB `<= 4.40s`; the caches-on-NVMe win is measured (Decision 8); no readonly/backend-down semantic claim is accepted from BBX1-009.
...
pnpm --filter @hachej/boring-sandbox run bench:mounts         # BBX1-009 rclone-FUSE-vs-local thresholds: warm rg, append, git, seq-write-50M
...
- The six Decision-12 smoke tests pass in BBX1-007; readonly/backend-down semantics are re-verified there because the baseline harness had PATH/ordering bugs.
- BBX1-009 `bench:mounts` publishes corrected raw results; a reviewed follow-up locks numeric thresholds only after the flawed baseline is replaced.
...
- Any transitional code carries `TODO(remove:<bead-id>)` naming its deletion-owner bead; a later owner is allowed only when explicitly named per [`../../../plan.md`](../../../../391/runtime-refactor/INDEX.md), and no marker outlives its named owner's phase.

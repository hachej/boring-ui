...
See also: [REVIEW_DECISIONS.md](procedures/REVIEW_DECISIONS.md) for adopted/deferred findings from external reviews.
...
| **Addendum (2026-08-26, recorded from #416 work)** | Production filesystem-**binding policy source is the host application database**, never workspace files — file-backed binding policy is a CLI/dev/test-fixture convenience only. Source: `issues/416/company-fs/01-foundation-binding-model.md`. |
...
| **Trigger note (2026-08-26, from the agent-identity plan)** | The workspace-budget gate this decision footnotes shares a **specific trigger: the public-signup launch of authored agent identity** (the ID1 line) — the budget gate lands before or with that launch, not on a calendar. Source: `issues/809` agent-identity plan. |
...
| **Addendum (2026-09-07)** | Narrowed by D33 for installed products: a per-installation release/activation ledger is required; the AgentHost/controller rejection stands. |
...
| **Addendum (2026-07-20, recorded 2026-08-26)** | Owner-ratified amendments generalize this decision from an Anthropic-only model-key policy to a **multi-provider, workspace-scoped credential vault**: a pluggable `KmsBackend` custody seam (OVHcloud KMS as the deployed default — Vault Transit is a backend option, not mandated), one credential profile per workspace/provider pair, and the explicit correction that the dev Vault instance is not a production custody path. Every constraint in the rows above (fail-closed, no plaintext to clients/sandboxes, membership-before-resolution) applies unchanged across providers. Source: `issues/820/byok-secret-vault-plan.md` (ratified amendments section). |

## 28. Application agent fleets, Workspace orchestration, and shared execution environments

> **Signup-default clause superseded by Decision 32.** A trusted creator or
...
| **Addendum (2026-09-07)** | Narrowed by D33: the application fleet remains deployment-static; installed products are a separate host-brokered tier that resolves immutable releases per installation without a host redeploy. |

## 29. AgentGateway v0 — one session contract, one construction funnel

| Field | |
|---|---|
| **Status** | **Accepted (2026-08-08).** Ratified by the owner; records the architecture shipped in v0.1.91. |
...
| **Addendum (2026-08-26)** | The Level D re-evaluation trigger is **called** by the multi-agent surface ruling (RECONCILIATION §8c, ratified via PR #1409): concurrent multi-agent streams are now a named product need, so Level D conformance lands and goes **default-on before** the multi-agent engine is built. Tracked as premise [durable-streams] (bead `wt-391-forward-9p50`). The implementation PR carries the conformance proof; this addendum records the policy change only. Everything else in D29 stands. |
| **Addendum (2026-09-07)** | Narrowed by D33: an installation record and activation receipt exist beside the gateway; no mutable registry of executable code in the trusted tier, no second construction funnel, no agent-minted scope. |

## 30. Presentation-only hostname → landing surface

| Field | |
|---|---|
| **Status** | **Accepted (2026-08-08).** Owner ratified path A ("static deployment for now" — lean/config-driven, no AgentHost revival) from the [landing-surface reconciliation memo](plans/agent-runtime/cloud-vision/landing-surface-reconciliation.md) (PR #1153). |
| **What** | A host application may declare a **presentation-only hostname → landing map** in static deployment configuration: exact trusted hostnames (no wildcards, no registry, no runtime mutation) each mapped to bounded landing content — title, summary, and an optional CTA label, length-validated and HTML-escaped. The CTA links only to the ordinary member sign-in flow. The presentation map has zero membership, routing, agent-selection, or authorization effect and never rewrites a persisted Workspace default: hostname selects pixels, never authority. Separately, Decision 32 permits the exact trusted signup-domain hook to add one specialist Seat during initial Workspace creation while the application Default Agent remains primary. Implementation: #1154, #1156, #1165. |
| **Why** | PR #794 deleted the landing surface as cargo of the AgentHost removal; no decision ever ruled against bounded landing presentation itself, leaving hostname-keyed landing content unowned. Decision 23 already characterized landing as "a surface mapping rather than an authorization mechanism". Closing the gap with a small config-driven surface serves the two-hostname fleet deployment without restoring any rejected machinery. |
...
| **Supersedes / retains** | Supersedes nothing. Retains Decisions 25 and 28 except for the signup-domain initialization clause amended by Decision 32; the rejection of AgentHost/controller/publication machinery remains unchanged. |
...
| **Addendum (2026-09-07)** | D33 is the decision amending 25/28 for installed products; the #794 deletion is not reverted — the product runtime is a separate tier, not restored controller wiring. |
...
| **Addendum (2026-08-31)** | One authorized Agent session may own several bounded concurrent **explicit sandbox leases**; "one sandbox per user session" is superseded. Each lease remains one hardware microVM with an independent sandbox-scoped mutable SeaweedFS namespace under workspace authority and per-lease `/scratch`; leases never share mutable backing. The primary durable Workspace prefix remains separate and changes only through a trusted, request-bound, idempotent publication operation. Release may delete a changed lease prefix only after publication settles or separately authorized explicit discard; unpublished changes are never lost silently. Create replay is keyed by `(workspaceId, clientLeaseId)` and operations by `(workspaceId, sandboxId)`; sandbox identity never grants authority alone. Canonical tools remain unchanged. An additive trusted server plugin exposes only `sandbox` lifecycle and `sandbox_bash` execution for opaque leases; authored plugin selection cannot grant authority without an independent host-owned allowlist/factory, and there is no mutable current sandbox or caller-selected provider/profile. Every effect/data/internal-readiness endpoint uses request-bound short-lived authority. Create/destroy follow accepted-effect semantics once the durable nested tool-execution work in `plans/durable-streams-p1a-plan.md` P1-A3 lands; outcome-unknown is immutable and never replayed by the model/tool path. The initial managed Vercel disposable implementation is an explicit interim exception: ordinary lifecycle tool calls, no automatic interrupted-turn resume, no durable per-tool replay claim, and containment through quotas, TTL, owner cleanup, and provider reconciliation. It uses provider-enforced bounded execution and auto-snapshot expirations but does **not** satisfy the sovereign D31 accepted-effect milestone. After SIGKILL, stopped Vercel state may remain resumable until the bounded snapshot-retention deadline (24 hours by default), and operators need a sweep for earlier reclamation. Reconciliation in the completed design observes by durable effect/create key before any separately keyed idempotent maintenance cleanup. Active work pins before use, retirement drains before delete, and ambiguous cleanup remains single-owner, visible, retryable, and quota-counted without rewriting the original receipt. This changes lease multiplicity, namespace scope, and lifecycle proof only; the Firecracker/Kata hardware boundary and SeaweedFS data plane remain unchanged. Owner-directed Factory/sandbox reconciliation; implementation proof is specified in `issues/1081/sandbox-sovereign-design.md` and `sandbox-sovereign-build.md`. |

## 32. Creator and vertical signup adds a specialist Seat
...
| **Supersedes / retains** | Supersedes Decision 28's narrow statement that a signup-domain mapping initializes `defaultAgentTypeId` and Decision 30's claim that this old signup behavior remained unchanged. Retains D28's application fleet, Workspace-owned default, no continuing hostname routing/authorization, and no implicit rewrite of an existing default. |
...
## 33. Installed products run in their own runtime; one trusted installation/activation path

| Field | |
|---|---|
| **Status** | **Accepted (2026-09-07).** Owner-directed, ratified via PR #1561 (RECONCILIATION §13). Policy only — implementation PRs carry the conformance proof (same pattern as the D29 addendum). |
| **What** | Three layers: the **host** is control plane and broker (identity, installation records, activation, data/attention brokering, revocation); a **product runtime** is durable, one per installed product, runs the product's agents and generated domain operations and serves its generated UI, isolated by a host-selected mode `embedded` (in-process; single-tenant curator deployments only; default off) · `local` (bwrap/runsc lease) · `remote` (hardware microVM); a **sandbox** is the disposable D31 lease for tool calls, builds and checks and is never the product host. One trusted, host-owned installation/activation path: immutable content-addressed release manifest → installation record (scope, mode, bindings, generation) → compare-and-set activation settled once under a request key → undo as a new activation. Execution is authorized under actor ∩ installation ∩ job ∩ current policy. Builder agents are a distinct agent class with their own tool catalog. |
| **Why** | The owner's product obligation: an expert creates, installs for a separate consumer, adapts and maintains software without founder source edits. Today the only generated-server path hot-loads plugin code into the host process (`packages/workspace/src/server/runtimeBackend/`), which P0.6 and ARCHITECTURE-PLAN D-b forbid; and no release, installation or activation record exists anywhere. |
| **Rationale** | Reuses what exists: the two-phase `agent_host_binding_admissions` / `agent_host_destructive_publication_events` ledger shape for activation receipts, `SandboxProviderV1` providers for `local`/`remote`, the WorkspaceBridge registry for brokered operations, Seats for participation. Isolation at the installation, not the agent, keeps multi-agent products from paying cross-runtime hops while single-agent products match D-e unchanged. |
| **Supersedes / retains** | **Supersedes** D25's reading that immutable identity implies no deployment/publication store (a per-installation release/activation ledger is now required); D28's "changing what serves requires deploy/restart" *for installed products* (the application fleet stays deployment-static; installed products are a separate host-brokered tier); D29's "no registry, no controller" *insofar as* an installation record and activation receipt now exist (still no mutable registry of executable code in the trusted tier, no second construction funnel, no agent-minted scope); D30's "journaled publication requires a decision amending 25/28" — this is that decision, scoped to installed products; ARCHITECTURE-PLAN D-b "server disabled" for the untrusted tier (see §13c). **Retains** one gateway funnel, host-minted authority, no self-granted capabilities, D31's sandbox/lease separation and hardware-isolation floor, D32's additive enrollment, and P0.6's refusal of agent-written server code in the host process (embedded mode is the explicit, default-off exception). |
| **Re-evaluate when** | A product needs a runtime shared across installations; `embedded` mode is requested on a shared host; `remote` mode cost or latency fails the first consumer; or a second structurally different consumer shows the installation-level runtime unit is wrong. |

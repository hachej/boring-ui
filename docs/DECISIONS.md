...
| **Sales-process requirement (Fable review, 2026-07-14)** | The "named dedicated-VM customer" trigger must be a LINE ITEM in the deal-close checklist / contract template — not a fact discovered after signature. Concretely: the attestation-gap disclosure (host-artifact runtime-attestation is deferred/dormant, re-dispatched only on this trigger) and the `wt-391-forward-tz4` evaluation must fire INSIDE the sales motion, before the customer signs, not after. **No deal-close checklist doc currently exists in this repo** — creating one is a prerequisite for enforcing this; this decision only records the requirement, it does not create the checklist doc. `wt-391-forward-tz4`'s bead body is updated to carry the same note. |

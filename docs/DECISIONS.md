...
## 18. Workspace runtime install: symlink-to-global vs. self-contained

| Field | |
|---|---|
| **What** | The CLI is installed into a workspace's node runtime (`.boring-agent/node`) as a `file:` dependency pointing at the host's global install (`npm install file:…/.npm-global/lib/node_modules/@hachej/boring-ui-cli`). **Direct mode (no OS sandbox) embraces this**: npm links the dep and the global `node_modules` is reused as-is — no per-workspace copy. Provisioning is therefore **symlink-tolerant**: existence/reachability probes (`workspaceFs.exists`) do not enforce a realpath-within-workspace boundary that the npm-generated `.bin/boring-ui` shim (which resolves outside the workspace) would trip. **Sandbox modes (local/bwrap, vercel-sandbox) do NOT get a correct runtime from this symlink** — the target lives outside the sandbox bind mount, so the link is invisible/dangling inside the sandbox; they require a self-contained runtime (bundled tarball / preprovisioned base). |
| **Why** | In direct mode the agent already runs on the host with full privileges, so a realpath boundary is ceremony, and reusing the one global install is the fastest, lowest-disk way to make `boring-ui` + its (large) dependency tree available out of the box. Copying it per workspace (e.g. `npm install --install-links`) would pull the CLI's entire dep tree (`vite`, `tailwind`, `react`, `tsup`, `vitest`, …) into every workspace — slow, fat boots — which is exactly what the global-install link avoids. |
| **Rationale** | The escaping `.bin/boring-ui` symlink is not a design choice; it is npm's standard materialization of a `file:` *directory* dep (same as `npm link`). It caused a regression where sandbox-mode `exists()` followed the link, threw `symlink-escape`, broke the skip-vs-reinstall fingerprint short-circuit, and crash-looped provisioning on every boot — fixed by making `exists()` a boundary-free `stat()` reachability check (content reads keep the strict boundary in sandbox modes). Eliminating the symlink properly (self-contained runtime) without paying the per-workspace copy cost is a larger effort already designed in [preprovisioned-base-runtime-acceleration-plan.md](./plans/preprovisioned-base-runtime-acceleration-plan.md); until that lands, sandbox modes function because serving runs on the host (where the link resolves). |
| **Re-evaluate when** | The self-contained / preprovisioned-base runtime ships for sandbox modes (then the symlink-to-global becomes direct-mode-only by design, and the symlink-tolerance can be scoped to direct mode); OR npm changes how it materializes `file:` deps; OR a workspace must run the CLI *inside* a sandbox where the global path is not mounted. |

...
> **Current scope under Decision 28:** this records the original four-tool
> baseline, not the current complete catalog. F0b inventories every base/upload/
> UI/automation/plugin/diagnostics/isolated-code contribution. Decision 28 keeps
> the native Environment-backed file/shell base set and requires explicit retain/
> migrate/disable disposition for every other tool; it does not silently remove
> live tools or add destructive model operations.
...
> Agent direction, logical source/path-governance and EU-default principles. It
> merges named filesystem access into named Environments consumed natively by
> file tools and one-Environment-only exec. Decision 28 supersedes Decision
> 19's dependency details with Workspace orchestration,
...
> `ModelClientIssuer` adapters; Workspace requests one opaque model client per
> authorized invocation; cached AgentApplications never capture
...
| **What** | The v1 hosted-workspace model-key policy is bring your own key (BYOK) per Workspace. A tenant provider key is stored in the existing encrypted `workspace_settings` path protected by `WORKSPACE_SETTINGS_ENCRYPTION_KEY`, resolved only after web authentication plus Workspace membership, and exposed per execution only through an opaque invocation-scoped model client issued by the trusted consumer adapter. It is never returned as plaintext, captured by a reusable AgentApplication, written to sessions/tasks/events/logs/filesystems, forwarded to general shell/Environment executions, or baked into an image, Agent definition, or deployment artifact. CLI supplies its own trusted-local model-client issuer without Core; it preserves current explicit environment keys, Pi auth/settings, supported OAuth/subscriptions, and CLI/Pi model-selection precedence, but the issuer becomes the sole client-construction seam. The instance-level `ANTHROPIC_API_KEY` remains the explicit self-host and missing-Workspace-key fallback; ambient Pi auth files/OAuth are not a hosted-Workspace payer fallback. Platform-billed pooled keys are deferred to #809/BL1 after #819 supplies metering. |
...
| **What** | A host application defines one deployment-static **agent fleet**: stable `agentTypeId` values, declarative authored sources, and trusted host-owned behavior/plugin bindings. CLI YAML expresses the same semantic fleet through an independent adapter. Core/web and CLI consume Workspace independently; neither is routed through the other. Every initialized Workspace durably persists `defaultAgentTypeId`, which must resolve to the current app fleet before execution. An exact trusted signup-domain mapping may initialize that value for a newly created default Workspace, but domain has no continuing routing, membership, selection, or authorization effect and never rewrites an existing Workspace. Workspace is the bounded-context owner behind one consumer API but delegates internally to small fleet, session-attribution, governance, lifecycle, and orchestration services; it bundles and orchestrates streaming/control Agent applications, initially in process. For each authorized invocation, trusted Workspace governance compiles which **named Environments** that Agent may access, including the logical source, file subset, operations, network policy, and expiry. Each Environment has one rooted filesystem and `exec` runs only inside that Environment; a delegated Agent receives a separately governed Environment view of only the approved subset, never an ambient mount or copied/synchronized Workspace. `@hachej/boring-bash` owns the native Environment file/search/watch/exec contract and service; the native file/shell base set (`read`/`write`/`edit`/`find`/`grep`/`ls`/`bash`) names, exact-map-selects, and directly consumes one opened Environment. Every other retained tool that touches files/exec must do the same; non-Environment tools retain their existing owners. Agent-independent web/CLI file operations use a distinct member-operation authorization variant rather than fake Agent/session/task identity. `@hachej/boring-sandbox` owns hidden Agent/Workspace-neutral provider, persistence, confinement, and cleanup mechanics. Agent receives only the already-authorized Environment objects, opaque model client, and Pi-backed session runtime—not policy evaluators, membership, raw roots, backend handles, provider credentials, or lifecycle administration. |
| **Editor amendment to What (2026-07-27)** | [Status: #909 shipped the agent construction/session funnel in v0.1.91 as `createAgentHost()`, the frozen session-shaped seven-method `AgentGateway`, and `AgentFleetCompiler` fail-fast validation with stable codes including `AGENT_FLEET_PLUGIN_UNKNOWN` and `AGENT_FLEET_MODEL_POLICY_UNCOMPILED`; see `docs/issues/909/plan.md`. Decision 28's `AgentApplication`/`invoke` vocabulary is superseded by that shipped naming. Decision 28 now scopes strictly to Environment file/exec governance and Workspace orchestration, not agent construction or session-contract shape.] [Environment disambiguation: #909's shipped `EnvironmentLease`, keyed by `workspaceScopeId` plus `placementIdentity` and gated by `provisioningFingerprint`, is the narrower Host-internal runtime-placement concept. Decision 28's named multi-root Environment is a separate, still-unbuilt `boring-bash` file/exec governance layer that must compose with, not redefine, the shipped lease. `WorkEnvironment` is proposed only as a collision-avoiding name; this amendment does not decide the rename.] |
...
| **Rationale** | A `workspaceTypeId → defaultAgentTypeId + allowedAgentTypeIds` graph duplicates fleet configuration and turns signup routing into permanent product partition. The application fleet is copied, frozen, and fully validated before serving; changing it requires deploy/restart, not a registry/controller. Workspace validates its persisted default and fails stably without silent fallback when the deployed fleet no longer contains it. A named Environment merges the former named-filesystem and execution-context concepts: file tools and shell use one native path space and one policy, while different Environments isolate different logical sources or subsets. Governance outputs immutable access records, not self-authenticating tokens; only trusted Workspace composition can ask the service to open them. The Workspace runner owns opening/closing; operation interruption is distinct from Environment-lifetime cancellation. Environments remain live through streaming/follow-up and close exactly once on start failure or terminal result/stop/cancel/expiry/shutdown before the outward result settles; survivor-capable acquisition uses a private durable pre-allocation intent plus idempotent/discoverable provider creation before exposure, and unconfirmed cleanup cannot report success or escape quarantine/restart reconciliation. Agent sees operations only. Delegation lineage is Workspace-derived and every origin→target→purpose edge is freshly authorized. Verified installed-Pi delegation guards remain authoritative; Workspace supplies only missing depth/cycle/fan-out/execution-timeout/cascade behavior once, without a second dispatcher/model loop. Backend reuse is optional and unobservable, never a caller-visible contract. Workspace retains session authority and acting-Agent attribution, while the Pi harness remains the transcript/replay/follow-up-queue runtime behind an adapter. `boring-bash` owns operation semantics and coherence; `boring-sandbox` owns hidden confinement/provider mechanics and must not depend on Core, Workspace, or Agent identities. Existing `workspaceTypeId` data may remain temporary compatibility metadata pinned to `default`, but it has no Agent-selection, membership, routing, provisioning, session, or cache authority. |
| **Supersedes / retains** | Supersedes Decision 26's domain-routed typed-Workspace topology, Core typed list/select/create track, Workspace-type default/allowed policy, exact shared `WorkspaceRuntime + Sandbox` object-identity requirement across Agent types, and combined-host R1–R6 dispatch graph. Also supersedes permanent internal `RuntimeBundle`, `WorkspaceSandboxPairV1`, named-filesystem adapters, local/remote Agent branches, generic Environment lease/capability-token machinery, and moving Pi session mechanics into Workspace; they may exist only as contraction-bound migration inputs. Retains Decisions 19/21/22 and compatible parts of Decisions 25/26: package layering and EU/self-hostability; ordinary Workspace membership before effects; static/no-controller composition; declarative source plus trusted executable plugins; actor-neutral Agent identity; session/history compatibility; protocol bindings at edges; governed contracted projections; and rejection of AgentHost, deployment/publication content-addressed storage, mutable registries, authored executable catalogs, and second behavior composers. |
| **Editor amendment to Supersedes / retains (2026-07-27)** | [The shipped v0.1.91 `createAgentHost()`/`AgentGateway`/`AgentFleetCompiler` construction and session seam supersedes Decision 28's planned `AgentApplication`/`invoke` naming and contract shape. Decision 28 retains authority only for the still-unbuilt Environment file/exec governance and Workspace-orchestration scope; it does not reopen or replace the shipped Host/Gateway seam.] |
| **Re-evaluate when** | A named product needs public default changes, per-Workspace fleet allowlists, remote Agent deployment, remote Environment transport, per-Agent copied workspaces, stronger isolation than named-Environment subset views, cross-Workspace delegation, or restart-safe external tasks. Re-evaluation must specify authorization, persistence, filesystem authority, governance enforcement, session migration, rollout, rollback, and proof. It cannot let signup domain/Agent identity/named Environment access grant membership, silently rewrite a persisted Workspace default, couple Core and CLI through each other, let an Agent mint its own Environment authority, create competing authoritative file trees, or restore AgentHost/controller/publication machinery. |

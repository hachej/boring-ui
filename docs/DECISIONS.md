...
| Field | |
|---|---|
| **Status** | **Proposed (accepted on merge of this PR).** Owner-settled 2026-07-11, research-backed. |
| **What** | An agent exposes ONE consumption contract in the contracts layer: task lifecycle (`submitted`/`working`/`completed`/`failed`/`canceled` plus the interrupted state `input-required`), `contextId` grouping tasks into conversations, messages with typed parts, and artifacts. Semantics deliberately mirror A2A v1.0 so a future A2A binding is a thin adapter. Bindings of that contract: **UI** (human chat); **MCP** (the external entry gate, authenticated via ID1 — "MCP for tools/entry, not for peer agent dialogue"); an **HTTP API** (regular REST projection of the same task model, API-key auth via ID1); a **CLI** (drives the HTTP API; same identity layer); a **native in-process binding** for internal agent-to-agent consumption (no MCP loopback, no serialization; two-way chat via `input-required`); and **A2A as a FUTURE external binding only**, taken up when an external org needs multi-turn task-driving against hosted agents (A2A v1.0, Linux Foundation, Apr 2026). Adopting A2A internally is rejected as unnecessary — there is no trust boundary inside one deployment. Every consumer in every binding is a regular principal + workspace (extends the "MCP consumers are regular users" ruling in `docs/issues/391/runtime-refactor/work/ID1-agent-identity/PLAN.md`). |

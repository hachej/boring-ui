...
const baseRef = process.env.TYPECHECK_BASE_REF || process.env.GITHUB_BASE_REF || 'main'
...
if (globalChange) {
  console.log(`typecheck-changed: global typecheck input changed; running full typecheck`)
  run('pnpm', ['run', 'typecheck'])
}

if (changedNames.size === 0) {
  console.log(`typecheck-changed: no workspace package changes since ${base}; skipping`)
  process.exit(0)
}
